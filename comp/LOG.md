...
## WARNING How to use this log (read after every /clear)

- **Append an entry every working session** — newest at the top of the log below.
- Record: date, which milestone/rung, what changed (files), what was verified
  (tests/diffs), and what's next. Link commits if any.
- **A story is written ONCE — here, in its dated entry.** When a milestone
  completes, the relevant plan flips a status line in its ledger and points here;
  the top pointer below POINTS at entries, it does not retell them.
- If you are reading this fresh after a `/clear`: read the **CURRENT POSITION**
  block below, skim the newest entries, then open the active plan's ledger
  (`PROVE.md` while this axis runs) — and remember to append here when you stop.

> **> NEWEST (2026-07-20, 13th):** book, **Wave E started — the mechanical book-scale hardening passes.** Set
...
> intermediate representation); Part III proof terms (Martin-Löf type theory, propositions as types, Pi/Sigma
...
>
> **> PRIOR NEWEST (2026-07-20, 12th):** repo hygiene, **folder/README repair pass — SLICE 1.** Set asked to (a)
...
>
> **> PRIOR (2026-07-20, 11th):** book, **Wave D COMPLETE — the book is now 0-stub.** Wrote all six
> front/back-matter pieces from scratch (appendix_oracle was already done): `foreword.tex` (names the companion
> *The Language Stack* no.~4 as a bridge-not-gate, the three independent strands, the honest-AI-coöperation
...
>
> **> PRIOR (2026-07-20, 10th):** repo structure, **folder reconciliation COMPLETE — steps 4 & 5 done,
...
>
> **> PRIOR (2026-07-20, 9th):** repo structure, **folder reconciliation STEP 3 DONE — the dev tangle
...
>
> **> PRIOR NEWEST (2026-07-20, 8th):** repo structure, **folder reconciliation STEP 2 DONE — the prove
...
>
> **> PRIOR NEWEST (2026-07-20, 7th):** repo structure, **folder reconciliation STEP 1 DONE (safe regen).**
...
>
> **> PRIOR NEWEST (2026-07-20, 6th):** book, **interlude2 "The Quadratic Wall" DONE → WAVE C COMPLETE, Part II
> closed** (STUB→DONE, 95→1355 words, 1 stub→0; see the dated `### 2026-07-20 — book: interlude2` entry
> below). **Parts I, II, III are now ALL written; only Wave D (front/back matter) + Wave E (hardening)
> remain.** The book's engineering conscience on the append-is-a-copy quadratic. CORRECTED the stub's "twice…
> interning" framing: the pattern bit THREE subsystems, each authored knowing the last — (1) the emitter's
> line-join (~3 GB→<200 MB, balanced pairs), (2) the type checker's substitution walk (12→4.73 GB, closed-
> scheme short-circuit + RB-tree, the clean dict-vs-immutable + side-index story), (3) the optimizer's
> fixpoint fingerprint (12→5.6 GB, structural compare — the one ch12 tells). Sections: *An append that is a
> copy* (O(n) append → O(n²) loop; Python's list/dict hide it in the oracle) · *The three faces* · *The
> constant that proves it* (heap÷output² ≈ 0.025 across a 180× span = proof of quadratic WORK not a leak;
> live set linear throughout) · *Does Lark need a GC?* (no — nothing leaked, heap = dead corpses; a tracing
> GC costs the determinism the language exists for; ch:types affine discipline → deterministic reclamation /
> in-place reuse → doorway to Part~III). Refs ch:selfopt/ch:types/part:prove all resolve; no dev names in
> prose. `make` green, 0 stubs, 0 tics. **NEXT: Wave D — foreword, introduction (seed from `../AIMS.md`),
> conclusion, appendix_lark/method/repo (appendix_oracle done); then Wave E hardening + public move.** After
> a /clear doing book work, read this, then `book/PLAN.md` Ledger + current Wave. Everything below still valid.
>
> **> PRIOR NEWEST (2026-07-20, 5th):** book, **WAVE C — ch12 "The Optimizer Optimizes Itself" DONE** (STUB→DONE,
> 92→1351 words, 4 stubs→0; see the dated `### 2026-07-20 — book: ch12` entry below). The Part II capstone,
> the two fixpoint claims. (1) **Two claims, not one** — split the BEHAVIORAL claim (Lark optimizer == Python
> reference byte-for-byte, every level; fails as a *diff*) from the SELF-APPLICATION claim (the optimizing
> compiler runs on its own 9 modules as a *fixpoint*; fails as a *crash/resource cliff*). (2) **The first
> claim, green** — earlier differentials fed each module an oracle IR in isolation (the cheat);
> `optimize/harness/optcompiler_difftest.py` assembles all 9 under one `module Selfhost`, runs end-to-end with
> no Python in the loop, demands port==oracle byte-identical every file/level. Green (pinned invariant). (3)
> **The second claim, and the wall** — ~7000-line compiler → ~1.5 MB C, 3-stage ladder C1==C2==C3 = the self-
> application fixpoint, **−18.4%** emitted C at top level. The wall (NOT a bug): 12 GB in ~2 s BEFORE any pass,
> in the fixpoint bookkeeping — the sweep loop pretty-printed the whole 1.8 MB program to a string ×2/sweep
> (§8 arena-join quadratic); fix = structural comparison (O(n)/0-alloc), 12 GB→5.6 GB, differential still
> green. Honest tail: 5.6 GB workstation vs 520 KB Pico → wall lowered not removed → hands to interlude2.
> **CORRECTED a stale stub**: the draft said the wall was the "interning fix" — that was the OLD WRONG GUESS;
> the real blocker was the pretty-print-to-compare fingerprint (per M7.5 CLOSE). No exercises (matches
> ch08–ch10). `make` green, 0 stubs, 0 tics. **NEXT in Wave C: interlude2 "The Quadratic Wall" (STUB, 1) —
> the LAST piece of Part II; CORRECT its "interning" framing to the pretty-print fingerprint.** After a /clear
> doing book work, read this, then `book/PLAN.md` Ledger + current Wave. Everything below still valid.
>
> **> PRIOR NEWEST (2026-07-20, 4th):** book, **WAVE C — ch11 "The Back End" DONE** (POLISH→DONE, 1318→3123
> words, 4 stubs→0; see the dated `### 2026-07-20 — book: ch11` entry below). The two ways down from the
> optimized IR, with the string-vs-tag bug as the spine. (1) **TAC to C, again** — WHY a second C emitter:
> ch06's reads the *syntactic* tree + links a runtime; this one reads the *flat* optimized IR and is almost a
> transcription, one instr→one line of C; real `lk_sq` body incl. the dead trailing `return (lkw)0;` trap and
> the self-contained-C / full-width-`lkw` departure. (2) **The real machine, briefly** (SIDEBAR, per the stub) —
> same `sq` as RISC-V: 5 honest moves around one `mul` (`x*x` reads `s11` twice), linear scan (Poletto-Sarkar,
> can't coalesce) vs graph colouring (Chaitin; interference graph, k-colour=k-regs, coalescing feeds the
> peephole) as the window into what clang does silently. (3) **A bug the fixpoint found** = the spine: `match`-
> on-ctor and string `==` both surface as `t == <const>`; TAC→C compiled BOTH as integer tag tests → lexer
> misclassified every keyword → self-compiled parser recursed forever → SIGSEGV in generated `lk_pDecls`.
> Differential-vs-interpreter MISSED it (CEK does string-eq right; native path only hit under the optimizing
> self-compile); found with a C debugger on generated C; fixed by threading the per-fn IGetTag-dst temp set to
> disambiguate (both oracle + port). Moral = the chapter's ceiling: differential testing is blind to a fault in
> a path neither witness travels; the fixpoint is the largest test, and it's free. Pre-written "The last link"
> silicon sidebar + 4 exercises left intact. `make` green, 0 stubs, tics 0, all paths reader-facing.
> **NEXT in Wave C: ch12_selfopt (STUB, 4) then interlude2 (STUB, 1) — closes Part II.** After a /clear doing
> book work, read this, then `book/PLAN.md` Ledger + current Wave. Everything below still valid.
>
> **> PRIOR NEWEST (2026-07-20, 3rd):** book, **WAVE C — ch10 "The Passes" DONE** (the longest chapter; see the
> dated `### 2026-07-20 — book: ch10` entry below). The TAC optimizer, one pass per section, each with a real
> before/after pulled from the oracle at the right level via a throwaway driver (parse→infer→lower→optimize→
> pretty; `OptOptions(level=n)` / `enable={pass}`): (1) **const fold** (`3*4`→`12`, instruction-local — HONEST
> that Lark does NOT constant-propagate: copy_prop threads temps not constants, so `2+t0` stays); (2) **copy
> prop + CSE** = the ch08 flagship `(x+1)*(x+1)` collapsing to `t0*t0` at O1, three passes separated (cse finds
> dup→emits copy, copy_prop threads, dce deletes) + the immutable/by-value soundness note for CSE-over-alloc;
> (3) **DCE** (`x*x*x` unused → gone; introduces liveness + CFG "gently, needed nowhere else" per the stub —
> the one backward fixpoint over the graph; call/global guards); (4) **inlining** the enabling pass (`sq(4)`
> inlined → const-fold reaches in → `16`; honest about the copy+jump-to-next-line scaffolding left for ch11's
> peephole; INLINE_MAX 12); (5) **devirt + closure-elim** (`describe(Blue)`→`describe$Color` recovered from the
> dispatch stub's own tag-chain; non-escaping `add=fn(y)=>y+x; add(10)` scalar-replaced to `10+x`); (6)
> **LICM = the honesty gem**: present, correct, and a NO-OP on the ENTIRE corpus (verified: all 9 samples
> unchanged) — Lark's CFGs are acyclic DAGs (recursion is a call, not a back-edge), so the dominator-based
> loop finder finds nothing; kept because correct + validated on a synthetic loop; (7) **fixpoint sweep** =
> `twice(inc,x)` collapsing to `x+1+1` (no call, no closure) — nested closures peel ONE level per sweep, cap 8,
> corpus settles in 2–3; (8) **levels** table O0–O4, O0=the ruler (observable-equivalence baseline), + TWO
> honesties: O3 names unbox/fusion/arena_reuse which are DECLARED-NOT-WRITTEN (in LEVELS, absent from PASSES),
> and O4's real speed win is the BACKEND (regalloc/immfold), not TAC. **Opener REWRITTEN**: the inherited
> "two-to-ten-times faster" claim is unsupported — real BASELINES numbers are ~18–20% static shrink from TAC
> passes (8106→6526 instrs), with dynamic −38% (regalloc) / −23% (immfold) coming at O4/backend; opener now
> says "smaller not faster; speed is ch11." All 9 PASSES confirmed real: devirt/inline/closure_elim/const_fold/
> copy_prop/algebraic/cse/dce/licm. 3,430 words, 0 stubs; both lints clean, `make` green; tics trimmed (10→0).
> **NEXT in Wave C: finish ch11 backend (POLISH, 4 stubs — regalloc/peephole/immfold, the RV32 ASM_PASSES +
> graph-coloring), ch12 selfopt, interlude2.** After a /clear doing book work, read this, then `book/PLAN.md`
> Ledger + current Wave. Everything below still valid.
>
> **> PRIOR NEWEST (2026-07-20, 2nd):** book, **WAVE C — ch09 "Lowering" DONE** (see the dated `### 2026-07-20 — book:
> ch09` entry below). The typed-tree→TAC translation, section by section: (1) *from typed tree to flat code* —
> the pass is one recursive walk returning the operand that holds each node's value; straight-line nodes are
> dull by design, the interesting ones are value-that-becomes-control-flow — pattern matching shown as a real
> lowered ladder (Opt/unwrap: tag test → GetField bind → shared `r0` dest → fall-through chain → the
> belt-and-braces `__lark_match_fail` trap the exhaustiveness check makes dead; the twice-read tag left for
> ch10). (2) *where the types go* — the FIRST use of inference results for anything but rejection: `describe`
> shows one `show`/`+` lowering to `show`/`__show_float`/`__show_bool`/`__str_concat` by argument type, and
> `a+b` splitting Int (`t0=a+b`) vs Float (`call __float_add`) on identical source = devirtualization. (3)
> *lambda lifting* — `adder` capturing `n`: lifted `adder$lam0(env,x)`, `cap0=env[0]`, `closure(adder$lam0; n)`
> at the site; free-var analysis + uniform (env,param) convention + currying. (4) *the port, and the bug it
> found* — the pass threads five things through one recursive call; writing the Lark port, the binop case's
> `Cons(lv,Cons(rv,Nil))` tripped ch04's monomorphic-recursion wall a SECOND time → the `lwPair` deformation
> (full diagnosis deferred to ch04, which already names lwPair as scar #2 of 3); differential `make lowertest`
> 35/0. All listings verbatim from real lowerer runs (opt/fl/gi/sh/lam fixtures). 2,210 words, 0 stubs; both
> lints clean, `make` green. **NEXT in Wave C: ch10 passes, finish ch11 backend, ch12 selfopt, interlude2** —
> source the optimize tree (`optimize/`, esp. `opt.py`/`cfg.py`/`liveness.py`); ch04/ch07 for long-technical
> voice. After a /clear doing book work, read this, then `book/PLAN.md` Ledger + current Wave. Everything below
> (from the prior NEWEST) still valid.
>
> **> PRIOR NEWEST (2026-07-20):** book, **WAVE C STARTED — ch08 "An In-Between Language" DONE** (the IR chapter;
> see the dated `### 2026-07-20` entry below). Part II's opener: introduces *intermediate representation* and
> *lowering* from nothing for a reader who has met neither word; the tree-resists-optimization argument on
> `(x+1)*(x+1)` (two structurally-equal-but-distinct `Add` nodes → the tree hides the redundancy, TAC surfaces
> it as two lines with identical RHS); TAC introduced with real lowered output (`fn f(x): t0=x+1 …`); the
> abs() example for labels/blocks/cond-jump; "the IR is a language too" = printed + differential-tested
> (`make lowertest`, 35/0), and the `Val`→`Operand` rename forced by whole-compiler concatenation. All
> listings are verbatim from `oracle/lower.py` runs. 1,947 words, 0 stubs; both lints clean, `make` green.
> **NEXT in Wave C: ch09 lowering, ch10 passes, finish ch11 backend, ch12 selfopt, interlude2** — source the
> optimize tree (`optimize/`); ch04/ch07 for long-technical voice. After a /clear doing book work, read this,
> then `book/PLAN.md` Ledger + current Wave. Everything below (from the prior NEWEST) still valid.
>
> **> PRIOR NEWEST (2026-07-18):** post-summit sequence **AFFINE → FIXPOINT → BOOK** — both proof-frontier rungs
> now landed: **STRAND-AFFINE** M0–M2 spine (`affine_sound`, the A in Lark; `make affine`/`affine-controls`
> 4/4 green) and **STRAND-FIXPOINT** re-verified (`49a4921c` byte-identical, Thompson caveat articulated) —
> see the two dated `### 2026-07-18` entries below. Then the **book** finishing pass began: **ch05
> evaluator + ch06 emitter + ch07 fixpoint DONE** (Wave A, 1st–3rd of 4; PRESERVE strand scrapped;
> companion-book note to *The Language Stack* banked into the foreword/intro stubs). ch07 lands the F2 loop
> (three modules → 1,858-line compiler, `49a4921c` byte-identical over 4 stages, `read_all` = the one
> oracle-unfreeze) AND the F2⁺ typechecking fixpoint (six modules + gate, 42/0/3 differential, the
> >12 GB self-compile scars named output-neutral). **interlude1 DONE too → WAVE A COMPLETE: Part I
> (ch01–ch07 + interlude1) fully written, 0 stubs.** **WAVE B STARTED: ch14 "A Proof Checker in C" DONE**
> (the trusted lcore base, deliberately lean) and **ch15 "Proving the Language" DONE** (the honest
> metatheory — preservation intrinsic/free, universal progress hits the lcore index-refinement wall so it
> proves values+rules+eval-soundness instead; now load-bearing under the V3 refinement relation; see dated
> entries) and **ch13 "What ``Correct'' Could Mean" DONE** (Part III's framing map: tests-vs-proofs, the
> four claims, floor-not-ceiling, no-verified-verifier handoff) and **ch17 "What a Verifier May Not Do" DONE**
> (the adversary/torture chapter: the nine false proofs by shape, adversary.py/torture.py/invariants.py, the
> hash quadratic, the eighth+ninth false proofs with verbatim verdicts, the tenth that never shipped) and
> **ch18 "Measures and Trees" DONE + ch16's residual stub closed** (measures ghost/erased, two-deep match,
> build-a-bst, the destructing-bst boundary, cost-of-firing O(n²)→O(n log n), the four-boundary ledger; all
> from real fixtures with verbatim verdicts) and **interlude3 "The Equality Seam" DONE → WAVE B COMPLETE**
> (Float == exists-and-lies vs String == does-not-exist-and-is-believed; the three worlds; the vetoed fix =
> ch17's tenth). **Part I + Part III now fully written, 0 stubs. NEXT = WAVE C (Part II optimization): ch08 ir,
> ch09 lowering, ch10 passes, finish ch11 backend, ch12 selfopt, interlude2 — source the optimize work →
> `optimize/`** (`book/PLAN.md`; ch16 remains the Part III voice model, ch04/ch07 for long Part II chapters). Also DONE this session: a styling pass (heading no-hyphenation, tabularx tables, listing
> reflows). After a /clear doing book work, read this, then `book/PLAN.md` Ledger + current Wave.
> Everything below is the prior (still-valid) V3 position.
>
...
>     on an indexed family with IHs ✓; definitional unfolding at constructors ✓; and
...
> ladder now says so per rung (V2.4 OK as foundation+b+c1–5; V2.5 ⏸ not picked up, reopen deliberately or
...
>     regression → **❌ 4 FALSE PROOFS, all `ZeroDivisionError`** (`nuse(Leaf, 0)`, `rd` declared nonzero,
...
>
> **>> START HERE (updated 2026-07-14). THE ACTIVE AXIS IS `PROVE.md`.**
> **Step 0 OK, V1 OK and V1′ (hardening) OK are DONE: Lark can state a contract, the checker can
> prove it, and it no longer hangs or crashes.**
> `{v : Int | v >= 0}` parses and **erases**; `08/src/refine.py` generates VCs bidirectionally;
> `08/src/solver.py` decides QF-UFLIA **from scratch** — congruence closure + the Omega test
> (real shadow, dark shadow, splinters) under DPLL(T). **No Z3. The plan's `--smt` escape hatch
> was never needed and was never built.**
>
> `make -C 08 test` = **drift 22 identical / 3 extended / 3 added** · **conservative 90/0/0 —
> unchanged from the Step-0 baseline, which is what makes it readable** · **solver 16/0** ·
> **prove 45/0** (`prove/`: 16 safe/unsafe pairs + the affine finding + 7 measure negatives + 12
> files that are checked *and then run*). **`make -C 08 harden` = `test` + robust 45/0/0 crashes +
> fuzz 3000 cases / 0 unsound + `adversary` — random PROGRAMS, proved ⇒ it runs, which is the
> suite that found three of the seven false proofs — + `torture` 14/14 (H, below): hostile-but-legal
> programs, where the only failing grade is **no answer at all** — + **`invariants`** (I, below):
> the rules, checked by a machine, over the whole corpus.** Details in `08/README.md` and
> `prove/README.md`.
>
> **>> I — INVARIANTS, AND THE EIGHTH FALSE PROOF OK (2026-07-14).** The ask that started it: *"there
> have been many chance findings — a bit too many for my taste. Can we do better, and start with more
> severe programming?"* **Yes, and the answer is embarrassing in the right way.** Every bug in this
> axis was a FINDING: a hostile program (V2.2‴), a suspicious read (V2.2′), a profiler (H). That method
> works — it worked seven times — and **that is exactly what is wrong with it: it cannot tell you the
> difference between "there are none left" and "I stopped looking", which is the very distinction we
> spent V1′ and H forcing the CHECKER to make about itself.** So the rules stopped being prose and
> became `08/tests/invariants.py`: three properties, checked over all 92 files, every run.
> **I1 COVERAGE** — every expression node in a checked body is *visited*, and once. (Missed = the shape
> of V2.2‴(7); revisited = the shape of H's quadratic. One question — *how many times did you look at
> this node?* — and the only right answer is once.) **I2 ASKING** — every integer division walked raises
> exactly one obligation (the shape of V2.2‴(5); coverage alone cannot see it, because the node WAS
> visited and the checker still declined to ask). **I3 WORK** — the checker's own cost is linear,
> checked by **COUNTING, not timing**: the counters are deterministic, so it is a regression test and
> not a wall-clock coin flip. (The SOLVER is held to a different bar on purpose: Omega on an n-term
> system is honestly superlinear and is *fenced*, not fixed. Conflating the two would either excuse a
> quadratic in the checker or forbid an honest one in the solver.)
> **AND I1 FOUND AN EIGHTH FALSE PROOF WITHIN MINUTES — THE FIRST ONE THIS PROJECT EVER FOUND ON
> PURPOSE.** 58 nodes across 7 corpus files came back **never walked**, and they were all the same
> thing: **the body of a lambda.** `check` walks a lambda's body (it has an `RFun` to push in); `synth`
> returned `ROpaque()` **without looking inside** — and an unannotated `let h = fn (a) => …` goes
> through `synth`. So:
>
>     let h = fn (a : Int) => 100 / a in h(0)      →  ok: 0 obligation(s) proved, exit 0
>                                                  →  then: ZeroDivisionError
>
> It is **V2.2‴(7) exactly** — a piece of the program the checker never walked — *wearing a hat nobody
> had thought to look under.* One of them was a real division inside `07/tests/25_torture.lark`
> (`filter(fn(x) => (x - (x / 2) * 2) == 0, xs)`), never once examined in the fork's whole life, in a
> file named *torture*. **THE RULE GENERALISES, AND THIS IS THE ENTRY TO REMEMBER: "every
> sub-expression is walked exactly once" is not a fact about EXPRESSIONS — it is a fact about EVERY
> PLACE CODE CAN HIDE, and a lambda is one of them.** Fixed in `refine.py` (synth binds the params to
> what is DECLARED about them — an unannotated one is opaque, which is the truth, so the body must be
> proved for EVERY argument — walks the body, and still returns `ROpaque`: walking is about the
> obligations inside, not about learning what the function returns). New pair `prove/22_lambda_*`;
> `25_torture`'s pin 4 → **5** obligations (the new one proves). prove **47/0**.
> **AND THE GENERATOR'S OWN BLIND SPOT, AGAIN.** `adversary.py`'s docstring has said *"a local lambda's
> result"* since V2.2‴ — and **the generator could not emit a lambda at all.** It knew the words and not
> the grammar. Now it does (`let g = fn (p) => … in g(…)`), and — per the standing rule, *never trust a
> fuzzer you have not seen fail* — the fix was **disarmed** and both harnesses were watched to fail:
> the adversary with a false proof, I1 by naming the exact unwalked nodes. **A generator only ever finds
> bugs in the language it can speak; its grammar is a claim about what programs exist, and an unexamined
> grammar is an unexamined claim.**
> All green: drift 22/3/3/0 · conservative 90/0/0 · solver 16/0 · **prove 47/0** · robust 45/0/0 ·
> fuzz 0-unsound · adversary clean · torture 14/14 · **invariants: 3954 nodes / 0 never walked / 0
> walked twice, 38 divisions / 38 obligations, growth 2.00× per doubling.**
>
> **> THE V1 FINDING (§7's open question, now answered — book-grade, both halves).**
> **(a) Naming an affine binding in a predicate is a MENTION, not a USE.** True *by
> construction* (refinements erase before inference walks them, so a predicate cannot increment
> a use count) and *sound* because **affinity restricts use, not truth** — Lark is pure, so
> consuming a value moves it rather than mutating it, and a fact proved about an affine binding
> stays true for its whole scope. **In a language with mutation the rule would be unsound: the
> permission is paid for by purity and nothing else.**
> **(b) The predicate is free; the GUARD that establishes it is not.** `if size(b) > 0 then
> take(b, 0)` evaluates `b` and then passes it on — two uses — so the affine checker rejects it.
> The borrow idiom that fixes the affine error (`size_of(b) : (Int, Buf)`) typechecks and *still*
> cannot be proved, because V1 has no refined tuple components. Affine and refinement are
> individually fine and **jointly blocked for non-Copy types** → **refined products are a V2
> requirement, derived rather than guessed.**
>
> **> THE V1′ FINDING (2026-07-14): a budget must give up in the SOUND direction.** The fuzzer
> found the solver *hanging* — not answering wrongly, not answering at all, which for a checker
> is worse. `Theory.consistent` asserts distinct literals distinct (`3 ≠ 4`) and `_lia` splits
> every `a ≠ b` into `a < b` or `a > b` at a cost of 2^k, so a formula merely **mentioning**
> seven constants arrived with 21 disequalities and asked for 2²¹ calls to Omega. Fixed twice
> over — ground disequalities settled by inspection, and a hard fence (`MAX_DISEQ`, `MAX_SPLITS`)
> behind a new `Budget` exception. **Every `except Budget` resolves towards *consistent*:** that
> makes `satisfiable` true, `valid` false, and the answer **"cannot prove"**. An exhausted budget
> can cost a proof that was deserved; it can never manufacture one that was not — checked on a
> contradiction hidden behind an exhausted split, where the solver declines to prove the goal
> *even though it follows ex falso*. It surrenders capability, not soundness. (Exhaustion used to
> raise `PredError`, i.e. a resource limit was shown to the user as if their program were
> malformed.) The corpus sweep separately caught three frontend exceptions escaping as raw Python
> tracebacks — and **two true positives in Lark's own code**: `Div(a,b) => eval(a) / eval(b)` in
> `07/samples/05_expr.lark` and `09_parser.lark` is a real latent division by zero. All of it
> **output-neutral** on every pinned suite.
>
> **>> V2.2′ — THE FOUR FALSE PROOFS, FOUND AND CLOSED (2026-07-14). READ THIS ONE FIRST.**
> After V2.2 landed I asked whether anything was still unsound, went looking, and the answer was
> **yes, four times**. Every one was reported `ok: N obligation(s) proved` and every one then
> **divided by zero when run** — the only false proofs this project has produced. They are two
> families of two: the checker **believing something it had not established** (a, b), and the
> checker **speaking about a name that no longer meant what it thought** (c, d).
> **(a) A FLOAT MAY NOT ENTER THE LOGIC.** `x == x` is VALID in the logic (equality is an
> equivalence relation; congruence closure makes it reflexive, and must) and **FALSE at run time
> when x is NaN** — which `0.0 / 0.0` and `float_sqrt(-1.0)` both produce (`cek.py:304`). The else
> branch assumed `not (x == x)`, found it contradictory, and was proved VACUOUSLY. The other half
> needs no NaN: `0.0 == -0.0` is TRUE at run time and they are DIFFERENT VALUES, so congruence
> concludes `f(0.0) == f(-0.0)` for an `f` that can tell them apart (`1.0 / x` → ±inf).
> **Reflexivity fails one way and substitutivity the other — and those two are the whole of what
> sort OTHER hands out.** OTHER = *named but not opened*, right for a String/Buf/List, **too much
> for a Float.** Fix: `refine.FLOAT`, a sort at which **NO TERM IS EVER BUILT** — not opaque,
> *unspeakable*. It had to be set on BOTH roads into a binder's type (`rtype_of_mono` for inferred,
> `_rtype_of_syn` for ANNOTATED) — the first fix looked right and did nothing, because the probe's
> parameter was annotated.
> **(b) "I COULD NOT TRANSLATE THIS" IS NOT "THIS IS TRUE" — and there is not a Float in it.** Both
> `if` sites read `cf = formula_opt(c, env) or Top()`. `formula_opt` returns None for *cannot
> express this*; that `or Top()` made it **true**, so the else branch assumed `Not(Top())` =
> **FALSE**, and from false every obligation in the branch follows. The guard that did it is
> `x * y > 0` — merely **NON-LINEAR**, the everyday way out of a linear fragment, not an exotic
> corner. Fix: `refine._branch` — an unreadable condition constrains NOTHING, so NEITHER branch
> learns anything and the negation is never taken.
> **(c) A UF SYMBOL IS A GLOBAL FUNCTION, AND ONLY THAT.** The logic names symbols by their
> **source name**, and source names **shadow**. `05_buf`'s idiom — a contract mentioning the
> program's own `size` — is discharged at the call site by a guard that also says `size(b)`; put a
> `let size = fn(x) => 1` in scope there and the guard is about **a different function**, but both
> are the symbol `size`, and congruence identified them. Fix: `term_opt` refuses to build `App(f,
> …)` when `f ∈ env.vsorts` (the value variables — params, lets, match binders — never a global
> fn). **A locally bound function is unspeakable.** It never had a contract to read off or an
> equation to instantiate; all it had was congruence, and congruence was the bug.
> **(d) AN AXIOM IS ABOUT A FUNCTION, NOT ABOUT A NAME — and this one is worse, because nobody has
> to CALL it for it to fire.** `refine.NONNEG_UF = {string_length}` asserts `string_length(t) >= 0`
> of every such term in a VC. True of the **primitive**. A program may declare `fn
> string_length(s : String) : Int = 0 - 4`, which overrides the builtin *including at run time* —
> so the axiom became a lie about the program's own function, and `div(100, string_length(s) + 4)`
> was proved and divided by zero. Fix: `Refiner` intersects `NONNEG_UF` with the names the program
> **leaves alone**. Temporary in the good sense, and the promise was kept: **V2.3 deleted
> `NONNEG_UF`** (below), and a theorem cannot be captured.
> **THE RULE IS V1′'s, ONE LEVEL UP.** V1′ made every exhausted budget *inside the solver* resolve
> toward "cannot prove". The same now binds the checker that BUILDS the obligations: **where it
> cannot speak it must say NOTHING — never `true`, never a negation, and never a claim about a name
> it does not own.** `prove` **27 → 35** (`13_float`, `14_guard`, `15_shadow`, `16_axiom`, each a
> safe/unsafe pair; **every safe file RUN**); everything else unmoved, **robust 45/0/0 with every
> verdict where it was** — the tell that the proofs the fix deleted were never proofs.
> **>> V2.2″ — THE IMPL HOLE, CLOSED OK (2026-07-14). An impl body is a body.**
> `refine.py` imported `ImplDecl` and **never matched it**, so for five rungs the code inside an
> `impl` was never walked. Not a false proof — a refinement on a trait method signature was
> *dropped* rather than trusted, so a lying impl could not discharge anyone's goal: **sound, and
> sound by luck.** It was the failure mode next door: `n / 0` inside an impl body, nobody reading
> the trait, verdict **`ok: 2 obligation(s) proved`, exit 0.** The division was never *seen*.
> **A weak answer is a result; a count that omits the obligation is a WRONG REPORT** — the first is
> allowed, the second never.
> **The fix is two halves of one decision.** `_check_impl` checks each `ImplMethod` as an `fn`
> against the contract in the **trait** (`_impl_rtype`: refinements from the signature, HM types
> from the `TFnDecl` infer.py already built, param names from the impl) — **and because every impl
> is now held to that signature, a CALLER may read it** (`_rtype_of_sig` installs the trait's
> refinement as the method's global contract). Inseparable: **a contract the callers trust and
> nobody checks is exactly the false proof V2.2′ was about.**
> **Two results fell out.** (a) **Nothing needs substituting** — the plan said "impl type
> substituted for the trait variable", but a refinement can only sit on a base the logic can NAME
> and a trait variable is not one, so a signature contributes *predicates and nothing about the
> type*. (b) **A trait not declared here (`Copy`, `Show`) is no excuse to skip the body** — every
> part falls back to its HM type, and **a trivial contract is still a contract.** ("I have no
> contract for this" must never become "I need not look.") Plus V2.2′(d) one place further:
> `NONNEG_UF` is withheld from names a **trait** declares too.
> **The counts are the finding.** `prove` **35 → 37** (`17_impl_{safe,unsafe}`, safe file RUN).
> Under the old checker *both* halves had exactly **ONE** obligation — a division out in ordinary
> code — and **both failed it**, the contract having been thrown away. Now **four**, all proved in
> the safe file, and the mutant caught **twice**, once per hook. *Too weak outside, blind inside:
> one bug.* Everything else unmoved (solver.py byte-identical again; robust 45/0/0 with every
> verdict on 07's corpus where it was).
> **Known and left, the weak direction:** a call's postcondition reaches the logic **through a
> name** — `let w = weight(c) in total / w` proves, `total / weight(c)` does not (the div VC
> assumes `env.facts`, and a bare subexpression's synthesised refinement is not one). A capability
> rung, not a soundness one.
>
> **>> V2.2‴ — THE ADVERSARY OK (2026-07-14). Three more false proofs, and none of them mine.**
> The four above were found **by hand**, by going looking. `08/tests/adversary.py` is what happened
> when we stopped looking by hand: random **programs**, checked, then **RUN** if the checker proved
> them — *if it says `ok`, it must not crash.* (`fuzz` tests the **solver**, and the solver has
> never been the problem: every bug this project has had is one level up, in the checker that
> *builds* the obligations.) `make -C 08 adversary`; `harden` runs it. **It found three in an
> afternoon.** After the three fixes: **10 seeds × 250 programs = 2 500 clean** (seeds 3, 11, 23,
> 101, 999, 4242, 7, 55, 1234, 31337 — 2026-07-14). Absence of evidence, but from a generator that
> had teeth an hour earlier.
> **(1) `18_divisor` — a GOAL it could not read was never asked.** `100 / (x * y)` is merely
> **non-linear**, `term_opt` said `None`, and the site raised **no obligation at all**; same for
> `100 / h(x)` (a local lambda, unspeakable since V2.2′(c)). **This is V2.2′(b)'s rule at the
> OPPOSITE POLARITY, which is exactly why it was easy to get wrong:** *a **hypothesis** it cannot
> read must be **DROPPED**; a **goal** it cannot read must be **ASKED ANYWAY**, and go unproved.*
> **Silence about what you may ASSUME is modesty; silence about what you must PROVE is a lie.** The
> asking condition is now **HM's** (this is an integer division, so it owes a proof), and the only
> excuse is positively knowing the division is a **Float's**. An unnameable divisor gets a
> **skolem inheriting its own contract**, so `100 / nz(x * y)` still proves.
> **(2) `19_floatlit` — the road into a Float nobody has to declare.** A Float **literal** had no
> sort → read as `OTHER` → an ordinary opaque value → **may be equated**. `nan == nan` believed,
> its negation UNSAT, and the else branch *NaN actually takes* became dead code in the logic. One
> missing sort, two bugs opposite ways: a falsehood **believed**, and an obligation **invented**
> (`0.0 / 0.0` looked like an *integer* division). *Unspeakability is a property of every value a
> Float can reach — the sort must survive arithmetic.*
> **(3) `20_condition` — THE WORST HOLE THIS FORK HAS HAD: the question that was never asked.**
> **`formula_opt`/`term_opt` TRANSLATE; `synth` WALKS; only the walk raises obligations** — and
> four sites translated a sub-expression *instead of* walking it: an **`if`'s condition**, a
> **comparison**, a **`not`**, a **unary minus**. Every division inside any of them **vanished**.
> `if (x / y) < x then …` → **`ok: 0 obligation(s) proved`, exit 0**, then divides by zero. *Zero.*
> The unary minus is the sharpest: it walked its operand only when `term_opt` **failed** — the
> better it understood the code, the less it checked. **Rule (the impl hole's, one level down):
> EVERY SUB-EXPRESSION IS WALKED EXACTLY ONCE, whether or not it is also translated.** And **it was
> never a corner case: 07's own primes sieve** (`fn divides(d, n) = n / d * d == n`, a body that
> *is* a comparison) **reported ZERO obligations for the entire life of this fork.** Its verdict has
> moved in `robust_sweep.py` — to `unproved`, honestly (`d : Int` does not promise `d /= 0`, and 07
> is frozen) — **and the move is the finding.**
> **Also closed: THE LANGUAGE MAY NOT GROW BEHIND THE CHECKER'S BACK.** `synth` ended in a silent
> `return ROpaque()`, so an operator it had never heard of fell through it — no obligation, no
> error, `ok`, exit 0: the impl hole's shape exactly. **`%` is the one waiting to happen** (the CEK
> already implements it and it traps on 0; only the **lexer** keeps it out of the language). An
> unknown operator now **raises** and says what it owes — so `%` can wait for a front-end rung
> instead of being rushed in.
> **The cost was NOTHING.** Every `_safe` file proves, runs, prints its answer: the "capability"
> these holes bought was the absence of a question. `prove` **37 → 43**; drift/conservative/solver/
> fuzz unmoved; robust 45/0/0 with **exactly one verdict moved, on purpose**.
> **THE RULE THE RUNG LEAVES BEHIND: a checker's silence is a verdict, and a report must never let
> silence read as approval.** `ok: 0 obligation(s) proved` was in that output the whole time, and
> it reads like success.
>
> **>> V2.3 — THE AXIOM THAT BECAME A THEOREM OK (2026-07-14). `NONNEG_UF` IS DELETED.**
> V1's own comment conceded that it "knows something it strictly should not have to": a set literal
> asserting `string_length(t) >= 0`, and with it, that lengths are never negative. A measure now
> **DECLARES** its result refinement — `measure len(xs : List(Int)) : {v : Int | v >= 0}` — and the
> checker **PROVES it by structural induction: one VC per constructor arm, with the declared
> refinement assumed at each recursive occurrence as the induction hypothesis.** Finite VC set,
> existing solver, `solver.py` byte-identical again; the whole rung is inside `refine.py`. What the
> induction establishes is then asserted at the applications a program mentions (`result_axioms`,
> the same shape as V2.2's `measure_axioms`) — which is why `total / (len(xs) + 1)` proves for an
> **opaque** `xs`, one never taken apart. **That is the first universal claim this checker has been
> able to earn**, and it is earned rather than believed.
> **THE ONE LINE THAT IS DOING THE WORK: an induction VC may not assume what it is proving.**
> `Refiner.run()` hands each VC the results table it may assert from, and for `ind=True` VCs that
> table is `prim_results` — **without** the measures' own declared refinements. Hand the `Nil` arm
> the axiom it is trying to prove and it discharges itself, a bogus declaration passes, and the
> checker has proved a division by zero. **Assuming the conclusion is not a subtle bug; it is the
> entire difference between an induction and a lie.**
> **DONE-WHEN, MET (`21_result_unsafe.lark`).** Move one character — `{v : Int | v > 0}` for `len` —
> and it is **rejected**, 1 of 5 unproved, at exactly the right place: the **`Nil` arm**, whose goal
> `len(Nil) > 0` sits beside its own equation `len(Nil) == 0`; the `Cons` arm still proves from the
> IH. (*A false claim about a recursive definition is usually false at the base case.*) A
> detonation, not a style complaint: admit it and `total / len(xs)` is discharged, `ok`, exit 0 —
> then `main` passes `Nil`.
> **WHAT IS STILL BELIEVED IS NOW NAMED: `PRIMITIVE_AXIOMS`** — one line, one entry,
> `string_length(s) >= 0`, for the one function whose body the checker cannot see (String is
> primitive; it has no arms to induct on). Still withheld from any name the program rebinds
> (V2.2′(d) kept). **The complete list of what this checker takes on faith is readable in one place.**
> **THE STATEMENT PROVED MUST BE THE STATEMENT ASSERTED** — caught by audit, before any test. For a
> measure with **extra parameters** (`drop(xs, i)`) the induction goal was built as `drop(Nil())`, a
> term no program mentions, while the axiom was asserted at `drop(xs, i)`, the terms programs do:
> **sound by luck.** The extras now enter as free variables, and such a measure goes honestly
> *unproved* until V2.4 lets its equations fire.
> **AND THE ADVERSARY HAD TO BE RE-AIMED.** Disarmed (`_prove_measure` stubbed out) it came back
> *clean* — because it generated `x / size(xs)`, a **bare call**, whose postcondition never becomes
> a fact (V2.2″: a fact travels through a NAME), so it was unprovable whatever the declaration said.
> Generating `let m = size(xs) in x / m` instead, the disarmed checker produced a false proof on
> seed 11 in seconds. **A fuzzer that cannot express the bug does not report its absence — it
> reports its own blind spot**, and the only defence is to disarm the fix and make the machine find
> it. `prove` **43 → 45**; everything else unmoved (drift 22/3/3/0, conservative 90/0/0, solver
> 16/0, robust 45/0/0 with every verdict where it was, fuzz 3000/0, adversary clean on 3 seeds).
>
> **AUDITED AND CLEAN** (the hunt's negative results, worth not repeating): mutually recursive
> measures — the structural check already constrains *every* measure call in an arm, not just
> self-calls, so `m1(c) = 2 + m1(c)` is rejected as malformed; the two remaining `Top()` sites
> (literal `true`, a fresh binder's trivial refinement) are *no constraint*, the weak direction;
> `_walk_match`'s `seen` only holds formulas that were really translated; `$div` asserts nothing
> and skolems inherit only their own declared type.
>
> **TWO GAPS LEFT OPEN, DELIBERATELY.** (1) **The logic is ℤ; the RV32/C backends are 32-bit** — a
> refinement proved over unbounded integers need not survive wraparound (cf. the `19_intoverflow`
> optimizer xfail). Sound against the CEK reference (Python bignums), *not* against a 32-bit
> target. (2) **Contracts are partial correctness** — a non-terminating function may be given any
> postcondition, because checking a recursive body assumes the declared result type. Nothing bad
> happens (you never reach the division), but "proved" means "if it terminates". (3) Since V2.3, a
> **measure's termination** is still structural *by syntax* (V2.1's check), not proved by a
> well-founded order — and that check is what entitles the induction to exist at all.
>
> **>> H — HARDENING THE CODE THAT IS OK (2026-07-14). NOT A RUNG: a pass over what exists.**
> Every previous hunt asked whether the checker is **RIGHT**. This one asked whether it **ANSWERS**,
> because the two failures a verifier may not have are *a wrong answer* and *no answer*, and only the
> first had a suite pointing at it. New harness `08/tests/torture.py` (`make -C 08 torture`, wired
> into `harden`): 14 hostile-but-legal programs — 3 000 terms in one expression, a 300-way
> conjunction, a constructor term 400 deep, 1 200 nested lets, 40 disequalities, 30 disjunctions —
> each in a subprocess on a wall clock. **A traceback is not a verdict, and neither is a hang.**
> It began at 5 failures of 15 and ends at **0 of 14** (a probe was found to be testing its own typo).
> **(1) FOUR RAW `RecursionError`s.** The checker walks a program with the C stack and Python's limit
> is 1 000, so a program deeper than that **crashed** — out of `parser.py`, `infer.py`, `refine.py`.
> Fixed by running the walk on a thread with a real stack (`_with_big_stack`, 512 MB / 200 000) and
> **classifying what is left as `TooDeep`** — *a stated limit, not a stack trace.*
> **(2) EVERY FENCE IN THE SOLVER WAS LOCAL, AND EVERY CALLER LOOPS.** V1′ fenced the *splits* — but
> `MAX_SPLITS` is re-armed on each `_lia`, `_propagate` calls `_lia` quadratically, and DPLL calls
> `consistent` per node, so **the total work in one `satisfiable()` had no bound at all.** A budget
> that resets is not a budget. Fixed with a **global work purse** (`MAX_WORK`, one `Exhausted` for the
> whole query, which the callers may **not** swallow) — and, the sharper half, by bounding the **SIZE**
> of a linear system (`MAX_LINEAR`) and not merely the number of them: **Fourier–Motzkin is quadratic
> per elimination, so counting calls bounds nothing.** `wide_conj` hung on ONE `omega()` call with 300
> rows. Both fences were set against **measurement, not taste**: peak honest work across the whole
> prove suite *and* 07's corpus is **131 units** (fence: 2 000 000 — 15 000×) and the largest honest
> linear system is **8 rows** (fence: 256 — 32×). Nothing real can reach them, and `prove` 45/0 +
> `conservative` 90/0/0 + `fuzz` 0-unsound say so.
> **(3) "I STOPPED LOOKING" IS NOT "THIS DOES NOT FOLLOW".** The provenance of a give-up now survives
> to the report: `solver.decide` returns *(proved, gave-up)*, `Result.gave_up` carries them, and the
> CLI says **"gave up (budget exhausted)"** where it used to say "cannot prove", with `(N abandoned on
> a budget)` on the tail line. *The first is a fact about the program; the second is a fact about the
> checker, and printing the second in the words of the first is how a tool teaches its user to
> distrust it.* (V2.2‴'s rule, one step further: silence is a verdict — so say **whose** silence.)
> **(4) A CRASH WAS HIDING A COST.** With the stack raised, `deep_expr` stopped crashing and started
> **hanging** — the overflow had been mercy-killing a **cubic** algorithm. Translating `1 + 1 + … + 1`
> re-derived the linearity of the **whole subtree at every level** of a walk that already visits every
> node: **173 million `is_linear` calls on an 800-term sum.** Two fixes, and the second is the one worth
> keeping. (a) `pred.linear_node` — subterms of a term `_term_opt` returns have **already passed**, so
> only the new node is news; the full `is_linear` survives as the **specification**, now checked once per
> formula at `pred.formula_linear`, **at the solver's door, where the fragment's boundary actually is.**
> (b) `Env.memo` — `synth` walks each sub-expression once (V2.2‴'s rule) but asked each node for the term
> of *its whole subtree*, which is n² translations; the memo is keyed by **AST identity within one Env**,
> because an Env is exactly the scope in which a node's translation is a pure function of the node — *and
> a new binder makes a new Env, which is what keeps a shadowed name from being answered out of the cache
> of the scope that did not have it.* **57s → 0.014s** on the checker's share.
> **FIXING THE CRASH WITHOUT MEASURING THE COST WOULD HAVE MOVED THE FAILURE SOMEWHERE THE TESTS
> CANNOT SEE IT.** That is the rule this pass leaves behind.
> **(4b) THE THIRD WALK, AND THE ONE NOBODY WRITES: `hash`.** Asked "did you find *all* the unnecessary
> walking in the trees?", the profiler said no. A `Term` is a **frozen dataclass**, so `hash(t)` hashes the
> tuple of its fields — and a field of a Term **is a Term**. *A term is a tree, and its hash is a walk of
> that tree.* Fine once; ruinous inside **congruence closure**, which looks a term up in a dict for every
> `find`, every merge, every subterm it registers: an O(n) hash inside an O(n) loop is **the quadratic
> nobody wrote down, because nobody wrote the loop.** 600-term sum → **4.34 M calls to `hash`, ~40% of the
> run.** Fix: `pred._node_hash` — terms are immutable and heavily *shared*, so a node's hash is computed
> once and kept (`object.__setattr__`); each node then hashes in O(arity) because its children answer from
> their own cache. **4.34 M → 0; 3.8s → 1.65s.** (Assigned in each class body, not inherited from a mixin:
> `@dataclass(frozen=True)` **overwrites an inherited `__hash__` and respects an explicit one** — a mixin
> would have silently done nothing, which is the sort of quiet defeat this whole pass is about.) Same
> sweep: `Env.sorts()` **copied both dicts on every call** — an Env is immutable, so the table is now built
> once and is **read-only**, and the one caller that wrote into it (`$v` in subtyping) asks `sorts_v` for a
> copy of its own. **The rule: an immutable structure may be walked once; every walk after that is a bug
> you have not noticed yet.** After all three, the checker's own share of a 1 600-term file is **0.04s of
> 19.9s** — the rest is 43% frozen-oracle HM and 41% Omega doing real work on a 1 600-term linear system.
> **(5) `ResultAxiom` — THE STATEMENT PROVED IS THE STATEMENT ASSERTED, BY CONSTRUCTION.** Found by
> poking at V2.3 rather than by a test: `result_axioms` substituted only the *value binder*, leaving a
> measure's own **parameter** free in both the induction goal and the asserted axiom — a name the checker
> does not own, i.e. **V2.2′(c)/(d)'s shape exactly**, sound today only by the accident that free variables
> in a validity query are universally quantified. Now one dataclass owns the instantiation (`ResultAxiom.at`),
> the induction **proves `at(m(C(y…)))`** and every obligation **assumes `at(m(t))`** — the same door, so
> the two cannot drift apart by carelessness.
> **WHAT IS LEFT, MEASURED AND LEFT ALONE: the residual quadratic is the ORACLE's.** With the checker's own
> cost gone, `deep_expr` is still O(n²) — in `infer.py`'s `_apply_env`, which rebuilds the whole type
> environment at every node (textbook naive HM). **07 pays it too: 7.9s to typecheck the same file, before
> any refinement work happens.** It is *bounded, terminating* work — not a hang, not a hole — and the fork's
> `infer.py` diff is **9 lines** (the one case that erases `{v:Int|p}`). Fixing it means threading the
> substitution lazily through inference: a real change to the type-checker, made to the one file whose job
> is to be indistinguishable from the frozen oracle, in exchange for speeding up an input no program writes.
> **Not unfrozen, not patched, MEASURED AND WRITTEN DOWN** — which is a better place for a known cost than a
> quiet edit to the file we promised not to move. (If it is ever fixed, it goes in **08's** copy, never 07's,
> and `conservative` 90/0/0 is the harness that would prove it changed nothing.)
> **Everything green after the pass: drift 22/3/3/0 · conservative 90/0/0 · solver 16/0 · prove 45/0 ·
> robust 45/0/0 · fuzz 3000/0 unsound · adversary clean · torture 14/14.** `solver.py` is no longer
> byte-identical to its V2.2 self — that claim was about the *rungs*, which added no theory, and this pass
> is not a rung; the fences it adds are checked by the suites that pin behaviour.
>
> **> ACTIVE: PROVE V2 — measures. THE LADDER IS IN `PROVE.md` §4 (V2.0–V2.6) — read it there,
> not here. V2.0 OK, V2.1 OK, V2.2 OK, V2.2′ OK, V2.2″ OK, V2.2‴ OK, V2.3 OK, plus the two passes that
> are not rungs — H (hardening) OK and I (invariants) OK — ALL COMPLETE 2026-07-14. Suites at rest:
> prove **47/0**, invariants clean.
> NEXT = V2.4 — Bool-valued and parametrised measures (`sorted`, `bst`).** Lift structural facts
> into the logic (`len`, `sorted`, `bst`) so the solver stops being blind to what a function
> *computes*, then re-verify Lark's own samples (`01_mergesort`, `02_bst`). `make -C 08 test` is
> the regression contract.
> **BST invariant: SET CHOSE min/max MEASURES (2026-07-14)** — no quantifier ever enters the
> logic; ghost parameters stay unbuilt unless V2.5's permutation forces the issue.
>
> > **> HINT FOR V2.4, WRITTEN BEFORE STARTING IT (2026-07-14, end of the I session). Two halves —
> > one about the rung, one about how to build it. THE SECOND HALF IS THE IMPORTANT ONE.**
> >
> > **(a) THE RUNG. V2.4's real difficulty is not `sorted` — it is the SORT.** A Bool-valued measure is
> > **not a term, it is an ATOM.** Today a measure equation reaches the solver as
> > `Cmp("==", App(f, args), rhs)` — and `sorted(Cons(x, xs))` **cannot take that road**, because its
> > right-hand side is a *formula* (`x <= head(xs) && sorted(xs)`), not a term. So V2.2's machinery
> > (instantiate the defining equation at the constructor terms the program *mentions*, quantifier-free)
> > needs a **formula-level `Iff`** — or the two implications — while congruence closure must go on
> > identifying `sorted(xs)` with `sorted(ys)` when `xs == ys`, and a Bool UF must **never** be able to
> > stand in an Int position. **That is exactly where a V2.2′-shaped bug would live: a sort that hands
> > out more than it should.** Re-read how `FLOAT` was made *unspeakable* and ask the MIRROR question for
> > `BOOL`: not "may a term be built at this sort" but **"may this term appear where an Int is expected."**
> > And V2.3's induction has to follow it up: a Bool measure proving its own result refinement by
> > induction means the IH arrives as a **hypothesis in the formula**, not as an equation between terms.
> >
> > **(b) THE METHOD — AND THIS IS THE PART TO CARRY. BUILD THE HARNESS *WITH* THE RUNG, NOT AFTER IT.**
> > The lambda bug (I) and the fuzzer's missing lambda grammar (I) were **the same mistake twice: the
> > checks knew the words and not the grammar.** So *before* `refine.py` learns to emit a Bool-measure
> > equation, teach `invariants.py` and `adversary.py` to **speak** one: a fourth invariant — *every
> > Bool-measure application that reaches the solver reaches it as an ATOM, never as a term* — and an
> > adversary that can actually generate `sorted`/`bst` programs and then RUN them. **If V2.4 lands and
> > the adversary cannot emit the thing V2.4 added, we have learned nothing from the I session.**
>
> **> V2.2 — THE EQUATIONS REACH THE SOLVER OK (2026-07-14). `prove` 25 → 27 ok / 0 fail, and the
> rung's central claim held LITERALLY: `solver.py` is byte-for-byte unchanged (solver 16/0), and
> so are `pred.py`, `parser.py`, `lexer.py`, `tree.py`, `infer.py`. THE WHOLE RUNG IS INSIDE
> `refine.py`.** Everything else output-neutral (drift 22/3/3, conservative 90/0/0, robust
> 45/0/0, fuzz 3000/0 unsound). `07_affine_borrow.lark` **1 of 3 → 3 of 3** (the thermometer read
> what it had to read); `10_measure_len.lark` **unproved → proved**, with no change to the file;
> new pair `12_measure_safe` (4 proved, *and runs*) / `12_measure_unsafe` (3 of 4 unproved — one
> lie per hook, including `head(Nil)`, which under V1 was not an unproved obligation but an
> **unprovable** one).
>
> **> THE THREE V2.2 FINDINGS.**
> **(a) A CONSTRUCTOR NEEDED NO THEORY. IT NEEDED A NAME.** `Cons(x, xs)` becomes the
> uninterpreted application `Cons(x, xs)` — and the solver already decides equality and
> congruence at *any* sort, so this costs nothing. Congruence gives exactly what a constructor
> deserves (**equal fields build equal values**) and no more; injectivity and disjointness are
> **not asserted and not needed** — an axiom nobody needs is an axiom nobody has checked. Then a
> measure's arms are instantiated **at the constructor terms the obligation mentions and nowhere
> else** — no schema, no trigger, no e-matching, and above all **no quantifier** (`forall xs.
> len(Cons(x,xs)) == 1 + len(xs)` would leave QF-UFLIA and take the decision procedure with it).
> **ONE mechanism (`measure_axioms`, a walk over the obligation's own terms), fed from two
> places** — and note the destructing hook does *not* assume `len(xs) == 1 + len(rest)`; it says
> the humbler **what the scrutinee is** (`xs == Cons(x, rest)`) and the equation follows because
> the term is now in the hypotheses. It **closes in a single pass**, and the reason is the good
> part: the subterms of a constructor term are constructor terms, and an arm may only recurse on
> a *field* — **so V2.1's structural check, written to keep the axioms consistent, is also what
> makes them terminate.** The negative half of a constructor pattern is left on the table on
> purpose (it would need disjointness): **V1's rule one level down — the mention is free, the
> guard is not.**
> **(b) THE TIME BOMB WAS DETONATED, NOT ASSERTED.** Disarm V2.1's structural check and nothing
> else, and V2.2 instantiates `bad(xs) == 1 + bad(xs)` at the `Cons` arm's own term and
...
> poison reaches exactly the terms it is instantiated at. **The structural check is LOAD-BEARING
> for V2.2**, and that is now a fact on the record rather than an argument in a comment. With the
> check in place the file is still rejected at *declaration* time, before a single VC is built —
> which is what PROVE asked this rung to confirm.
> **(c) A LATENT BUG OF V1's, FOUND BECAUSE V2.2 NEEDED THE THING IT BROKE.** `_bind_pattern`
> built a **new `ty.Fresh()` per call**, whose counter restarts at 0 and collides with the type
> variables already inside the schemes; `instantiate` mapped a scheme's `α₀` to a "fresh" `α₀`,
> `ty.apply` chased `α₀ ↦ α₀` to a `RecursionError` — and a bare `except Exception` swallowed it
> and **bound nothing**. So a constructor pattern over a *polymorphic* ADT gave its binders no
> types at all, silently, in V1, V2.0 **and** V2.1. A *monomorphic* one (`| Buf(n) =>`) was always
> fine, because `instantiate` returns early when there is nothing to quantify — which is precisely
> why the `Buf`-shaped suite never noticed. One seeded supply per `Refiner` now; the `except` is
> narrowed to `FRONTEND_ERRORS`. **V1′'s rule again and one turn worse: a crash is never a
> verdict — this one was a crash reported as a *proof attempt*.** Its sibling: an ADT-typed binder
> was `ROpaque`, so `| Cons(_, r) => r` could not selfify `r` and the fact never left the
> function. So the sentence completes: **refined products let a fact TRAVEL; a measure CREATES
> one; a NAME is what carries it out through a return.**
>
> **> V2.1 — MEASURE DECLARATIONS OK (2026-07-14). `prove` 17 → 25 ok / 0 fail; everything else
> output-neutral (drift 22/3/3 — `lexer.py` never moved — conservative 90/0/0, solver 16/0
> byte-for-byte, `harden` robust 45/0/0 + fuzz 3000/0 unsound).** `measure len(xs : List(Int)) :
> Int = match xs with | Nil => 0 | Cons(_, rest) => 1 + len(rest) end` parses and is checked
> well-formed (`refine.Refiner._elab_measure`): one arm per constructor, recursion only on the
> matched constructor's fields, arm bodies translated STRICTLY, result sort Int or Bool, and the
> name owned by nothing else in the logic. Seven negatives (`prove/11_measure_*.lark`) hold each
> line, all failing as *malformed*, never as *unproved*.
>
> **> THE TWO V2.1 FINDINGS — both about where a thing is KEPT, not what it does.**
> **(a) Erasure is by CONSTRUCTION, so there is no erasure.** A `MeasureDecl` lives in
> `Program.measures` and **never in `Program.decls`** — the two halves of a program split at the
> parser door and never rejoin — so `infer.py` and the CEK machine cannot see a measure. There is
> no ghost-elimination pass because there is nothing to eliminate, and the corollary is free and
> is the better half: **calling a measure from real code is an unbound-name error from HM**, a
> rule nobody had to write. `prove/10_measure_len.lark` is checked *and then run* to say so.
> **(b) The declaration is what lets the name into a contract AT ALL — and that is NOT what the
> plan thought it was buying.** Under V1, `len(v) > 0` in a contract was not an unproved
> obligation but a *refinement error*: `len` had no declaration, hence no sort, and `_int_term`'s
> guard threw it out of the ordering — rightly, an unsorted symbol has no business in an
> arithmetic comparison. **The only names a V1 predicate could apply were the program's own
> functions**, which is a strange rule once said out loud, and saying it is what the declaration
> removes. V2.1's win is smaller than "measures work" and larger than "it parses".
> Also: **`measure` is a CONTEXTUAL keyword** — a decl could never begin with a bare NAME, so the
> only token stream newly accepted was a syntax error before, and the frozen lexer never learns a
> word only the type layer cares about.
>
> **> THE FIXTURE WITH TEETH IS `prove/11_measure_nonstructural.lark`, and it is a TIME BOMB
> ARMED FOR V2.2.** A measure's arms become AXIOMS. `bad(xs)` where only `bad(rest)` is legal
> asserts `bad(xs) == 1 + bad(xs)` — inconsistent — and the file's `absurd` returns `0` under the
> contract `{v : Int | v < 0}`. Once V2.2 instantiates equations at the terms an arm mentions,
> a checker that admitted that measure would discharge `0 < 0` and report a false program as
> verified. **Termination is optional for a contract and mandatory for a measure**, and that file
> is where the claim is tested. **OK V2.2 DETONATED IT (2026-07-14) — see finding (b) above. The
> bomb was live: with the check disarmed the checker verifies a false program. With the check in
> place the file is rejected at *declaration* time, before a single VC is built.**
>
> **> V2.0 — REFINED PRODUCTS OK (2026-07-14). `prove` 15 → 17 ok / 0 fail; everything else
> output-neutral (drift 22/3/3, conservative 90/0/0, solver 16/0 byte-for-byte, `harden` robust
> 45/0/0 + fuzz 3000/0 unsound).** All of it in `08/src/refine.py`; **the parser needed no change**
> (`{` already began an atom type, so `({v:Int|…}, {v:Buf|…})` parsed on the first try). `RTuple`
> (positional — a component may mention the *argument* it came from, never a sibling, because the
> surface gives siblings no names); **refinements on non-Int/Bool bases**, which the borrow idiom
> requires and V1 rejected — cheaper than it sounds, since an OTHER-sorted binder can only be used
> in equality and UF application, and **congruence closure does not care what a `Buf` is**; and
> `_bind_pattern` now takes the scrutinee's **already-synthesised** `RType` — the fix was not to
> infer harder but to **stop inferring**.
>
> **> THE V2.0 FINDING — AND IT CORRECTS THE LADDER'S OWN DoD, WHICH I HAD WRITTEN WRONG.** The
> plan said `07_affine_borrow.lark` would flip to *proved*. It went **half** way — **1 of 3** —
> and the half it did not go is the finding. The **consumer** is discharged: the contract's facts
> (`n == size(b)`, `size(b2) == size(b)`) now survive the tuple pattern, the guard adds `n > 0`,
> and `take(b2, 0)`'s bound follows (checked non-vacuous: weaken the guard to `n >= 0` and it
> fails). The **producer** cannot be, ever, by any amount of product refinement: `size_of`'s body
> needs `size(Buf(n)) == n`, and the two goals left standing are **the two halves of one missing
> measure equation**. So:
> **REFINED PRODUCTS LET A FACT *TRAVEL*; ONLY A MEASURE CAN *CREATE* ONE.** That is why V2.0
> could not have been the last rung, and it was found by building it rather than by reasoning
> about it. The file is now the thermometer for V2.2: it reads 1/3 and must read 3/3. **OK It
> reads 3/3 (V2.2, 2026-07-14), and the diff was one word: `fn size` became `measure size`.**
>
> Three more things the ladder settled, worth carrying in the head:
> - **Induction on programs is ALREADY THERE, for free** — checked, not assumed: `self.globals`
>   holds every fn's contract and `_base_env` seeds from it, so inside `_check_fn` a fn's own name
>   is in scope *with its contract*. A recursive call is checked against it and may assume it.
>   **V2 is that same move applied to measures.**
> - **V2.3 is the rung with the finding in it: `NONNEG_UF` becomes a THEOREM.** A measure's own
>   result refinement is checked **by structural induction — one VC per constructor arm, the IH
>   assumed at each recursive occurrence** — a *finite* VC set the existing solver already
>   discharges. The teeth-test is that a bogus `{v:Int | v > 0}` for `len` is *rejected*.
> - **Termination is OPTIONAL for a contract and MANDATORY for a measure.** A non-terminating fn
>   just never returns (partial correctness — V1 already leans on this); a non-terminating
>   *measure* makes its equations an inconsistent axiom set and the checker proves anything.
>
> **> NEXT: V2.3 — `NONNEG_UF` BECOMES A THEOREM.** A measure's own *result refinement*
> (`measure len(xs : List(a)) : {v : Int | v >= 0}`) checked **by structural induction: one VC per
> constructor arm, the IH assumed at each recursive occurrence** — a finite VC set the existing
> solver already discharges. This is the rung that turns the last hand-written axiom in the
> checker into something the checker proves, and its teeth-test is that a *bogus* declaration
> (`{v : Int | v > 0}` for `len`) is **rejected**. Note where the wall now is: V2.2 can prove
> things about lists the program **built**, because it instantiates at terms; a claim about
> **every** list is universal, and the only way to earn one is induction.
>
> ⛔ **The one rule of this axis: NEVER EDIT `07/src`.** It is the frozen oracle every
> differential in both trees compares against, and three fixpoints (`49a4921c`, `829410dc`,
> `f1dedfa9`) plus every number in both `BASELINES.md` are pinned to its bytes. PROVE §0.1
> exists because an earlier draft of the plan got this wrong. `08/` relates to `07/` exactly
> as `self/` does: **a differential, not an edit.**
>
> **> The book is the OTHER axis, and it is the larger debt — parked, not cancelled.**
> Parts I–III, `book/chapters/`. **ch01, ch02, ch03, ch04 and ch11 are written**; ~90 stubs
> left in 24 files. Part III (ch13–ch16) *cannot* be written until PROVE exists, which is why
> PROVE goes first; ch05–ch10 and ch12 can be written at any time and are waiting.
> **`ch16_refinement.tex` is now a REPORT, not a design** — its opening stub said "planned, not
> done" and that is stale (PROVE V1 + V1′ shipped). Its `\section{What it catches}` stub carries
> the finding it should be built on: the checker's **true positive in Lark's own source** —
> `| Div(a, b) => eval(a) / eval(b)` in `07/samples/05_expr.lark` *and* `09_parser.lark`, the
> parser we compiled, optimized, self-hosted and **ran on a microcontroller in ch11**. Well-typed,
> never failed, latent for the whole book; the last chapter's tool finds it in the first chapter's
> code, on its first run, unasked. **The bug stays** (`07/src` is frozen, PROVE §0.1) — it is worth
> more as a finding than as a fix. Two rules
> are mechanical, not remembered: prose cites the **reader's** paths only (`make -C book paths`,
> runs with the build; stub blocks exempt), and **never hand-edit `repo/`** — it is generated
> (`make -C self repo`, checked by `make -C self drift`). `book/PATHS.md` is the orientation.
> The companion ships as `ext/` (book + the three strands as **siblings**); the prose is already
> immune to that move.
>
> **> CURRENT POSITION (code): M7.5 OK COMPLETE — O5′ optimizing self-application fixpoint CLOSED
> (2026-07-11). M7 DONE. The self-hosted OPTIMIZING compiler compiles its own ~7.5 k-line
> source to C1==C2==C3 byte-identical (sha `f1dedfa9`, baked -O3). Win: emitted C −18.4 %.
> opttest 128/0/52 (output-neutral). SELFHOST M0–M7 all complete; OPTIMIZE O0–O5+O5′ all complete.**
>
> **> THE BOOK REPO (`repo/`) IS GREEN AND VERIFIED (2026-07-12).** First full run of every
> target in all three strands: **zero failures** — 10 differentials, 3 fixpoints, the proofs.
> `optcompilertest` went 115/25/40 → **140/0/40** (25 harness bugs, none in the compiler);
> `make fixpoint` now actually closes (`f1dedfa9`) — it had been calling a ladder that never
> had; `bootstrap-tc` re-pinned `45c1982a → 829410dc` (stale, not broken). Everything in
> `repo/` is now pinned in `BASELINES.md` **and** verified by running it. See the newest log
> entry. Commits on branch `worktree-book-repo-stubs`, unmerged.
>
> **> SETTLED (2026-07-12): `repo/` IS GENERATED. There are not two repositories.** Set chose
> *generate + drift check*. `tools/mkrepo.py` now produces all **196 files** of `repo/` from the
> dev tree — modules and oracle and corpus copied, the **13 harnesses copied with a mechanical
> path rewrite** (`07/src/lexer.py` upstream must read `oracle/lexer.py` for a reader who has no
> `07/`). `make -C self repo` writes it; `make -C self drift` fails if anyone hand-edits it.
> **`repo/` is now an artifact, like a PDF is an artifact of the LaTeX — never edit it.**
> Consequence for what ships: the dev tree is *supposed* to hold what the book must not (fuzz,
> optbench, RISC-V, firmware, old plans). The **manifest in `mkrepo.py` selects**; anything not
> named there never reaches a reader. That is the answer to "the old repo has a lot of stuff not
> suitable for the book" — it doesn't need cleaning, it needs projecting.
>
> **THE FINDING: `repo/`'s harnesses were better than ours, and only by accident.** Fixes had
> landed in whichever copy was being run and never came back. All **13** had diverged; the two
> `BASELINES.md` **disagreed about `cektest`** (dev 33 ok, `repo/` 34 — `repo/` was right, since
> the `25_torture` harness fix). Everything real in `repo/` is now backported upstream: the
> `LARK_TIMEOUT` env budget and **timeout→skip** in all 13 (dev `lex_difftest` referenced an
> undefined `PORT_TIMEOUT` and had no `TimeoutExpired` handler — it would have *crashed* on a slow
> box). The drift checker caught the stale pin on its first run.
>
> **> SETTLED (2026-07-12): `repo/` IS AN ILLUSTRATION, NOT A CODEBASE — and the three strands
> stand apart.** Set: *"it is suppose to illustrate things like optimisation, not be the code base
> for developing optimisation for Lark further… Lark is only a pedagogical tool."* Two consequences,
> and they are the opposite of what a working repo would want:
> - **Duplication across strands is the design, not a debt.** `self/`, `optimize/` and `prove/` each
>   carry their *own* `oracle/`, `lark/`, `harness/`, `tests/`, `samples/`, `Makefile`, `README.md`.
>   The front end appears in two trees because a reader of Part II must be able to run Part II
>   without ever opening Part I's directory. **Nothing is a shared library. Do not factor it out** —
>   there is no future development to amortise it over.
> - **No file in a strand may name another strand.** Enforced by hand today: `optimize/` used to
>   send the reader to `../self/harness/BASELINES.md` (its `Makefile` *and* its `README`) because it
>   had no baselines of its own — it has them now (`optimize/harness/BASELINES.md`, its four
>   differentials + the O5′ fixpoint `f1dedfa9` + the RV32I ruler), and `mkrepo.py`'s pin check
>   cross-checks **every strand's** numbers against the dev pins. `prove/README` no longer cites
>   `self/lark/`. Grep after any change: `grep -rn '\.\./self\|self/lark' repo/optimize repo/prove`
>   must be empty.
>
> The *generator* survives all of this — but it is for **our** side of the wall, not the reader's:
> it exists so the dev tree and the book tree cannot silently disagree (they already had). The
> reader never sees it.
>
> **> CURRENT POSITION (2026-07-11): SELFHOST M0–M7 OK + OPTIMIZE O0–O5+O5′ OK — that list is exhausted
> (the last stale ⬜, M5.5.4, was a bookkeeping error; it closed in OPTIMIZE §8b on 2026-07-08 and is
> now marked OK).**
>
> **> ACTIVE AXIS = M6 / F3 — cross-compiling to bare metal (RE-SCOPED 2026-07-11, Set chose it).**
> Ladder **F3.1–F3.4 written into SELFHOST.md §4 (M6)**.
>
> **> F3.1 OK COMPLETE (2026-07-13) — AND F3.2 WITH IT. THE RISC-V BACKEND IS SELF-HOSTED.**
> `self/regalloc.lark` (~360 L, linear scan) + `self/asm.lark` (~600 L, TAC→RV32I) + the **9th and
> 10th differentials**: `make -C self regalloctest` = **32/0/13** and `make -C self asmtest` =
> **32/0/13** — byte-identical `.S` over the corpus, same skip shape as `emittactest` (3 imports +
> 7 reject fixtures + 3 CEK-overflow). Both pinned in `self/tests/BASELINES.md`; `make test` runs
> them; `repo/` regenerated (`make -C self repo`) — the canonicalisation below had to reach
> `repo/optimize/oracle/regalloc.py`, and `make drift` caught that it hadn't.
> **`regalloc.lark` reuses `opt.lark`'s CFG + liveness rather than re-porting them** — that is why
> the real port surface was ~950 lines, not the 822 the plan estimated plus a CFG.
>
> **> THE FINDING, AND IT IS BIGGER THAN ANY PRE-REGISTERED HAZARD: `asm.py` WAS NOT A FUNCTION.**
> `regalloc.py::_compute_intervals` iterated **frozensets** (`live_in`/`live_out`/`defs`/`uses`), so
> the interval table's insertion order — the **tiebreak of the stable sort** in `_linear_scan`, hence
> which `Tmp` won which register — was a function of **`PYTHONHASHSEED`**. **The compiler emitted
> different assembly on every run**: 10 of the 45 corpus files varied (different byte lengths), and
> `09_parser` gave **5 distinct shas in 5 runs**. There was no byte-identity for F3.1's differential
> to be a claim *about*, and it had been true for as long as `asm.py` had existed — invisible because
> **nothing downstream ever compared two runs of it**. Fixed by canonicalising all four iterations to
> `sorted()` (the only edit outside `self/`). Verified: `make -C 07 difftest` **34 passed** (CEK ≡
> TAC-C ≡ RV32 emulator — the emulator *executes* the re-allocated code), deterministic across 4
> seeds + 3 unseeded runs, and — since **any edit to `07/src` re-opens the fixpoints** — all three
> re-confirmed: `bootstrap` `49a4921c` OK, `bootstrap-tc` `829410dc` OK, O5′ ladder `f1dedfa9` OK.
> (`self/` never imports `regalloc`/`asm`, so the edit is provably off the compiler path; re-ran anyway.)
>
> **> THE NATIVE RV32 CROSS-COMPILER EXISTS: `self/tests/rv32c.py`.** The ten Lark modules
> (`lex→parse→types→tast→infer→tac→lower→opt→regalloc→asm`, **8,003 lines**) assembled into a stdin
> compiler and built through the O5′ ladder's stage0 path (`emit_tac_c.py -O0` → self-contained C →
> `cc`). `python3 self/tests/rv32c.py --check` = **9 ok / 0 fail**: its `.S` equals `asm.py`'s on
> **every sample**, including the three `asmtest` can only skip (`05_expr`, `09_parser` — 3,180 lines
> of RV32I; the skips are CPython's stack, not the port). **Firmware built from a Python compiler
> tests Python** — this is what makes the `.uf2` a claim about the *self-hosted* compiler.
>
> **> F3.3 CAPSTONE — RE-SCOPED, AND THE OLD WORDING WAS UNATTAINABLE.** SELFHOST asked for the 9
> committed `07/firmware/*.uf2` to be **reproduced byte-identically** from the Lark compiler. They
> cannot be — **not by the Lark compiler and not by `asm.py` either**: they were built before the
> canonicalisation above, from **one unrecoverable hash seed**, and **no `.S` was ever checked in**.
> So the capstone is now: *rebuild* the nine from the canonicalised pipeline and prove the two
> compilers agree on every byte, which is **strictly stronger** than matching an artefact.
> `self/tests/firmware_difftest.py` does it — **all three machine-checkable claims are GREEN**:
> **(1)** assembly identity, self-hosted vs `asm.py`, **9/9**;
> **(1½)** the Lark compiler's assembly **executed on the RV32 emulator prints what `cek.py` prints,
> 9/9** — `make -C 07 difftest` sweeps `07/tests` only, so **not one of the nine flashable programs
> had ever been run as RV32**;
> **(2)** the real GNU RISC-V toolchain (pico-sdk, found under `~/.pico-sdk` — cmake/ninja/toolchain/
> picotool are all there, just not on `PATH`) turns that assembly into **9 `.uf2`, byte-identical
> from both pipelines**. Staged in `build/firmware/` with `expected/<name>.txt` (the CEK transcripts).
> **(3) = SILICON. OK 9/9 ON A REAL PICO 2/2W (2026-07-13). F3.3 IS CLOSED — ALL FOUR CLAIMS GREEN.**
> `python3 self/tests/firmware_difftest.py --board` flashes each image with `picotool`, captures what
> the board prints over USB-CDC and **diffs it against `expected/`**: **9 ok / 0 fail**, `09_parser`
> included — Lark's own parser, on a microcontroller, compiled by a compiler written in Lark. No BOOTSEL
> press needed at all (`picotool -f` resets the board between images). The register allocator, the frame
> layout and the tail-call loop survive contact with a real Hazard3 core. **`07/firmware/` re-pinned to
> the nine hardware-verified images** — the exact bytes that were flashed (shas below, unchanged).
> **§7 finding — the harness bug that looked like a hardware bug: `tty.setraw()` defaults to `TCSAFLUSH`
> (= discard pending input).** Opening the port asserts DTR → the firmware's wait loop releases → it
> prints *microseconds later* → our very next line flushed the transcript away. We then listened at a
> board that had already spoken and halted. `screen` never flushes on open, so it always worked, which
> made it look like DTR/macOS/silicon. **Raw mode by hand, `TCSANOW`, never `TCSAFLUSH`.** The move that
> cracked it: flash the **old, hardware-verified** image and find it **equally silent** — a known-good
> artefact cannot be a compiler bug, so the fault had to be ours.
> **`--install` replaces `07/firmware/*.uf2`** — do that only after claim 3 passes.
> Rebuilt image shas (sha256, first 12): 01_mergesort `9bb1dc6b1897` · 02_bst `a32ae55267be` ·
> 03_primes `43c482e98ca0` · 04_queens `5f5fda9487cc` · 05_expr `22520abb5c98` · 06_rle `250455d6210e` ·
> 07_hanoi `5354233ce529` · 08_life `9f93c28bb7a9` · 09_parser `c80e9260d60a`.
>
> **> NEXT: F3.4** (optional — `coloring.py` + `igraph.py`, the graph-coloring allocator behind `gen`'s
> `allocator=` hook), then **PROVE.md**. Set commits/tags ([[feedback_commits]]).
>
> **Pre-registered §7 hazards for F3.1** (expect these, they have bitten every port):
> • **occurs-gap redux** — the frozen `infer`'s *monomorphic* self-recursion trips any polymorphic
>   accumulator-reverse (`ecRev`/`etcRev` both hit it). Pin the element type on such a helper.
> • **affine-across-arms** — cannot `print(io, …)` in two match arms; build a `String` purely, print once.
> • **the arena wall (§8a/§8b)** — build the `.S` text with a **balanced join** from the start; a
>   left-nested `acc + line` is O(n²) and is what blew 12 GB in M7.5.
> • **byte-identity is formatting-identity** — immediate formatting, label naming, section directives,
>   whitespace. Mirror `asm.py` character-for-character, as `tacReprStr` had to mirror Python `repr`.
> • **F3 is FAR closer than the old plan said:** M6's port list was written *before* M7, and **M7 already
>   did most of it** — `tac.py`→`self/tac.lark` (M7.1), `lower.py`→`self/lower.lark` (M7.2), and the
>   "backend analyses" (cfg/liveness/dominators) already live inside `self/opt.lark` (M7.4).
> • **REAL port surface = 2 modules, ~822 lines: `asm.py` (547, TAC→RV32 assembly text, entry
>   `gen(tac, allocator=None)`) + `regalloc.py` (275, linear-scan `allocate_tac`).** Smaller than
>   `lower.lark` was.
> • **DO NOT PORT `riscv_asm.py` (617) / `riscv_vm.py` (726).** They are the assembler + RV32 emulator =
>   **test infrastructure, oracle not product** (same status as `cek.py`). **The Pico path never touches
>   them**: `make pico FILE=…` runs `asm.py FILE → runtime/program.S`, then the **real GNU RISC-V
>   toolchain** (pico-sdk + cmake/ninja) assembles/links the `.uf2`. The emulator exists only to run RV32
>   in software for testing (`assemble_lark(gen(tac))` → bytes → VM).
> • `coloring.py` (367) + `igraph.py` (162) = the O4 **graph-coloring** allocator, swapped in at `gen`'s
>   `allocator=` hook → **F3.4, optional increment**, the one genuinely new piece (purely-threaded
>   interference graph + spilling). cfg/liveness for it are already ported.
> • **The seam is clean — F3 is a BACKEND SWAP.** `lex→parse→infer→lower→opt` is done and green; F3
>   replaces one terminal stage, `emit_tac_c` (TAC→C) → `asm` (TAC→RV32), on the same TAC input.
> • **Testing needs NO hardware** until the capstone: F3.1 = byte-identical `.S` vs `asm.py` (the 9th
>   differential, same recipe as the other 8); F3.2 = behavioral via the Python emulator vs CEK.
> • **F3.3 capstone is fixpoint-grade:** `07/firmware/` already holds **9 prebuilt `.uf2`s** built by the
>   *Python* pipeline and verified on a real Pico 2/2W. Reproducing them **byte-identically from the
>   Lark-written compiler** is the F3 analogue of F2⁺.
> • **WARNING SAY IT PLAINLY (incl. in the book): F3 is CROSS-COMPILATION, not on-device self-hosting.** The
>   compiler runs on the host. A self-compile needs ~5.6 GB arena; an RP2350 has **520 KB** — a ~10 000×
>   gap no allocator fix closes. The milestone's name invites the wrong expectation.
>
> **> NEXT AXIS AFTER F3: PROVE.md** — the separate *guarantees* axis (refinement types FIRST =
> Algorithm W + SMT, then MLTT), prototyped in the Python oracle 07/src, NOT self-hosted. **F3 and PROVE
> are orthogonal** (F3 = backend, TAC in; PROVE = front-end/Algorithm W) → doing F3 first costs PROVE
> **no rework**. Hand-off: Set tags a git commit for the M7.5 close (I don't commit/tag —
> [[feedback_commits]]).**
>
> **> WHERE PROVE WORK GOES: the dev tree. NEVER `repo/` (it is generated — `make repo` destroys
> hand edits).** It reaches `repo/prove/` by being added to `mkrepo.py`'s manifest, when it is
> book-ready and not before. **THE HAZARD, flag it loudly:** PROVE V1 adds `{v:b|p}` *syntax*, so it
> touches `07/src/lexer.py` + `parser.py` — **the frozen oracle the entire differential method rests
> on.** It survives *only* if the extension is additive and **no existing corpus file uses the new
> syntax** (the differentials compare oracle-vs-port on the corpus; if neither side emits the new
> token there, both still agree and every baseline holds). New syntax ⇒ **new suite**, never edited
> fixtures. **Rule: any edit to `07/src` re-opens the fixpoints — re-run `bootstrap` /
> `bootstrap-tc` / `fixpoint` and confirm `49a4921c` / `829410dc` / `f1dedfa9`. If a sha moves you
> changed emitted code, and you must say so deliberately.** (Those three targets now actually run —
> before 2026-07-12 `make fixpoint` called a ladder that had never closed.) One naming collision to
> settle first: PROVE.md §6 wants a top-level dev `prove/`, but `repo/prove/` already means
> lcore + the Lark formalisation (it is generated from `formal/proof/`).
>
> **> DEAD END, DO NOT RE-DERIVE: hand-vendoring `07/` into `self/`.** Tried 2026-07-11, **reverted the
> same day.** `tools/mkrepo.py` already solves it, and better — it *generates* `repo/{self,optimize,prove}/`,
> each strand self-contained (`lark/` + `oracle/` + path-rewritten `harness/` + `tests/` + `samples/`;
> "nothing in it points outside itself"). **`self/` never needs to leave `07/`:** move the whole lark tree
> and `07/` moves with it; publish `self/` standalone and that *is* `repo/self/`. A hand-maintained copy
> guarded by a drift-check is strictly weaker than a generator — and mkrepo exists **because** the
> hand-made copy drifted across thirteen harnesses. Likewise `book/PATHS.md` (reverted): `book/tools/
> checkpaths.py` already enforces it via `make -C book paths`.
> **The one finding worth keeping** (the question was "is the oracle small enough to vendor?" — answered
> **yes, decisively**): the harnesses' import closure is **14 of `07/src`'s 26 modules, 7,348 lines /
> ~267 KB, pure stdlib, zero third-party deps** (lexer parser tree ty typed_tree infer cek emit_c_ast tac
> lower emit_tac_c opt cfg liveness); the RV32/regalloc/asm/repl half (~3,431 L) is never imported by
> `self/`. Corpus = 36 `.lark` + 9 samples. **Do NOT turn this into machinery** — trimming mkrepo per-strand
> by closure buys nothing (copying ten small files is free) and adds a real failure mode (miss one import →
> the reader's repo is broken).**
>
> **SESSION 2026-07-11 (late) — HOUSEKEEPING, no milestone. Both plans confirmed DONE; a vendoring
> detour was built and then REVERTED (see the DEAD END note in CURRENT POSITION above — `tools/mkrepo.py`
> already solved it, and a generator beats a hand-maintained copy + drift-check). What survives from the
> detour is the sizing answer, kept below; `self/vendor/`, `self/VENDORING.md` and `book/PATHS.md` are
> gone. Also deleted: the stale, git-tracked `.claude/memory/` (dead since June 7, read by nothing, index
> had a dangling link; its one real note — ch08 RLHF/Arrow — is preserved in the live memory store). The
> repo's durable memory is THIS LOG.**
> • **Question answered: is `self/` complete in itself?** *Self-hosting yes, self-contained no.* `self/`
>   depends on the sibling `07/` three ways: (1) the CEK **bootstrap seed** — `Makefile` hardcodes
>   `CEK := ../07/src/cek.py`, and every `.lark` needs it to run at stage 0; (2) every `*_difftest.py`
>   does `SRC = ROOT/"07"/"src"` + `sys.path.insert` and imports the **Python oracle**; (3) the
>   **corpus** lives in `07/tests` + `07/samples`.
> • **Oracle sized for vendoring: comfortably small.** Traced the harness import closure = **14 of
>   07/src's 26 modules, 7,348 lines / ~267 KB** (lexer parser tree ty typed_tree infer cek
>   emit_c_ast tac lower emit_tac_c opt cfg liveness). The RV32/regalloc/asm/repl half (~3,431 L) is
>   **never imported** by `self/`. **Pure stdlib — zero third-party deps**, so vendoring is a file copy.
>   Corpus = 36 `.lark` (1,136 L) + 9 samples (787 L). Total staging ≈ 9,300 L / <300 KB — smaller than
>   the `.lark` compiler it tests.
> • **BUILT `self/vendor/` (vendor.sh + check_drift.sh + a staged copy) and `self/VENDORING.md` and
>   `book/PATHS.md` — then REVERTED ALL THREE the same day.** They duplicated `tools/mkrepo.py` and
>   `book/tools/checkpaths.py`. **The mistake was not looking for prior art**: mkrepo already *generates*
>   a self-contained `repo/self/` (oracle + path-rewritten harnesses + corpus), and its own docstring says
>   it exists **because** the earlier hand-made copy drifted across thirteen harnesses. I hand-rolled a
>   copy + drift-check — i.e. exactly the weaker mechanism mkrepo was written to replace, and a *third*
>   copy of `07/src` in the tree. Deleted; see the DEAD END note above.
> • **WARNING The lesson, which generalises past this detour:** when a copy must be kept in sync, **derive it,
>   don't guard it.** A generator has one source of truth; a copy + drift-check has two and a promise. If
>   a future session is tempted to vendor, mirror, or snapshot anything in this tree — check `tools/` for
>   a generator first.
> • **Stale checkbox FIXED:** `SELFHOST.md` M5.5.4 still read ⬜ though `OPTIMIZE.md` §8b records it
>   **CLOSED (2026-07-08)**. Now OK with the cross-ref. **With that, no ⬜ remains on either plan.**
> • **Nothing committed** ([[feedback_commits]]). Set still owes the M7.5 tag (`git tag selfhost-f2`
>   equivalent for the O5′ close) **plus** these vendoring files if he wants them tracked.
> • **NEXT:** self-hosting + optimization are **both exhausted** — see CURRENT POSITION. The live
>   choices are **PROVE.md** (the guarantees axis, refinement types first, prototyped in the Python
>   oracle) · **wiring the vendoring** (only when `self/` must actually leave `07/`) · **F3 Pico/RISC-V**
>   (stretch, uncommitted) · the **pedagogical net-only chapters**.
>
> **M7.5 CLOSE (2026-07-11):**
> • **Claim 2 (self-application fixpoint) CLOSED.** Ladder (`self/tests/ladder_opt.sh`):
>   stage0 = `emit_tac_c.py optc_O3.lark -O0` → clang → stage1; stage1 self-compiles → C1 →
>   stage2; stage2 → C2; stage3 → C3. **C1==C2==C3 byte-identical, sha `f1dedfa9`, 1,512,018 bytes**,
>   on the baked-`-O3` optimizing compiler. Self-contained backend (`emit_tac_c`, `cc -O2 -fwrapv
>   -DLARK_HEAP_BYTES=12G`), each stage `ulimit -s 65520` (macOS 8 MB stack vs deep self-recursion).
> • **THE WIN (same input optc_O3.lark):** emitted C **1,852,562 (-O0) → 1,512,018 (-O3) = −18.4 %**;
>   binary 398,600 → 408,360 (noise); wall ~1 s each. Headline = −18.4 % generated code.
> • **The arena wall was NOT the passes (nor emit_tac_c interning as the old NEXT guessed).** Level-≥1
>   self-compile blew 12 GB in 2 s *before any pass ran*: `self/opt.lark`'s `optSweeps` fingerprinted
>   the whole ~1.8 MB program via `tacPretty(t)` (O(n²) left-nested string join = the §8a/§8b arena
>   wall) twice per sweep, first line first. **Fix: structural `optTacEq`** mirroring opt.py's
>   `_fingerprint` (compare `(f.name, f.body)` per fn; O(n)/O(1)-alloc). 12 GB overflow → **5.6 GB
>   free**. Output-neutral: `opttest` stayed **128/0/52** byte-identical vs `opt.py`.
> • **Two smaller output-neutral cleanups (same pass):** structure-preserving `optCpKill`/`optCseKill`
>   (add 0-alloc `AnyKill` guard → rebuild the block-local map only on a real kill, not every instr);
>   `optLicmFn(f)=f` (drop the discarded O(B²) dominator computation — licm is a global no-op on
>   Lark's DAG-only CFGs, opt.py's licm acts only on back-edges which functional lowering never emits).
>   The now-dead dominator helpers (`optDominators`/`optDomInit`/`optHasBackEdge`/`optDomOf`) are left
>   in place (never called → zero alloc; removing them would move the fixpoint sha and force a re-run).
> • Earlier this session (pre-compaction): `dce`/liveness rewritten to String-keyed RB-tree `StrSet`
>   (`optStrLt`/`ss*`), and `optDomOf` inlined to fix an `unbound optLiveInOf` from that rewrite.
> • BASELINES.md pins sha `f1dedfa9` as invariant #5; SELFHOST.md M7.5→OK / M7 COMPLETE; OPTIMIZE.md
>   §9.4 has the RESULT block; this LOG updated.
>

> **M7.5 SESSION 2026-07-11:**
> • **`inferProgram` missing link DONE** — `self/infer.lark` now exposes `inferProgram(prog:Prog) :
>   Result(TProgram, String)`, the parse→typed-AST bridge the optimizing driver needs.
> • **Claim 1 (behavioral fixpoint) — the self-hosted OPTIMIZING pipeline
>   parse→inferProgram→lowerProgram→optimizeProg(L)→emitTacC reproduces the Python oracle's
>   optimized C byte-for-byte at every level.** (harness `self/tests/optcompiler_difftest.py`; note
>   the `want + "\n"` print-vs-emit trailing-newline quirk.)
> • **REAL native-backend BUG found via lldb (user said "use c debugger?") and FIXED** — the TAC→C
>   backend miscompiled `match` on **string-literal patterns**: `keyword_kind` in `self/lex.lark`
>   misclassified every keyword as `KName`, so the self-compiled parser never terminated (infinite
>   recursion → SIGSEGV in `lk_pDecls`). Root cause: `emit_tac_c` lowered a string-Const `==` as a
>   *tag test* unconditionally. Fix (BOTH oracle `07/src/emit_tac_c.py` AND port
>   `self/emit_tac_c.lark`): disambiguate — if LHS tmp is an `IGetTag` dst → integer tag test, else
>   `__lark_streq`. Threaded the per-fn IGetTag-dst set (`_tag_tmps` / `Ctx`'s new 4th field
>   `etcCtxTagTmps`, filled by `etcTagTmps`). Reproducer `strmatch.lark`: CEK=120, native was 000 →
>   now 120. **Corpus-neutral & parity-preserved:** tacctest 100/0/0 · emittactest 32/0/13 ·
>   lowertest 35/0/10. **A genuine §7 correctness finding the fixpoint surfaced.**
> • **`read_all` added to the TAC backend** (was CEK-only) — 4 symmetric sites: `07/src/lower.py`
>   `_BUILTINS`, `07/src/emit_tac_c.py` `_PREAMBLE` (stdin-slurp C fn), `self/lower.lark`
>   `lwBuiltins`, `self/emit_tac_c.lark` preamble. Baseline-neutral.
> • **Native self-hosted optimizing seed (`stageA`) WORKS** — compiles hello / traits / 08_life
>   byte-identical to the oracle (modulo the known trailing-newline quirk).
> • **Claim 2 diagnosis (the capstone, self-compile of the ~7363-line compiler → ~48416 C lines):**
>   NOT a correctness bug, and NOT plain "no-GC retention." Measured with an instrumented peak-heap
>   build: **peak heap = O(output²)** — ratio heap ÷ (emitted-C bytes)² is a constant ≈ **0.025**
>   across a 180× span (08_life 0.018 GB @ 27 665 B out; fe.lark 3.31 GB @ 356 591 B out). The live
>   set is linear; the blow-up is quadratic *work* → the **§8 arena-join wall, recurring in the new
>   TAC backend**. The top-level join is already the balanced §8a `etcJoinNL`, so this is a NEW
>   residual: (a) `etcIntern`/`etcIndexOf` = linear scan + `tacSnocStr` copy-whole-list append per C
>   string literal → O(K²)/O(M·K) (the frozen `emit_tac_c.py` hides it behind a `dict` — EXACT §8b
>   pattern), and (b) `etcCStrGo` char-by-char `acc + c` → O(len²) per literal. **Fix is
>   output-neutral (tree/hash side-index keyed by String, insertion-order index preserved; chunked
>   literal build) and needs NO memory manager** — extrapolated to drop the ~50–100 GB self-compile
>   peak under 1 GB, closing Claim 2 GC-free. Left UNfrozen pending that fix (do not pin M7.5 in
>   BASELINES yet). Kept artifacts in `$CLAUDE_JOB_DIR/tmp/fix2/`.
> • **On "should Lark get a GC?" (user asked):** no — see the design note appended to `OPTIMIZE.md`
>   §9.5 neighbour discussion. The §8b-class interning fix closes Claim 2 without touching memory
>   management; beyond that the Lark-native, PROVE-orthogonal path is affine-driven deterministic
>   reclamation (phase/region arena reset, then Perceus-style reuse), NOT tracing GC.
>
> **> (superseded) M7.4 was DONE (2026-07-11) — `self/opt.lark` landed.**
> `self/opt.lark` (~1770 lines) = the `07/src/opt.py` port — the TAC→TAC optimization passes only
> (the `PASSES` list: devirt, inline [+ prune], closure_elim, const_fold, copy_prop,
> algebraic_simplify, cse, dce, licm), NOT the RV32 `ASM_PASSES` (peephole/immfold/branchlayout —
> asm-text, irrelevant to a C backend). opt.py leans on mutation + a module-global site counter +
> cfg/liveness helpers; all threaded PURELY: **each pass is `(TAC, Int) -> (TAC, Int)`** where the
> Int is the shared inline/closure_elim **site counter** (opt.py's `_SITE`); `optimizeProg` starts
> it at 0 and threads it across every pass AND every sweep (fuel = `_MAX_SWEEPS` = 8). cfg/liveness
> are ported inline (`optBuildCfg`/`optLiveness`/`optDefs`/`optUses`) — only `dce` needs the full
> CFG + backward-dataflow liveness; `copy_prop`/`cse` need only block boundaries (detected inline,
> no CFG); `licm` needs dominators (no-op on the acyclic corpus). Differential
> `tests/opt_difftest.py` (`make -C self opttest`): for every corpus file **AND every level
> -O0..-O3**, the oracle lowers (lower.py) then `opt.optimize` at that level and pretty-prints; the
> port serialises the SAME unoptimized TAC and runs lex+parse+tac+opt +
> `tacPretty(optimizeProg(<TAC>, level))`, demanding byte-identical optimized TAC. **128 ok / 0
> fail / 52 skip** — every accepted single-file program matches at all four levels (incl. the
> live-fnmap inline case, 17_mutual_rec); 52 skips = (3 imports + 7 reject fixtures + 3 CEK-overflow
> on deep serialised TAC [24_stringprims, 05_expr, 09_parser]) × 4 levels. Two determinism points
> make it line up: the harness RESETS `opt._SITE` per oracle call (so both start a file+level at
> site 0), and RE-LOWERS a fresh TAC per level (opt.py mutates `fn.body` in place). All differentials
> green: infertest 42/0/3 · cektest 33/0/15 · emittest 37/0/7 · typechecktest 42/0/3 · lowertest
> 35/0/10 · emittactest 32/0/13 · **opttest 128/0/52**.
> **Continue with M7.5 per `OPTIMIZE.md §9.3`:** the F2-style fixpoint on the *optimizing* self-hosted
> compiler — assemble lex+parse+lower+tac+opt+emit_tac_c into a stdin compiler, put opt ON the path
> (optimize before emit), and check the C1==C2==C3 byte-identical fixpoint (as `bootstrap.py` does for
> the emit-only compiler). NOTE the emittactest oracle still emits from UNoptimized TAC — flip its
> `oracle()` to optimize-before-emit once M7.5's driver exists, so the emitter differential runs on
> the *optimized* IR too.
>
> *M7.4 findings (for M7.5 and as §7 differential lessons):*
> • **The live-mutable `fnmap` is load-bearing — a frozen snapshot is WRONG.** opt.py's `inline`
>   (and `closure_elim`) build `fnmap = {f.name: f for f in tac.functions}` holding LIVE references
>   and set `fn.body = new_body` in place. So a function that GROWS past `INLINE_MAX` (=12) from an
>   earlier inline becomes **ineligible** for call sites processed later in the same pass, and its
>   grown body is what later sites splice. 17_mutual_rec (-O2/-O3) is the witness: is_even inlines
>   is_odd → its body exceeds 12 → is_odd is no longer inlined elsewhere and gets pruned as
>   unreachable. My first port used a frozen snapshot and inlined everything (is_odd survived) →
>   TAC diverged. **Fix: thread an updated fnmap left-to-right** via index walks
>   (`optInlineWalk`/`optClosureElimWalk` + `optNthFn`/`optReplaceFn`), rebuilding each function on
>   the already-updated function list. Purely-functional restatement of opt.py's in-place mutation.
> • **No `%` operator in Lark** — const_fold's wrap-to-32-bit uses a floor-mod built from `/`
>   (truncates toward zero), `*`, `-` (`optFloorMod`/`optWrap32`). const_fold folds int +,-,* under
>   wrap32 + comparisons + bool &&/||, NEVER `/` or `%` (matches opt.py, which also leaves division
>   to runtime). No occurs-gap this port — the block/set helpers that recurse are already concrete.
> • **`fn` is a reserved binder** (redux) — every `ICall(_, fn, _)` / `IAllocClosure(_, fn, _)`
>   pattern binder had to be renamed `fnm`. And a `match`-in-`let` needs its `end` before the `in`
>   (`let x = match … end in`) — two silent parse errors from omitting it.
>
> **> (superseded) M7.3 pointer — kept for the findings below:**
> `self/emit_tac_c.lark` = the `07/src/emit_tac_c.py` port (TAC → C on the optimizing backend
> path). emit_tac_c.py is a mutable `CEmitter` class with a `strlits` interning table; the port
> threads that table PURELY as a `List(String)` (first-seen order = the C string-array index),
> read-only tables (tag→id, defined-fn names, globals) ride in a `Ctx` record (`impl Copy`).
> The giant C `_PREAMBLE` (bump heap + builtins, 7838 bytes) is carried verbatim as ONE Lark
> string constant `etcPreambleRaw` (byte-injected, not hand-transcribed). Differential
> `tests/emit_tac_c_difftest.py` (`make -C self emittactest`): the oracle typechecks (infer.py)
> + lowers (lower.py) **with NO optimization** (opt.lark is M7.4, not yet ported — so both sides
> emit from the identical lowered TAC) + emits C via CEmitter; the port serialises that TAC into
> tac.lark ctors and runs lex+parse+tac+emit_tac_c + `emitTacC(<TAC>)`, demanding byte-identical
> C. **32 ok / 0 fail / 13 skip** — every accepted single-file program matches (incl. 08_life 867
> lines, 06_rle 647, Stdlib 584, mergesort 576, bst 534; error-suite 04_nonexhaustive/06_matchfail
> correctly EMIT). 13 skips = 3 imports + 7 reject fixtures + 3 CEK-overflow on a deep serialised
> TAC (24_stringprims, samples/05_expr, samples/09_parser — a capacity verdict, same class as
> lowertest's overflow skip). All differentials green: infertest 42/0/2 · cektest 33/0/15 ·
> emittest 37/0/7 · typechecktest 42/0/2 · lowertest 35/0/10 · **emittactest 32/0/13**.
> *(M7.4 DONE 2026-07-11 — `self/opt.lark`, opttest 128/0/52; see the current NEXT block above.)*
>
> *M7.3 findings (for M7.4 and as §7 differential lessons):*
> • **Occurs-gap: the polymorphic accumulator-reverse bites again.** `etcRev(xs : List(a), acc :
>   List(a)) : List(a)` trips the frozen infer's monomorphic self-recursion (RecursionError in
>   ty.py `apply` — the same occurs gap that forced emit_c.lark's `ecRev`/`ecApp` to be String-
>   monomorphic). Fix: pin `fn etcRev(xs : List(String), acc : List(String)) : List(String)` (only
>   used on `List(String)` in `etcLocalsGo`). Note `etcLen(xs : List(a)) : Int` stayed polymorphic
>   and typechecks fine — only the reverse/append accumulator pattern (result-var cons'd back into
>   the accumulator) diverges, not every polymorphic recursion. Sole code fix during the port.
> • **Serialiser feeds UNOPTIMIZED TAC to both sides** so M7.3 isolates the emitter cleanly; when
>   M7.4's opt.lark lands and joins the path, re-run this differential with optimization ON both
>   sides (the harness `oracle()` currently calls `_lower.lower(tprog)` with no opt — flip that).
> • **The C `_PREAMBLE` byte-injection idiom** (Python one-off: read `_PREAMBLE`, `.replace("\\",
>   "\\\\").replace('"','\\"')`, substitute a `@@PREAMBLE@@` marker) reproduced emit()'s `parts[0]`
>   element exactly incl. its single trailing `\n`; reuse it verbatim if opt.lark ever needs a raw
>   C blob. Reminder: emit_tac_c's `_cstr` uses OCTAL (`\NNN`) escaping — DIFFERENT from emit_c's
>   hex — so `etcCStr`/`etcOct3` are distinct from emit_c.lark's `ecCStr`.
>
> ---
> **> (superseded) M7.2 pointer — kept for the findings below:**
> `self/lower.lark` = the `07/src/lower.py` port (typed AST → TAC). lower.py is a mutable
> `Lowerer` class; the port threads that state PURELY: read-only tables as an `LCtx` record
> (type_tags / trait_impls / global_lets / show_impls / global_arity), the mutable lowerer
> state as a threaded `LSt` record (globalFns / lambdaCtr / lifted-fns), and the per-Function
> instruction stream + counter as tac.lark's `Function`. Every lowering has shape
> `(…, ctx : LCtx, fnc : Function, st : LSt) -> (Operand, Function, LSt)` — `fnc`/`st` threaded
> SEPARATELY (the lambda case swaps the Function while keeping shared st). Both `LCtx`/`LSt`
> are `impl Copy`. Differential `tests/lower_difftest.py` (`make -C self lowertest`): the oracle
> typechecks (infer.py) + lowers (lower.py) + pretty-prints; the port concatenates
> lex+parse+types+tast+tac+lower and runs `tacPretty(lowerProgram(<serialised TProgram>))`,
> demanding byte-identical TAC. **35 ok / 0 fail / 10 skip** — every single-file corpus program
> matches incl. 09_parser (851-line self-hosted parser lowering ITSELF, 190+ TAC lines); 10 skips
> = 3 imports + 7 reject fixtures (infer raises, nothing to lower). All differentials green:
> infertest 42/0/2 · cektest 33/0/15 · emittest 37/0/7 · typechecktest 42/0/2 · **lowertest 35/0/10**.
> **Continue with M7.3–M7.5 per `OPTIMIZE.md §9.3`:** M7.3 `self/emit_tac_c.lark` (differential vs
> the M7.0 `emit_tac_c.py` oracle; `self/emit_c.lark` is the structural template) → M7.4
> `self/opt.lark` (TAC passes) → M7.5 the F2-style fixpoint on the *optimized* self-hosted compiler.
>
> *M7.2 findings (for M7.3 and as §7 differential lessons):*
> • **`fn` and `()` are reserved tokens** — a param named `fn` collides with the fn keyword, and
>   `fn f()` (zero-arg) is unparseable (`()` lexes as one UNIT token). Named the threaded Function
>   param `fnc`; `_BUILTINS` is a top-level `let` value, not a nullary fn.
> • **Monomorphic-recursion type blowup (the hard one).** Inside a self-recursive fn, the recursive
>   call uses a fresh rec-var (check_fn_decl overrides the name to `Scheme((), rec_var)` — the frozen
>   oracle does this unconditionally, so all self-calls are monomorphic). Consing TWO self-call
>   results into one list (`Cons(lv, Cons(rv, Nil))` in TBinOp's ICall args) unifies their element
>   types, tying the list-element type back through the fn's own result → an ever-growing type that
>   never terminates in `apply` (looks like RecursionError under a low limit, still diverges at 200k).
>   **Fix: route the pair through an annotated helper `lwPair(a : Operand, b : Operand) : List(Operand)`**
>   — the annotations pin each to concrete `Operand` at the call site so the two result-vars never
>   unify. Runtime value identical, so emitted TAC is byte-unchanged. Watch for this in any self-
>   recursive lowering that builds multi-element lists from its own results.
> • **tac.lark `tacReprStr` bug FIXED (M7.1 defect surfaced by M7.2).** Python `repr(str)` switches to
>   `"`-delimiters when the string contains `'` and no `"` (so single quotes need no escaping);
>   09_parser emits `expected ')'` → Python reprs it `"expected ')'"`. The M7.1 port always used
>   `'…'` and escaped. Now mirrors the quote-selection exactly (tacContainsChar + escSingle flag);
>   tacsmoke still green. Non-printable `\xNN`-escaping still deferred (no corpus program needs it).
> • **`pretty` → `tacPretty` rename** in tac.lark (permanent): the M7 concat also pulls in types.lark
>   whose Mono printer owns `pretty`. tacsmoke updated to match.
>
> *M7.1 notes for M7.2:* Const models tac.py's raw `int|float|bool|str|None` as `ConstVal`
> (`CInt`/`CFloat`/`CStr`/`CBool`/`CUnit`); ints/floats carried as SOURCE STRINGS (like parse's
> VInt/VFloat) — int pretty is passthrough, **float repr is the one deferred canonicalisation**
> (Python `repr(float)` ≠ raw lexeme; same class as the M5 float lesson — handle in the M7.2
> differential if a corpus float diverges). `CStr` covers BOTH string literals AND tag names
> (tac.py's `Const(str)` is ambiguous, resolved by context at `==` tag tests) — faithful, kept.
> `dst` fields are `Tmp`; ICall dst = `Maybe(Tmp)`, IReturn val = `Maybe(Operand)` (parse's Maybe).
> `tacEmit` appends (O(n) snoc) — lower.lark should reverse-accumulate to dodge the O(n²) wall (§9.5).
>
> *(Prior NEXT — M7.0 DONE 2026-07-10.)* `07/src/emit_tac_c.py` is a working, CEK-validated TAC→C
> emitter on the optimizing path (differential `tests/emit_tac_c_difftest.py` = 100 ok/0 fail across
> 25 corpus × -O0..-O3; only 19_intoverflow xfails at -O1+, an opt 32-bit-const-fold artifact). It
> emits a *self-contained* C file (`intptr_t` word, inline heap+builtins, no runtime.c link).
>
> *(Prior NEXT — O5 DONE 2026-07-10.)* O5 (self-optimizing
> fixpoint) closed as a *behavioral* fixpoint after fixing two real backend bugs (bare-fn-as-value
> eta-expansion in `lower.py`; branch relaxation in `riscv_asm.py`) — see the newest log entry.
> Optimizer shrinks the compiler's own RV32 image −58 %; the TAC interpreter emits byte-identical
> C across -O0..-O3 on the real compiler (the whole compiler on the RV32 *VM* is infeasible — no-GC
> arena wall, §8a/§8b). But behavioral O5 leaves two gaps (the optimizer
> is *Python*, not Lark; and it sits on the RV32 backend, disjoint from the F2/C fixpoint). Set
> asked what "true" O5 needs → **scoped as O5′ / SELFHOST M7, full breakdown in `OPTIMIZE.md §9`**
> (planned 2026-07-10, not started): port `tac`+`lower`+the TAC subset of `opt` to Lark + a net-new
> `emit_tac_c` = a self-hosted *optimizing C-compiler*; the compiler literally optimizes itself and
> reaches an F2-style fixpoint on the *optimized* compiler. ~1500 Lark lines (M3-scale). Remaining
> axes, Set to choose: **(a) O5′/M7** (true self-optimizing fixpoint, §9); **(b) `PROVE.md`** — the
> *guarantees* axis (refinement types first, in the 07/ Python oracle); **(c)** M5.5.4 native fixpoint
> on the typechecking compiler (arena-wall, `OPTIMIZE` §8); **(d)** pedagogical "net-only" chapters.
> **Tier-4 is now COMPLETE** (all four backend items landed): O4 increment 3
> (instruction selection + branch layout) is DONE — see the newest log entry: two post-gen
> asm passes in `src/opt.py`, `immfold` (li+ALU → immediate form: addi/andi/ori/xori/slti,
> sub→addi -C, mul-2^k→slli; folded `li` swept by the reused peephole dead-scratch pass)
> and `branchlayout` (invert `bnez L; j M; L:` → `beqz M; L:` for fall-through). `optbench
> --levels 0,1,2,3,4` = 25/25 observably ==-O0; corpus asm 4683→**4443** (immfold −123,
> branchlayout −117), bin 23900→**22940**, **dyn 14.033M→10.828M (−23%)**. Two axes still
> open: (a) **O5** — the self-optimizing fixpoint (run the optimizer on the self-hosted
> compiler itself). (b) **`PROVE.md`** — the separate *guarantees* axis (refinement types
> first, prototyped in the 07/ Python oracle, not self-hosted). Set to choose.
>
> *(Prior NEXT, DONE 2026-07-09 — O4 increment 2: graph-coloring regalloc in
> `07/src/coloring.py`. Chaitin-Briggs iterated register coalescing over `igraph.py`'s
> interference+copy graph; optimistic spill needs no program rewrite [asm.load/store
> already spill per-instr]. Coalescing collapses the cross-block `mv sX,sY` copies the
> peephole can't reach + packs into fewer callee-saved regs. Deterministic across
> `PYTHONHASHSEED` [min-element worklists]; `regalloc.verify` clean on all 25 files.
> Guard 25/25 ==-O0; asm 5199→4683, bin 25964→23900, dyn ~unchanged [static moves]. See
> newest log entry + OPTIMIZE §8g.)*
>
> *(Prior NEXT, DONE 2026-07-09 — O4 increment 1: RV32I peephole in `07/src/opt.py`
> (`peephole`, new `ASM_PASSES` registry + `postgen()`, `-O4` = O3 TAC passes + this
> post-gen asm pass). Window-local t-register (t0-t6) copy-prop / dead-scratch
> elimination / result coalescing on the emitted asm, + `mv r,r` and fall-through-`j`
> removal. Guard green 25/25 ==-O0; asm 6526→5199, bin 31272→25964, dyn 22.551M→14.036M;
> heap/stub untouched (backend pass). diff_test 34/0, licm green. See newest log entry +
> OPTIMIZE §5/§8f.)*
>
> *(Prior NEXT, DONE 2026-07-09 — O3 increment 2: `opt.optimize()` iterates the enabled
> pass sweep to a fixpoint so non-recursive escaping-into-HOF closures (compose/twice)
> collapse one layer per sweep. heap_allocs 358→**354**, asm 6736→**6526**, bin
> 32128→**31272**; 10_closures 146→86 asm / heap 5→3. Guard green 25/25 ==-O0;
> diff_test 34/0; licm green. Site counter made monotonic across sweeps to avoid
> `.i0_…` label collisions. See the newest log entry + OPTIMIZE §5/§8e.)*
>
> *(Prior NEXT, DONE 2026-07-09 — balanced-tree `Subst`: OPTIMIZE §8b residual CLOSED;
> `self/types.lark` `Subst` → Int-keyed Okasaki red-black tree, O(log n) lookup;
> output-neutral (infertest 42/0/3 + typechecktest 42/0/3 byte-identical); tc native
> fixpoint re-measured C1==C2==C3 sha `45c1982a`, peak 10.15 GB → **4.73 GB**.)*

**Current position:** SELFHOST **OK COMPLETE — FROZEN at F2⁺**. **OPTIMIZE ACTIVE;
the whole §8 arena-wall now CLOSED (§8a join + §8b env-side + subst-side residual =
balanced-tree `Subst`, tc peak **4.73 GB**, sha `45c1982a`); M5.5.4 CLOSED, O0 RULER
DONE (§8c); pre-O1 hygiene [1]–[5] DONE (§8d); O1 DONE (both increments), O2 DONE,
O3 DONE (both increments), **O4 DONE — Tier-4 COMPLETE (increment 1 RV32I peephole +
increment 2 graph-coloring regalloc + increment 3 instruction selection + branch
layout)**, **O5 DONE 2026-07-10 (behavioral self-optimizing fixpoint; fixed
bare-fn-as-value miscompile + branch relaxation; optimizer −58 % on the compiler's
own RV32 image; TAC interpreter emits byte-identical C across -O0..-O3 on the real
compiler — whole compiler on the RV32 VM is infeasible, no-GC arena wall — see
newest log entry).** OPTIMIZE ladder O0–O5 now COMPLETE (behavioral sense; true
self-optimizing fixpoint scoped as O5′/M7 in §9).
**O5′/M7 progress: M7.0 (`07/src/emit_tac_c.py` oracle) · M7.1 (`self/tac.lark` IR) ·
M7.2 (`self/lower.lark` = lower.py port) · M7.3 (`self/emit_tac_c.lark` = emit_tac_c.py
port) ALL DONE 2026-07-10; M7.4 (`self/opt.lark` = opt.py TAC-passes port) DONE 2026-07-11.
`make -C self lowertest` = 35/0/10, `make -C self emittactest` = 32/0/13, **`make -C self
opttest` = 128 ok / 0 fail / 52 skip** (optimized TAC byte-identical vs opt.py at every
level -O0..-O3; 52 skips = (3 imports + 7 reject fixtures + 3 CEK-overflow [24_stringprims,
05_expr, 09_parser]) × 4 levels). The whole TAC pipeline (lower → opt → emit_tac_c) is now
self-hosted and differentially green. NEXT = M7.5 fixpoint (the F2-style ladder on the
*optimizing* self-hosted compiler) — per `OPTIMIZE.md §9.3`.**
**M7.0 DONE 2026-07-10 (O5′ leg 1): `07/src/emit_tac_c.py` = a Python TAC→C emitter
oracle on the OPTIMIZING path (`parse→infer→lower→optimize→emit_tac_c→clang→run`).
Self-contained C (bump heap + all builtins inline, `intptr_t` word so ptrs AND
fn-ptrs fit — no runtime.c link, no 32-bit truncation). New differential
`tests/emit_tac_c_difftest.py` = **100 ok / 0 fail** (25 corpus × -O0..-O3) vs the
CEK oracle; 19_intoverflow xfail at -O1+ only (opt const-fold is 32-bit by RV32
design, diverges from CEK bignum — same class as diff_test's RV32 XFAIL; matches at
-O0). Two real bugs fixed in the emitter along the way: duplicate top-level names
(imported Stdlib `length` shadowed by local `length` → emit LAST-wins, matching
tac_vm's `{fn.name:fn}`) and unconstructed-ctor tag tests (`Green` never allocated →
`tag_ids.get(name,-1)` like asm, not a strcmp on an int → was a segfault). NO
existing pipeline file touched (only 2 new files) → frozen baselines intact. NEXT
O5′ leg = **M7.1 `self/tac.lark`** (IR types), then M7.2 `self/lower.lark`, M7.3
`self/emit_tac_c.lark` (differential vs THIS oracle), M7.4 `self/opt.lark`, M7.5 the
fixpoint — order per `OPTIMIZE.md §9.3`.**
**O4 increment 3 DONE (2026-07-10): instruction selection + branch layout — two more
post-gen asm passes in `07/src/opt.py`, appended to `ASM_PASSES` after `peephole`.
`immfold` = immediate-form instruction selection: forward-tracks (per straight-line
window, reusing the peephole's window machinery via a new `transform` param on
`_peep_local`) the integer each `li tX,C` puts in a scratch reg, then folds
`add/and/or/xor`→`addi/andi/ori/xori` (commutative, either operand), `sub C`→`addi -C`
(rhs only), `slt C`→`slti` (rhs only), `mul` by 2^k→`slli k`; the now-readerless `li` is
removed by re-running `_opt_window` at window end. Soundness: 12-bit signed imm range
[-2048,2047] checked on every fold; floats excluded twice (`# float`-commented `li` +
float arith lowers to `ICall`, never a reg-reg ALU op); only peephole-known mnemonics
emitted (no sltu/sltiu). `branchlayout` = invert `bnez tX,L; j M; L:` → `beqz tX,M; L:`
(delete the taken `j`, fall through to L on true) via a `_BRANCH_INVERT` table over the
full 1-/2-reg conditional set. Guard green `optbench --levels 0,1,2,3,4` = **25/25
observably ==-O0**; movers (O4 increment2→increment3): asm 4683→**4443** (immfold −123,
branchlayout −117), bin 23900→**22940** (−960), **dyn 14,032,752→10,828,342 (−23%, the
per-iteration `li` folded out of hot loops)**; heap/stub UNCHANGED (pure backend). Per
attribution the passes are toggled independently. Hardening: full chain
`branchlayout∘immfold∘peephole` is IDEMPOTENT on all 25 + no residual self-move;
DETERMINISTIC across `PYTHONHASHSEED` (per-reg dict queried not iterated; drop set is
membership-only) — O4 asm sha identical across seeds 0/12345/999. Isolation BY
CONSTRUCTION — `opt` imported only by `optbench`/`opt_licm_test` (grep-verified), `self/`
never references it, 4 self-host differentials unaffected. `diff_test` **34/0** (O0 path,
no postgen → byte-identical), `opt_licm_test` green, O0 identity so `optbench_O0.json` NOT
re-pinned; BASELINES.md O4 row + OPTIMIZE §4/§5/§8h updated. NEXT = O5 (self-optimizing
fixpoint) OR `PROVE.md`.**
**O4 increment 2 DONE (2026-07-09): graph-coloring register allocation —
`07/src/coloring.py` (~260 lines), Chaitin-Briggs iterated register coalescing (Appel
§11.4) over the interference+copy graph `igraph.py` already builds. K=11 (s1-s11).
Conservative (Briggs) coalescing merges move-related non-interfering temps so the
cross-block `mv sX, sY` the peephole can't reach becomes a self-move and vanishes;
optimistic spill needs NO program rewrite because `asm.load`/`asm.store` already
materialise a register-less temp from/to its stack slot per use/def (same as linear
scan). Drop-in: `color_allocate` returns the same `regalloc.Allocation`,
`allocate_tac_color` mirrors `allocate_tac`. GATING = a codegen-strategy flag
`regalloc_color` (new `opt.CODEGEN_FLAGS`, named in `LEVELS[4]`) dispatched INSIDE
`asm.gen(allocator=…)` — NOT a TAC pass (PASSES) or asm-text pass (ASM_PASSES); linear
scan stays the O0..O3/diff_test/CLI default, so those stay byte-identical.
`optbench.run_worker` queries `opt.wants_graph_coloring(opts)` and passes the coloring
allocator. Guard green `optbench --levels 0,1,2,3,4` = **25/25 observably ==-O0**;
attributed by the flag toggle: corpus asm **5199→4683** (−516), bin **25964→23900**
(−2064); dyn 14036245→14032752 (barely — coalescing removes STATIC setup moves; 04_tailrec's
13M-instr hot loop dominates the dynamic total); heap/stub UNCHANGED (pure backend tier);
coloring-OFF reproduces increment-1's 5199 exactly. Hardening: (a) DETERMINISTIC across
`PYTHONHASHSEED` (min-element worklist selection, never `set.pop()`) — O4 asm sha identical
across 3 seeds on 08_traits/25_torture/06_lists; (b) `regalloc.verify` clean on every fn of
all 25 (color_allocate also asserts internally); (c) isolation BY CONSTRUCTION — `coloring`
imported only by `optbench` (grep-verified), `self/` never references it, 4 self-host
differentials unaffected. `diff_test` **34/0**, `opt_licm_test` green, O0 identity so
`optbench_O0.json` NOT re-pinned; BASELINES.md O0→O4 table + OPTIMIZE §8g updated. NEXT =
Tier-4 remainder (instr-sel: fold address arith / `IMAC`; branch layout) OR `PROVE.md`.**
**O4 increment 1 DONE (2026-07-09): the first POST-GEN pass — `peephole` in
`07/src/opt.py` runs on the emitted RV32I assembly (str→str), not on TAC. New
`ASM_PASSES` registry + `postgen(asm, opts)` (mirrors `PASSES`/`optimize` but for asm
passes); `optbench.run_worker` now calls `postgen` after `gen`; `-O4` = LEVELS[4] =
the O3 TAC passes PLUS `peephole`, so O4's win over O3 is entirely the peephole. It
does window-local optimization on the caller-saved SCRATCH registers t0-t6 (which the
generator reloads every fragment and never keeps live across a label/branch/call/ret —
the load-bearing invariant, so live-out(t-regs)=∅ at every window boundary): (A)
copy-prop of `mv tX, R` into later reads, (B) delete a pure instr whose t-reg dst is
dead, (C) coalesce `<op> tX, …; mv D, tX` → `<op> D, …`; then two structural rewrites
over the whole listing — drop `mv rX, rX`, drop a `j L` that falls through to `L:`.
Windows with any non-generator mnemonic are skipped untouched (defensive). Guard green
`optbench --levels 0,1,2,3,4` = **25/25 ALL observably ==-O0**. Movers (O3→O4 totals):
asm 6526→**5199**, bin 31272→**25964**, **dyn 22.551347M→14.036245M (−38%, the biggest
executed-instruction cut of any tier — hot recursive loops shed per-iteration moves)**;
heap_allocs/heap_bytes/stub_calls UNCHANGED (a pure backend pass, doesn't touch alloc
or IO). Per file e.g. 10_closures asm 86→59, dyn 94→67. Hardening (same session):
peephole is IDEMPOTENT on all 25 (`peephole∘peephole == peephole`), leaves NO residual
`mv r,r`, and skips unknown-mnemonic windows — verified by a scratch script. Isolation:
only `07/src/opt.py` + `07/tests/optbench.py` touched; `diff_test` **34/0** (O0 path,
no peephole → byte-identical CEK==TAC==RV32 preserved), `opt_licm_test` green, O0 stays
identity (totals 8106/.../361 unchanged → `optbench_O0.json` NOT re-pinned); the 4
self-host differentials unaffected BY CONSTRUCTION (`opt` imported only by
optbench/opt_licm_test, grep-verified). BASELINES.md gains the O0→O4 progression table.
NEXT = O4 increment 2 (graph-coloring regalloc via `igraph.py`, instr-sel, branch
layout) or `PROVE.md`.**
**O3 increment 2 DONE (2026-07-09): `opt.optimize()` iterates the enabled pass sweep
to a fixpoint (`_fingerprint` equality, `_MAX_SWEEPS=8` cap), so non-recursive HOFs
(compose/twice) that a single sweep only half-collapsed now peel fully — one closure
layer per sweep. Site counter for inline/closure_elim made MONOTONIC across sweeps
(`_next_site()` off one `itertools.count`; a per-call `site=0` would re-mint `.i0_…`
labels on sweep 2 and collide). Guard green `optbench --levels 0,1,2,3` = 25/25
observably ==-O0. Movers vs increment 1: heap_allocs 358→354, heap_bytes 3732→3708,
asm 6736→6526, bin 32128→31272, stub_calls 524→520; 10_closures asm 146→86/heap 5→3,
25_torture 1848→1790/heap 159→157 (iterating also nudged O1 7821→7817, O2 6820→6726).
CEILING reached: remaining escaped closures flow into RECURSIVE HOFs (map/foldr in
06_lists/16_stdlib/11_tree) that `inline` correctly won't expand — needs
specialization/defunctionalization, not iteration. diff_test 34/0, licm unit green,
O0 stays identity so `optbench_O0.json` NOT re-pinned; the 4 self-host differentials
unaffected BY CONSTRUCTION (`opt` imported only by optbench/opt_licm_test). NEXT = O4
(Tier-4 backend) or `PROVE.md`.**
Older detail: **O3 increment 1 DONE (2026-07-09): `closure_elim`
in `07/src/opt.py`, `-O3` wired (PASSES: closure_elim right after inline, before the
Tier-1 cleanup). Guard green `optbench --levels 0,1,2,3` = 25/25 ALL observably ==-O0;
this is the first pass to MOVE the Tier-3 headline — heap_allocs 360(-O2)→358(-O3),
heap_bytes 3756→3732 (+ asm 6820→6736, bin 32472→32128, dyn −86, stubs 526→524).
closure_elim scalar-replaces NON-ESCAPING closures: a closure whose value is used
only as the fn of an IClosureCall (never passed/returned/stored/field-read) is inlined
at each call site with `IGetField(env,i)`→`IAssign(_,caps[i])` and the `IAllocClosure`
deleted (a real heap alloc gone). Whole-function + follows single-assignment copies, so
it fires across the `.i…_ret` block seam `inline` leaves; caps must be Const/param/
single-def (dominance via SSA value). Fired on the 2 corpus files with a non-escaping
applied closure: 10_closures asm 188→146 heap 6→5, 25_torture asm 1890→1848 heap 160→159.
Most corpus closures ESCAPE into HOFs (map/compose/twice) — exposing those needs deeper
inline iteration (deferred). Other three Tier-3 passes named in LEVELS[3] stay FILTERED
OUT (not in PASSES) = analyzed-and-deferred: `unbox` is a genuine no-op (scalars Int/
Float/Bool are immediates on this backend, never boxed — only ADT ctors IAlloc, verified);
`fusion` needs higher IR (map∘map structure is gone at TAC); `arena_reuse` needs affinity
threaded to TAC + a non-bump allocator. Four self-host differentials unaffected BY
CONSTRUCTION (`opt` imported only by optbench/opt_licm_test — grep-verified); O0 still
identity so `optbench_O0.json` NOT re-pinned; `diff_test` 34/0; licm unit test green.
NEXT = O3 increment 2 (iterate inline+closure_elim to a fixpoint to collapse escaping-
into-HOF closures) OR the OPTIMIZE §8 arena-join wall.** Older O2 detail below.
**O2 DONE (2026-07-09): `devirt` + `inline` (+ reachable-
function prune) + `licm` in `07/src/opt.py`, `-O2` wired. Guard green
`optbench --levels 0,1,2` = 25/25 ALL observably ==-O0; corpus asm 7821(-O1)→6820(-O2),
bin 36512→32472, dyn 22.5526M→22.5518M, heap unchanged (Tier-3's number). devirt
recovers the tag→`m$Type` map from each dispatch stub's own body + rewrites a stub
call whose arg was just `IAlloc`'d with a known tag in the same block (sound: stub
routes identically, no other effect). inline = params substituted, temps/labels
freshly renamed, `IReturn v`→`dst=v;goto ret`; single-level/sweep, skips recursive
callees (keeps tail-loops for the backend), body ≤ INLINE_MAX=12; then a reachability
sweep from main/__global_init__ drops now-dead functions (so static size FALLS —
08_traits bin 1828→1724, 25_torture asm 2450→1890). PASSES order = devirt, inline,
const_fold, copy_prop, algebraic, cse, dce, licm (O1 subset + relative order
unchanged, so O1 unaffected — O1 total still 7821). licm = real dominator/back-edge
natural-loop LICM but a NO-OP on the corpus (TAC CFG is acyclic — Lark iteration is
inter-procedural recursion, not a back-edge); validated on a synthetic loop in
`07/tests/opt_licm_test.py`. Four self-host differentials unaffected BY CONSTRUCTION
(`opt` imported only by optbench/opt_licm_test — grep-verified); O0 still identity so
`optbench_O0.json` NOT re-pinned; `diff_test` 34/0. NEXT = O3 (Tier-3 allocation
passes: closure elim, unboxing, fusion, affine arena reuse — where heap_allocs finally
moves).** Older O1 detail below. O1 INCREMENT 2 (2026-07-09): `const_fold` + `algebraic_simplify`
+ `cse` added to `07/src/opt.py`, registered in `PASSES` (order
const_fold→copy_prop→algebraic→cse→dce). Guard green: `optbench --levels 0,1` =
25/25 ok, ALL observably ==-O0; asm total 8106(-O0)→7821(-O1) (incr-1 was 7831 →
the 3 new passes account for −10: const_fold −6, algebraic −4, cse −2, on
16_stdlib/19_intoverflow/25_torture), heap allocs 361→360. Design locked to RV32
semantics: `_wrap32` = signed 32-bit (matches ADD/SUB/MUL & mask + `signed()`
print); comparisons fold via signed-wrapped operands (`slt` is signed); `/`·`%`
NEVER folded (÷0: RV32 sentinel vs CEK raise); folding gated on INT/BOOL consts
only (float `==`/`!=` stay IBinOp → excluded), so float/NaN off-path. `x−x` NOT
simplified (no type on IBinOp). `cse` is block-local, non-allocating only
(IBinOp/IUnary/IGetTag/IGetField, keyed by opcode+operands), with operand/dst-kill
on redefinition; CSE_ELIGIBLE (allocating-prim allowlist) still empty=deferred.
The 4 self-host differentials unaffected BY CONSTRUCTION — `opt.py` is imported
ONLY by `07/tests/optbench.py` (verified grep), nothing on the cek/emit_c/infer
path touches it; O0 still identity so `optbench_O0.json` NOT re-pinned. NEXT = O2:
inline / devirt / licm (see LEVELS[2]).** Older detail below.
The prior O0 build-out: New this
session: `07/src/opt.py` (the `-O`/per-pass plumbing — `optimize(tac,OptOptions)`,
`PASSES` empty so identity at every level = a ruler not yet a transform),
`07/src/riscv_vm.py` +9 output-neutral profiling counters (`dyn_instrs`/`stub_calls`/
`heap_allocs`/`heap_bytes`), and `07/tests/optbench.py` (per-file × per-`-O`-level
metrics via subprocess+timeout; observable-equivalence check; `--save`/`--compare`).
Baseline pinned: `07/tests/optbench_O0.json` + BASELINES.md O0 section (**now 25/25 ok ·
8106 asm instrs · 22.5 M exec instrs · 361 heap allocs**, incl. the new deep witness
`25_torture.lark` [item [2], 2026-07-09] = ~half the corpus's asm + ~80 % of its heap
allocs in one file; `diff_test` **34/0**). **`24_stringprims` RV32 crash
FIXED 2026-07-08** (self-contained, off the OPTIMIZE-pass work): the six string-decomp
prims were missing from the RV32 backend — added `string_index`/`string_slice`/
`char_to_string`/`float_to_bits` (plain runtime stubs) and lowered `string_to_int`/
`string_to_float` via a raw `[flag,payload]`-pair stub that `lower._lower_string_to_result`
wraps in `Ok`/`Err` (tags via `asm._collect_tags`, consistent with user `Ok`/`Err`);
mirrored in `tac_vm.py` (3rd backend). `diff_test.py` **33/0** (CEK==TAC==RV32
byte-identical). Baseline re-pinned (every binary `+24 B` = 6 new `ebreak` stub entries,
output unchanged). **O1 = append
const-fold/prop, algebraic-simplify, copy-prop, DCE, CSE to `opt.PASSES`, wire `-O1`,
guard by observable-equivalence (`optbench --levels 0,1`) + the 4 differentials.**
The prior OPTIMIZE detail follows. §8a: emitter O(n²) line join fixed
(`self/emit_c.lark` `ecJoinNL` → balanced `ecJoinPairs`), output-neutral —
emit-only bootstrap arena **~3 GB → 177 MB**, fixpoint at **512 MB** (C sha
`2ce6a281 → 49a4921c`). §8b: the second wall was the `infer` pass — Algorithm W's
assoc-list `Subst` applied O(n) times, never freed in the no-GC arena, overflowed
**>12 GB** on the 3534-line tc compiler. **Fixed** in `self/types.lark`:
`applyScheme` now short-circuits on a **closed** scheme (applying a subst to a
fully-generalised scheme is the identity → return unchanged, allocate nothing),
killing the env-side `applyEnv` multiplier. Output-neutral (`infertest` **42/0/2**
+ `typechecktest` **42/0/2** byte-identical). **M5.5.4 native fixpoint CLOSED:**
tc self-compile **>12 GB overflow → 10.15 GB completing**, and
`self/tests/bootstrap_tc.py` reaches **C1==C2==C3 byte-identical (sha `34a07692`,
18549 lines)** at a 14 GiB lazily-committed arena. Pinned in `BASELINES.md`;
`make -C self bootstrap-tc`. **Residual:** curve still ~O(n^1.9) (subst-side
`compose`/`apply` on the assoc-list `Subst`); a balanced-tree `Subst`
(O(log n) lookup, output-neutral) or arena compaction in `cek.c` is the next
reduction — deferred, not a blocker now the fixpoint closes. **NEXT (decided):
OPTIMIZE O1, but O0-metrics-rig FIRST.** The correctness half of O0 exists (the
`self/tests` differentials); the *measurement* half (§6) does not. So the next
session builds the ruler before any pass: per-file × per-`-O`-level static RV32I
instr count, executed-instr count (`riscv_vm.py` counters), heap allocs executed,
binary size, bootstrap wall-clock, + per-pass on/off flags; baseline current `-O0`
output. THEN O1 Tier-1 TAC passes (const fold/prop, algebraic simplify, copy prop,
DCE [liveness.py exists], CSE [sound via purity]); wire `-O1`. **Guard flips from
byte-identical to OBSERVABLE-EQUIVALENCE** (O1 changes emitted code by design):
optimized output == `-O0` output across the corpus + F2 bootstrap still reaches a
*behavioral* fixpoint. (Deferred alt: A1 tree-`Subst` in `types.lark`/`infer.lark`
— O(log n) lookup, output-neutral so guarded by the *existing* differentials —
a self-contained arena win to slot in opportunistically.) Then `PROVE.md`
(separate guarantees axis). See the newest log entry + OPTIMIZE.md §5/§6/§8.

*Freeze context:* Set
chose *freeze now*. The self-hosted Lark toolchain lexes, parses, **type-checks**,
and emits C; reproduces its own emitted C byte-identically (**F2**: C1==C2==C3, sha
`2ce6a281`, `make -C self bootstrap`); and rejects ill-typed programs with the
oracle-identical diagnostic (**F2⁺**: M5.5.3 = `make -C self typechecktest` =
**42 ok / 0 fail / 2 skip**, strictly stronger than emittest — the 7 error-suite
skips became live tests). Baselines pinned in `self/tests/BASELINES.md`; SELFHOST.md
status marked COMPLETE. Frozen differential counts: **infertest 42/0/2, cektest
33/0/15, emittest 37/0/7, typechecktest 42/0/2**. One residual, M5.5.4 (native
binary fixpoint of the ~3700-line *typechecking* compiler), is **OPTIMIZE-gated**:
the emitter's O(n²) line join in the no-GC arena wants >10 GB — handed to
`OPTIMIZE.md §8` as its motivated first win (fix the join, output-neutral, closes
the native fixpoint). **NEXT: activate `OPTIMIZE.md`** (O0 harness reuses these
differentials; first target = the arena-join wall). Then `PROVE.md` (a separate
*guarantees* axis — refinement types first, then MLTT; prototype in the Python
oracle at `07/src`, not self-hosted). F3 (Pico/RISC-V) stays a stretch, uncommitted.
**Hand-off to Set:** after committing the F2⁺ state, tag it — `git tag selfhost-f2`
(I don't commit/tag; Set commits own work).

*Prior position, for context:* SELFHOST **HARDENING PASS (1) DONE** (2026-07-08) —
the fuzz/property harness + skip/canon audit that was queued before F2 is in place
and green. `self/tests/fuzz_gen.py` deterministically generates random **well-typed**
Lark programs (type-directed over the four Copy scalars) and `self/tests/fuzz_difftest.py`
pushes each through **lex→parse→infer→emit** demanding `port == oracle` at every
stage (reusing the four sibling harnesses' own drivers/oracles; `make -C self fuzz`
/ `fuzzfast`). It immediately paid off: **found + FIXED a real port defect** —
`float_to_bits` was in the oracle's builtin env but missing from both `self/infer.lark`
AND `self/cek.lark` (the M0 runtime lock-step never reached the `.lark` ports), so
the port wrongly rejected/could-not-eval programs calling it; now byte-identical to
the oracle. Also surfaced a benign parsed-AST float-representation difference (port
keeps lexeme text; normalised downstream via float32 bits — canonicalised in the
fuzz parse compare) and an oracle robustness limit (`ty.apply` RecursionError on
long subst chains → skip). Full audit in **`self/tests/AUDIT.md`**: every skip
across all 5 differentials confirmed out-of-port-control (structural / no-oracle-
verdict / meta-circular perf, argued slow-not-wedged) and every canonicalisation
shown value-equivalent; oracle bugs the port mirrors catalogued. `fuzzfast`=80/80
over 40 progs; full four-stage `fuzz` (2000–2011)=all ok. WARNING Run only ONE
fuzz/infer/emit/cek harness at a time (siblings share fixed temp-driver filenames).
**ADT GENERATOR EXTENSION DONE (2026-07-08):** every generated program now carries
a fixed prelude ADT (`type Adt = | CA of Int | CB of Float, Float | CC` +
`impl Copy for Adt = {}`, keeping ADT values Copy so no affine reasoning), and
`Adt` joins the *binding* type universe (~1 in 4) — fuzzing the mixed-arity `type`
decl, construction (`CA(e)`/`CB(e,e)`/bare `CC`), and `match` on an ADT scrutinee
with field-binding constructor patterns; never where a scalar is required. Results:
full four-stage **fuzz (3000–3029) = 120 ok / 0 fail / 0 skip byte-identical**
port==oracle incl. constructor-`match` emitted to C, and front-end sweep
(5000–5079) = **160 ok / 0 fail / 0 skip**. Also re-verified my earlier
`float_to_bits` port fix carries no regression: **infertest 42/0/2, cektest
33/0/15** (both baseline). Reject-path fuzzing deferred (corpus error suite already
covers reject; byte-identical messages across random ill-typed progs are
order-sensitive) — see AUDIT.md §5. **NEXT: F2 two-stage binary bootstrap.**
*Prior position, for context:* SELFHOST **M5 TEXT
DIFFERENTIAL COMPLETE** (2026-07-08) —
the C-AST emitter is self-hosted. `self/emit_c.lark` (~445 lines) ports
`07/src/emit_c_ast.py` (typed AST → C source for the `cek.c` runtime), emitting
straight from `parse.lark`'s **syntactic** `Prog`: the emitter only reads
structure / names / literal values, never inferred types, and the typed AST is a
1:1 structural copy of the syntactic one — so `infer.lark` is not on this path.
State is one `Emit` record (four node counters `_e/_p/_d/_a` + `List(String)` of
emitted lines, newest-first) threaded purely; `impl Copy for Emit = {}` dodges the
affine-across-arms wart (SELFHOST §7). Differential harness
`self/tests/emit_c_difftest.py` concatenates lex+parse+emit_c (same assembly as
`cek_difftest`), embeds each corpus file's source as a String literal, and
compares the emitted C line-for-line vs the oracle: **37 ok / 0 fail / 7 skipped**
(`make -C self emittest`). Every self-containable file emits **byte-identically**,
including `09_parser.lark` (851 lines — the self-hosted parser emitted to C by the
self-hosted emitter), `08_life` (519), AND both multi-module `import` programs
(`09_modules/main`, `16_stdlib`), resolved by inlining. The remaining 7 skips are
exactly the 7 error-suite files the oracle rejects (exit 1) — nothing left in the
port's control. **Four findings logged in SELFHOST §7:** (a) Lark has **no `%` operator** (`%` is
not a lexer token) — hex/mod done as `n - (n/16)*16`; (b) added a runtime prim
**`float_to_bits : Float -> Int`** (IEEE-754 float32 bit pattern) in M0 lock-step
across `cek.py`/`cek.c`/`infer.py` (`24_stringprims.lark` parity, `make cektest`
= 31/31) — the emitter needs it for float literal inits and Lark had no way to get
the bits; (c) the oracle renames a wildcard param `_` → `_anon_{id(node)}` using a
**nondeterministic CPython `id()`** and bakes that address into the emitted C
(the name is never referenced) — no port can reproduce it, so the harness
canonicalises `_anon_\d+` → `_` on both sides (the token never appears in the
corpus); (d) the oracle's `emit()` wrapper emits imported decls TWICE (both
`_load_imports` and `typecheck` inline them), a real bug surfaced by self-hosting —
the harness inlines the imported bodies twice to mirror it, so both `import`
programs now pass. **The self-hosted front-end (lex→parse→infer), evaluator (cek),
and now C emitter (emit_c) are all differentially green vs the 07/ oracle — every
stage of the pipeline is self-hosted at the text level, incl. multi-module
programs.** **NEXT — a HARDENING pass BEFORE F2** (agreed with Set 2026-07-08):
(1) a **fuzz / property harness** — generate random well-formed Lark programs and
assert `port == oracle` through lex→parse→infer→emit, to attack the corpus-coverage
blind spot (the differential only proves equivalence on constructs the ~44 fixed
files happen to exercise); (2) an **audit of skips & canonicalisations** — confirm
each timeout-skip is merely slow (not a wedged port bug under a bigger budget), and
that each harness canonicalisation (`_anon_\d+`→`_`, error-message greek renaming)
erases only genuinely-equivalent differences, not real ones; plus a short catalogue
of the known oracle bugs the port intentionally mirrors (occurs-check gap,
import double-emit) so they aren't later mistaken for port defects. **THEN the F2
two-stage binary bootstrap** (emit the toolchain to C, build stage-1/stage-2
binaries, byte-diff); remaining cost is interpretive-tower perf, not features. *Prior position, for context:* SELFHOST **M3 COMPLETE** (2026-07-08) —
the whole type checker is self-hosted. `self/infer.lark` (~1213 lines) ports
`07/src/infer.py`
(Algorithm W + affine use-tracking + trait bounds), threaded purely: `fresh`=Int,
`tracked`=`List((String,Int))`, `env`=`List((String,Scheme))`, `Subst`=assoc list,
every failure an explicit Result-ADT (no exceptions/mutation). Differential
harness `self/tests/infer_difftest.py` runs `infer.py`'s `typecheck` **in-process**
(oracle), serialises its TProgram to the same normalised signature block the port
prints (type vars → α,β,γ by first occurrence), and compares vs `checkProgram`
over the corpus: **42 ok / 0 fail / 2 skipped** (`make -C self infertest`). Every
single-file test matches — all 24 feature tests, `Stdlib.lark` (27 sigs), all 9
samples (incl. `09_parser.lark` = the self-hosted parser type-checking itself,
21 sigs), and the full error suite (`errors/*` rejects match by message;
`04_nonexhaustive`/`06_matchfail` correctly *accept* — infer.py checks no
exhaustiveness). The 2 skips are exactly the two `import` files (`09_modules/main`,
`16_stdlib`); the port has no import mechanism. Makefile gains `infersmoke`/
`infertest`, and `infertest` joins the `test` aggregate. **The self-hosted
front-end (lex→parse→infer) + evaluator (cek) are all differentially green vs the
07/ oracle.** **NEXT: M5/M6** (bootstrap / fixpoints F1–F3) per SELFHOST.md — the
remaining work is the interpretive-tower perf (cubic meta-circular cost; a
compiled bootstrap removes it), not features. *Prior position, for context:*
SELFHOST **M4 evaluator FEATURE-COMPLETE for the slice**
(2026-07-07) — the meta-circular CEK evaluator. `self/cek.lark` (~690 lines)
ports `07/src/cek.py`'s evaluator (RVal runtime values R-prefixed to dodge
parse.lark's `Val`; two-level env = local assoc list + threaded globals; output
accumulated into a String and printed once). Differential harness
`self/tests/cek_difftest.py` compares stdout vs cek.py over the corpus:
**29 corpus + 4 read = 33 ok / 0 fail / 15 skipped**. Floats, custom
trait-method dispatch, **multi-module `import` (via harness inlining)** and
**`read` (stdin, via `RIO of String`)** all done — see newest log entry. All
15 skips are legitimate (oracle-rejected library files, or too-slow
meta-circular loops). **07/ oracle stays untouched.** Remaining M4→F1 work is
not features but the interpretive-tower perf (cubic; a compiled bootstrap
removes it) — so the natural next step is to resume **M3's `infer.lark`**.
[Older status, for context:] the previous checkpoint read
**28 ok / 0 unexpected fail / 16 skipped** (44 files) after float arithmetic AND
custom trait-method dispatch landed. Skips are all documented: `import` (Stdlib,
slice 1) and compute-heavy files that exceed the 90 s meta-circular budget
(timeout ⇒ skip-with-reason, not fail; the set shrinks/grows a little run-to-run
as borderline files like 08_life cross the budget). Passing set now includes
**08_traits** (custom dispatch) and 8/9 samples.
**Two findings logged in SELFHOST §7:** (a) an `infer.py` **occurs-check gap** —
a generic fn that *builds* `List(a)` from a polymorphic element loops the checker
(`compose` makes `α↦α`, no occurs-check); worked around by monomorphising
`ckSnoc` to `List(RVal)`; (b) meta-circular eval is cubic (harness perf, not a
correctness gap). **Float arithmetic DONE** and **custom trait dispatch DONE**
(see newest log entries): floats via a `string_to_float` prim (cek.py+infer.py;
cek.c parity pending) + real-`Float` `RFloat`; dispatch via an `RDispatch of
String` value, a `runtimeType(v,g)` helper, and a con→type map + `<meth>#<type>`
dispatch entries stashed in the threaded globals `g` (keys contain `#`, which no
Lark identifier can, so no collisions). `apply` now takes `g` so `RDispatch`
resolves the impl by the argument's runtime type. **cek.c `string_to_float`
DONE** (2026-07-07, newest entry): the prim now exists in *both* CEK runtimes
(Python + C) with byte-identical parity — `07/tests/24_stringprims.lark` extended
to exercise it, `make cektest` = **31/31** C↔Python. That closes the last runtime
parity gap the float work left open. **Next in M4:** `read` (stdin), then lift
the import/Stdlib skip so real multi-module programs run.
**M3 (in progress):** slice 1/3 `self/types.lark` (ty.py port) + slice 2/3
`self/tast.lark` (typed_tree.py port) both landed; the one still pending is the
big one — `infer.py` → `infer.lark` (Algorithm W, Pass 1.5, traits+affine) + its
differential harness. The M-prefixed monotype ctors and `Result`-threaded unify
are fixed in `types.lark`; the typed AST + `tDeclSig` signature printer are in
`tast.lark` (see the M3 slice-2 entry below).
M2 (done): `self/parse.lark` ports `tree.py`+`parser.py`+S-expr printer,
`make -C self parsetest` = 47/47 byte-identical incl. self-parse.
OPTIMIZE is planned, deferred until after self-hosting.
**Also queued (user's next big interest, drafted 2026-07-07):** `PROVE.md` — push
Lark toward refinement/dependent types so it can *prove programs*, not just build
itself. Independent of self-hosting/optimizer; to be pursued after self-hosting
(or after OPTIMIZE). Refinement-first (V1/V2), then mechanize the refinement
calculus' soundness in lcore (V3), full dependent elaboration via ch12 bridge
(V-deep).
...
file as `REGRESS`, and prints a trailing `⚠ N output change(s), M regression(s)` (or
