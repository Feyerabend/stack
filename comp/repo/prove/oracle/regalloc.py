...
def _compute_intervals(cfg: CFG, lv: Liveness,
    ...
    for blk in cfg:
        ...
        # These sets are frozensets: iterate them sorted, or `ivs` insertion
        # order — the tiebreak of the stable sort in _linear_scan — becomes a
        # function of PYTHONHASHSEED and the emitted assembly varies per run.
        for name in sorted(lv.live_in[blk.label]):
            extend(name, s); extend(name, e)
        for name in sorted(lv.live_out[blk.label]):
            extend(name, s); extend(name, e)

        # Individual def/use points
        for i, instr in enumerate(blk.instrs):
            p = s + i
            for name in sorted(defs(instr)): extend(name, p)
            for name in sorted(uses(instr)): extend(name, p)
