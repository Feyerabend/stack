...
def syntype_to_mono(t: Type, tvar_env: dict[str, TVar], fresh: Fresh) -> Mono:
    ...
    match t:
        ...
        case STRefine(base=b):
            # Refinements ERASE here, and this single line is the whole interface
            # between the refinement layer and the HM core: {v:Int|p} is Int as far
            # as inference, generalisation and — decisively — the affine checker are
            # concerned.  The predicate is never traversed, so a name it mentions is
            # never `infer`red, so `tracked[n]` is never incremented: a predicate is a
            # mention, not a use.  refine.py's header argues why that is sound.
            return syntype_to_mono(b, tvar_env, fresh)
...
def check_fn_decl(
    ...
    # already registered the annotated type; generalise it, so that each recursive
    # call instantiates it afresh.  Bound to a bare monotype var instead, every
    # recursive call would have to agree about one unknown — and a polymorphic
    # function that recurses on its own result could not be typed at all.
    rec_var = fresh.var()
    fully_annotated = (decl.return_type is not None
                       and all(p.ann is not None for p in decl.params))
    prior = env.get(decl.name)
    rec_sc: Scheme
    if fully_annotated and prior is not None:
        outer: Env = {k: v for k, v in env.items() if k != decl.name}
...
def _initial_env() -> tuple[Env, frozenset[str]]:
    ...
    env: Env = {
        ...
        "read_all": Scheme((), TFn(T_IO, TTup((T_IO, T_STRING)))),
        ...
        "float_to_bits":   Scheme((), TFn(T_FLOAT, T_INT)),
        ...
        "min":             Scheme((), t_fn(T_INT, T_INT, T_INT)),
        "max":             Scheme((), t_fn(T_INT, T_INT, T_INT)),
        ...
        # String decomposition (for self-hosting): index/slice/build/parse.
        # Indices and codepoints are Int; String is Copy (see BUILTIN_COPY), so
        # the source text may be indexed non-linearly.
        "string_index":    Scheme((), t_fn(T_STRING, T_INT, T_INT)),
        "string_slice":    Scheme((), t_fn(T_STRING, T_INT, T_INT, T_STRING)),
        "char_to_string":  Scheme((), TFn(T_INT, T_STRING)),
        "string_to_int":   Scheme((), TFn(T_STRING, t_result(T_INT, T_STRING))),
        "string_to_float": Scheme((), TFn(T_STRING, t_result(T_FLOAT, T_STRING))),
