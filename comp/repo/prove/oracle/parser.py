...
from tree import (
    ...
    # Declarations
    Decl, FnDecl, LetDecl, TypeDecl, TraitDecl, ImplDecl, MeasureDecl,
    # Top-level
    ImportDecl, Program,
)
...
class Parser:
    ...
    def parse(self) -> Program:
        ...
        decls, measures = self._parse_decls()
        self._expect(TK.EOF)
        return Program(module, tuple(imports), tuple(decls), tuple(measures))
    ...
    def _parse_decls(self) -> tuple[list[Decl], list[MeasureDecl]]:
        """The two halves of a program: what runs, and what is only ever proved.

        They are separated here, at the door, and never rejoined — that is where
        the ghosthood of a measure actually lives (tree.MeasureDecl)."""
        decls:    list[Decl]        = []
        measures: list[MeasureDecl] = []
        while not self._at(TK.EOF):
            if self._at_measure():
                measures.append(self._parse_measure_decl())
            else:
                decls.append(self._parse_top_decl())
        return decls, measures

    def _at_measure(self) -> bool:
        """`measure` is a CONTEXTUAL keyword, and the lexer is not told about it.

        It has to be: adding it to _KEYWORDS would reserve the word everywhere,
        breaking every existing program that has a variable called `measure` — and
        the oracle's lexer is frozen, so a real keyword would mean
        drifting lexer.py for a word only the type layer cares about.  No ambiguity
        is created: a declaration could never begin with a bare NAME before, so the
        only token stream this newly accepts is one that used to be a syntax error.
        """
        tok = self._peek()
        return tok.kind == TK.NAME and tok.text == "measure"

    def _parse_top_decl(self) -> Decl:
        exported = bool(self._match(TK.EXPORT))
        tok = self._peek()
        if exported and self._at_measure():
            raise self._error(
                "a measure is ghost: it has no runtime value to export")
    ...
    # -- measure declaration (the 08/ fork; the refinement work) --

    def _parse_measure_decl(self) -> MeasureDecl:
        """measure len(xs : List(a)) : Int = match xs with ... end

        Shape only.  Whether the arms cover the constructors exactly once, whether
        the recursion is structural, and whether the bodies live in the predicate
        fragment are all questions about *meaning*, and they are refine.py's
        (`Refiner._elab_measure`).  The one thing enforced here is that the return
        type is written: a measure with no declared sort is not a measure the logic
        can use, and there is no inference to fall back on.
        """
        ...
        if not self._match(TK.COLON):
            raise self._error(
                f"measure '{name}' needs a declared result type (: Int or : Bool)")
        ret = self._parse_type()
        self._expect(TK.ASSIGN)
        body = self._parse_expr()
        return MeasureDecl(name, params, ret, body)
    ...
    def _parse_atom_type(self) -> Type:
        ...
        if tok.kind == TK.LBRACE:
            return self._parse_refine_type()
    ...
    def _parse_refine_type(self) -> Type:
        """{ v : Int | v >= 0 }  — a refinement type (the 08/ fork; the refinement notes).

        LL(1) is preserved: '{' begins no other type, and the predicate is an
        ordinary expression, whose grammar has no '|' — so the predicate parse
        always stops at the closing brace on its own.
        """
        self._expect(TK.LBRACE)
        var = self._expect(TK.NAME).text
        self._expect(TK.COLON)
