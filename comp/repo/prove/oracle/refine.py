...
generate (the refinement work–2).
...
WHAT THIS CHECKER DOES NOT DO:
  ...
  - No refinement INFERENCE.  A position with no annotation gets `{v : b | true}`,
    ...
    checker failing, it is the checker declining to guess.
  - No proof of the CHECKER ITSELF.  The guarantee is "checked by a trusted tool",
    not "checked by a proven tool"; the soundness proof lives separately, in the
    lcore metatheory, and is not what runs when this checker runs.
"""
...
# "cannot prove" — the sound direction, as the first version requires.
FLOAT = "float"
...
class RBase:
    ...
    b is `Int`, `Bool`, or the name of any type the logic cannot
    ...
    # a missing monotype means the field stays OTHER, exactly as before.
...
@dataclass(frozen=True)
class RTuple:
    """(S, T, ...) — a product, refined componentwise.  the refinement work.
...
def rtype_of_mono(m: Mono, adts: set[str] | None = None) -> RType:
    ...
    what a later version needs: a value of a declared ADT gets `{v : List | true}` rather than
    ...
    It costs nothing, for the reason a later version already established: the binder is sorted
...
# Lark function body is not.  these equations are built and *checked* here; a separate step hands them
...
# is vacuously kept, and the first version already leans on exactly that.  A non-terminating *measure*
...
@dataclass(frozen=True)
class Guard:
    """One step of a NESTED match, flattened.
...
class MArm:
    ...
    `bsorts` is what the measure-into-logic step needs that the bare structural pass did not: to instantiate this equation at a
    ...
    `guards` is the flattened chain of a NESTED match: empty for
...
@dataclass(frozen=True)
class Measure:
    """A well-formed measure: what a later version will read, and nothing it has to re-check."""
    name:  str
    param: str                            # the structural argument
    tycon: str                            # the ADT it takes apart
    extra: tuple[tuple[str, RType], ...]  # the non-structural parameters
    ret:   RBase                          # base + the declared refinement
    arms:  tuple[MArm, ...]
...
class ResultAxiom:
    ...
    call `xs`.  That is a later version exactly — a symbol is a function, not a name — and
...
#
# Until a later version that something was a bare set literal (`NONNEG_UF = {"string_length"}`)
...
PRIMITIVE_AXIOMS: dict[str, "ResultAxiom"] = {
    ...
    # `min`/`max`.  Three facts, no more: two ORDERING bounds and the
...
# -- Measures into the logic (the refinement work) ------------------------------------
#
...
# walk has already seen.  a later version's structural check, which was there to keep the axioms
...
def _instantiate(m: "Measure", arm: MArm, target: Term,
    ...
    A measure with EXTRA parameters is instantiated with the
    ...
    # The NESTED match, one guard at a time.  Each guard names a
...
def measure_axioms(fs: list[Formula],
    ...
    descends into — so the axiom set closes in a single pass.

    An EXTRA-PARAMETER measure (`lt(t, b)`, a later version) cannot: its equation is about
...
def _term_opt(e: Expr, env: Env) -> Term | None:
    match e:
        ...
        # A CONSTRUCTOR IS A TERM.  `Cons(x, xs)` becomes the uninterpreted
...
def formula_opt(e: Expr, env: Env) -> Formula | None:
    match e:
        ...
        case IfExpr(cond=c, then_=t, else_=e):
            # A GUARDED PREDICATE.  `if c then p else q` means exactly
            ...
            #
            # This is a later version's law one level up from the solver, and the same rule
...
def _branch(c: Expr, env: Env) -> tuple[Formula | None, Env, Env]:
    ...
    This is the first version's rule, and it now holds one level up from the solver: a checker that
...
class Refiner:
    def __init__(self, program: Program, tprog: object, path: str) -> None:
        ...
        # is the "layer on top of Hindley-Milner" of the refinement notes, made concrete —
        ...
        # binders no types at all, in the earlier versions.  A monomorphic one (`| Buf(n)
        ...
        # Computed BEFORE the globals, because the globals need it: a
        ...
        # Which ADT a constructor belongs to, and — the measure index — which measure
        # equations fire at it.  A measure is TOTAL, so every
        ...
        for d in program.decls:
            if isinstance(d, FnDecl):
                ...
                # nobody checks is exactly the false proof a later version was about.
                for tm in d.methods:
        ...
        match t:
            case TRefine(var=v, base=b, pred=p):
                ...
                if base_name is None:
                    # A tuple or a function.  a later version refines the COMPONENTS of a product,
                ...
                #.  Only for ADTs, and best-effort: if the base does not
    ...
    # -- measures: surface → equations (the refinement work) --
    ...
    def _elab_measure(self, md: MeasureDecl) -> Measure:
        ...
        out: list[MArm] = []
        # (arm, the env its body was translated in, the body) — kept for a later version, which
    ...
    def _resolve_adt(self, ty: Type, name: str) -> tuple[tuple, dict[str, Type], str]:
        ...
        turn.  The same checks either way: it must be a declared
    ...
    def _elab_arms(self, name: str, ret: RBase, p0_name: str, p0_ann: Type,
        ...
        into the flat set of `MArm` equations.
    ...
    def _prove_measure(self, m: Measure, proofs: list[tuple[MArm, Env, Expr]]) -> None:
        ...
        over its own arms (the refinement work).  One VC per constructor; no new solver.
        ...
        (`NONNEG_UF`) saying that a length is not negative — a thing the first version knew and, in
        ...
        itself needs no special handling: `m(C(y…))` is in the goal, so a later version's walk
        instantiates the arm's own equation at it, and congruence does the rest.  The
        induction is *structural* because the arms are: every
        ...
        for (arm, env, body) in proofs:
            ...
            # shape its equation fires at.  Each guard replaces
            ...
            # extra parameters is where that stops being automatic.  `drop(xs, i)`
            ...
            # them (the observation the refinement notes leans on), so nothing leaves the
            # fragment.  Until a later version lets an equation fire for such a measure, this
    ...
    def _measure_ret(self, md: MeasureDecl) -> RBase:
        """The declared result type, as a refinement.  a later version recorded the predicate and
        did nothing with it; it is PROVED (`_prove_measure`, by induction
    ...
    def synth(self, e: Expr, env: Env) -> RType:
        ...
        match e:
            ...
            # THE THIRD ROAD INTO A FLOAT'S TYPE, and a later version paved only two of them.
            ...
            # vacuously.  The same bug as a later version, reached by a different road.
            case Lit(value=v) if isinstance(v, float):
                return ROpaque(sort=FLOAT)

            # THE BUILDING HOOK.  A saturated constructor application knows
            ...
            case BinOp(op=op, left=l, right=r) if op in ("+", "-", "*", "/"):
                ...
                if op == "/" and sort_of_rtype(lt) != FLOAT \
                    ...
                    # freedom of the refinement work, and it applies to EVERY `/` in the
                    ...
                    #
                    #   `100 / help(...)` where `help` is a LOCAL lambda — a later version made
                    ...
                    #
                    # This is a later version's rule at the OPPOSITE POLARITY, and the two must
            ...
            # A LAMBDA'S BODY IS CODE, AND CODE IS WALKED.
            #
    ...
    def _pat_term(self, p: Pat, env: Env,
        ...
        `len(xs) == 1 + len($f1)` — weak, but it is the fact a later version will multiply by
    ...
    def _walk_match(self, e: MatchExpr, env: Env,
        ...
        Only literal patterns speak to the logic in the first version: `| 0 =>` tells us the
        ...
        THE DESTRUCTING HOOK is the line that used to say "constructor patterns
            ...
            match pat:
                ...
                case PCon(name=c, args=ps) if st is not None and \
                    ...
                    if ok:
                        ...
                        # have and that a later version does not need.  So we take the positive
    ...
    def _bind_pattern(self, pat: Pat, env: Env, scrut: RType | None = None) -> Env:
        ...
        A TUPLE pattern was the first version's gap, and it is what a later version closes.  `infer_pat` can
        ...
        at sort OTHER and the first version could not see through `match f(x) with | (a, b) => ...`,
        ...
        try:
            ...
            return env
        # a later version — pin polymorphic fields against the scrutinee's ACTUAL monotype.
...
def _builtin_contracts() -> dict[str, RType]:
    ...
    return {
        ...
        # the facts live in the axiom table where they can be audited.
        "min": RFun(
...
class Result:
    ...
    # direction is the whole of the first version.  It is reported separately because it is a
