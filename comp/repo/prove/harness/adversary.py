...
The adversary — random Lark programs against the checker's own promise.
    ...
    python3 harness/adversary.py [cases] [seed]      # or: make adversary
...
that BUILDS the obligations — the four false proofs of a later version and the unwalked `impl`
body of a later version — and all five were found BY HAND, under a suite that was green the whole
...
THREE more false proofs, none of which two hand-audits had caught (the refinement work):

  - a divisor the logic could not NAME (`x * y`, or a local lambda's result) raised no
    obligation at all — a later version's rule at the opposite polarity.  A hypothesis it cannot
  ...
  - and the floor: a division inside an `if` CONDITION, a comparison, a `not` or a unary
    ...
    Open .  Sitting inside 07's own primes sieve the whole time.     → 20_condition
...
The generator is aimed at what has actually drawn blood, not at Lark in general:
  ...
  - guards it cannot read, for the same reason
  - Floats, and the two NaN factories `0.0 / 0.0` and `float_sqrt(-1.0)` —
    now in BOTH the positions a Float reaches the checker from: a literal/param in a
    guard, and (the float interlude, fixtures/29) a CONSTRUCTOR FIELD, destructured and
  ...
  - local bindings that SHADOW a global function
  - a program that redeclares `string_length`
  - `impl` bodies with real code in them
  - `string_index`, the one builtin with a precondition worth proving
  - `min`/`max`, in BOTH the positions that matter: as a DIVISOR over
  ...
  - `if`-PREDICATES, as coverage: checked and run under random code, though
    ...
    (see `abs_decls`)
  - NESTED-MATCH measures, the shapes a BST needs and the
    ...
    computes here (see `nested_measure_decls`).  Two shapes, one per direction:
      · `rd`, a nested Int measure with a DECLARED result, consumed as a DIVISOR — the lie-and-catch hazard one match deeper: `{v >= 1}` false of a Leaf arm of 0,
...
# carries a real precondition (`fixtures/01_bounds`).
...
class Gen:
    ...
    def expr(self, env: list[str], depth: int) -> str:
        ...
        if r < 0.22:
            # `min`/`max` — the surface the adversary could not SPEAK
        ...
        if r < 0.78:
            ...
            # through a guard — which is precisely the road a later version came down.
            if self.has_floatbox and self.rng.random() < 0.5:
                # The SAME reflexivity trap, but the Float now arrives through a
                # CONSTRUCTOR FIELD (fixtures/29): build an `FB`, bind its Float field, and
        ...
        if r < 0.97:
            ...
            # said "a local lambda's result" , because that was the SHAPE of
    ...
    def divisor(self, env: list[str], depth: int) -> str:
        ...
        if r < 0.70 and self.in_entry and self.has_measure and "m" in env:
            # a later version's hazard — but only reachable through the NAME `m` (see `program`).
            return self.rng.choice(["m", "(m + 1)", "(m - 1)"])
        if r < 0.82:
            # `min`/`max` of two atoms — a later version's hazard as a DIVISOR.  The atoms
    ...
    def cond(self, env: list[str], depth: int) -> str:
        r = self.rng.random()
        if r < 0.25:
            # A guard the logic cannot READ (non-linear).  a later version: it must constrain
            # NEITHER branch.
            return (f"({self.atom(env)} * {self.atom(env)} "
    ...
    def measure_decls(self) -> list[str]:
        ...
        computes it — the shape, generated to be believed or caught.
        ...
        `Nil` arm is 0, and the whole of a later version is that the lie must be caught by
    ...
    def bool_measure_decls(self) -> list[str]:
        """A BOOL-valued measure and a consumer that divides by a field it guards — the shape, and the language this generator learned to speak last.
        ...
        solver bridge a later version added.  `root` may read a node's value only under `{v : Tree |
    ...
    def poly_measure_decls(self, declare_list: bool) -> list[str]:
        """The shape: a BOOL measure over a POLYMORPHIC `List(a)`, destructured at a
    ...
    def abs_decls(self) -> list[str]:
        ...
        the shape, and the second language this generator learned to speak.
    ...
    # -- nested-match measures: the tree shapes -----
    #
    ...
    def nested_measure_decls(self) -> list[str]:
        ...
        `measure_decls`' a later version hazard and the thing I6 watches.  An honest bound, or `Int`
    ...
    def program(self, idx: int) -> str:
        ...
        if redeclare:
            # a later version: the program's own `string_length`, which the axiom is NOT about.
        ...
        if self.has_floatbox:
            # THE FLOAT INTERLUDE's road (fixtures/29).  A Float carried in a CONSTRUCTOR
        ...
        pre = ""
        if shadow:
            # a later version: a local binding that captures a global function's NAME.
            pre = f"let help = fn(a, b) => {rng.randint(0, 3)} in\n  "
        xs_param = ", xs : List(Int)" if self.has_measure else ""
        xs_arg   = ""
        if self.has_measure:
            # THE ONLY ROAD TO a later version'S HAZARD, and finding it took disarming the check to
            ...
            # `v == m1(xs)`.  And a contract reaches the logic through a NAME:
        ...
        if self.has_bool_measure:
            # The hazard, and it is a NAME too, for the reason the Int measure's is:
        ...
        if self.has_poly_measure:
            # The hazard, a NAME for the same reason: `phead(list)` bare is
        ...
        if self.has_abs:
            # The COVERAGE shape (see abs_decls for why it is coverage, not a
        ...
        if self.has_floatbox and rng.random() < 0.7:
            # Force the float-field hazard (fixtures/29_floatfield_unsafe's shape).  Wrap the
...
def main() -> int:
    ...
    for i in range(cases):
        ...
        try:
            ...
            errored += 1
            continue
        except Exception:                       # a crash is never a verdict
    ...
    if false_proofs:
        ...
        for i, src, why in false_proofs[:5]:
            ...
            print()
        print(f"  reproduce with: python3 harness/adversary.py {cases} {seed}")
        return 1
