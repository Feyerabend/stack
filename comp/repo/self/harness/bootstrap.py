"""
The bootstrap fixpoint — the two-stage self-hosting proof for lark-in-lark.

This packages true self-hosting as one repeatable command.
It assembles the self-hosted compiler and drives the classic bootstrap ladder:

    stage0  = the Python oracle (oracle/emit_c_ast.py) compiles compiler.lark
...
(LC_UUID, embedded paths) and is deliberately NOT asserted here.
...
minutes with a ~3 GB peak.  That is a performance characteristic,
not a correctness one; the arena size is overridable below.

Usage:
    python3 harness/bootstrap.py            # full ladder: C1==C2==C3
    python3 harness/bootstrap.py --stages 2 # minimal proof: C1==C2
    python3 harness/bootstrap.py --keep DIR # write artifacts to DIR (kept)
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="bootstrap fixpoint")
