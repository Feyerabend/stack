"""
Differential test for the self-hosted C-AST emitter.

For every self-contained .lark file in the corpus it compares two C outputs:

  • the oracle — oracle/emit_c_ast.py (the frozen Python emitter) run on the
    file, its stdout (the generated C source) captured;
  • the port    — lark/emit_c.lark, driven through the Python CEK, emitting the
...
Pass 1.5 — a known wart of the language); the driver is built by CONCATENATION:
...
Usage:
    python3 harness/emit_c_difftest.py [-v]      # -v: show a unified diff
"""
...
# oracle's emit() wrapper, surfaced by self-hosting (a known wart of the language); the
