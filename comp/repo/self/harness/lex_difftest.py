"""
Differential test for the self-hosted lexer.

For every .lark file in the corpus it compares two token streams:

  • the oracle  — oracle/lexer.py (the frozen Python lexer), serialized here;
  • the port     — lark/lex.lark, run through the CEK interpreter (oracle/cek.py).
...
Usage:
    python3 harness/lex_difftest.py [-v]      # -v: show a diff on failure
"""
