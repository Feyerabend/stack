"""
Smoke test for the TAC IR (tac.lark).
...
the optimizing pipeline is assembled by CONCATENATION rather than `import` (see the note
...
under one module and runs it through the CEK interpreter (oracle/cek.py).
...
oracle it is diffed against by `make emittactest`.

Usage:
    python3 harness/tac_smoke.py            # exits 0 on success
"""
