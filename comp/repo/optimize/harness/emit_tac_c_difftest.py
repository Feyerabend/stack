"""
Differential test for the self-hosted TAC→C emitter (
emit_tac_c.lark: the port of oracle/emit_tac_c.py).
...
source, so this harness ISOLATES it the same way lower_difftest does:
  ...
  • the oracle parses + typechecks (infer.py) + lowers (lower.py) the object
    program to a TAC value — WITH NO OPTIMIZATION (opt.lark is not yet
...
emit_tac_c.lark + tac.lark (already green) run in Lark.  No types/tast module
...
Usage:
    python3 harness/emit_tac_c_difftest.py [-v]     # -v: unified diff on fail
"""
