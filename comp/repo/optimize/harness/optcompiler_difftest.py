"""
End-to-end differential for the self-hosted OPTIMIZING compiler (the behavioral pipeline).
...
Every earlier differential fed an oracle-produced IR (serialised TAC / TProgram)
into ONE Lark module in isolation.  Here the modules compose end to end — in
particular `infer.lark`'s new `inferProgram` produces the typed AST that
...
It also checks the behavioral claim directly: for each file the port's C at -O0
...
Usage:
    python3 harness/optcompiler_difftest.py [-v] [--levels 0,3] [--only NAME]
"""
...
def make_compiler(source: str, level: int) -> str:
    ...
    # io is affine and cannot appear in two match arms (an affine wart) — build the
...
def main() -> None:
    ...
    for path in corpus():
        ...
        # Behavioral report: does the port emit identical C at -O0 vs the top level?
