"""
Differential test for the self-hosted type checker (
infer.lark: the port of oracle/infer.py, Algorithm W + affine + trait bounds).

For every self-contained .lark file in the corpus it compares two verdicts:

  • the oracle — oracle/infer.py's `typecheck`, run IN-PROCESS.  On success its
  ...
  • the port    — lark/infer.lark's `checkProgram`, driven through the Python
...
toolchain's import path skips infer Pass 1.5):
...
The error suite (tests/errors/*) is the heart of the REJECT half: each file
is a program that must be rejected for a specific reason, and the port must
reject it with the same message.

Usage:
    python3 harness/infer_difftest.py [-v]     # -v: show a diff on failure
"""
