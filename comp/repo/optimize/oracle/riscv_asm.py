...
class LarkAssembler:
    ...
    RUNTIME_STUBS: list[str] = [
        ...
        '__string_to_int_raw', '__string_to_float_raw',
    ]
    ...
    def _parse_instr(self, parts: list[str], addr: int) -> Instruction:
        ...
        def target(i: int) -> int:
            t = parts[i]
            off = self.labels[t] - addr if t in self.labels else imm(i)
            # Defensive range checks: after relaxation these must hold; a failure
            # means a silent-truncation bug (the class the original derailment was).
            if op in self._BRANCH_INV:
                if not (self._B_LO <= off <= self._B_HI):
                    raise ValueError(
                        f"branch to {t!r} out of ±4KB range ({off}) after relaxation")
            elif op in ('JAL', 'J'):
                if not (-(1 << 20) <= off <= (1 << 20) - 2):
                    raise ValueError(
                        f"jump to {t!r} out of ±1MB range ({off}); needs long-call relaxation")
            return off
    ...
    # -- Branch relaxation --------------------------------------------------
    #
    # B-type conditional branches encode a 13-bit signed offset (range
    # [-4096, +4094]).  The generator emits label-relative branches with no
    # awareness of that range; for corpus-sized programs every branch fits, but
    # a large program (e.g. the self-hosted compiler, ~131 KB of text) has
    # branches whose target is farther than ±4 KB away.  Without relaxation the
    # offset silently truncates and the PC derails into garbage.
    #
    # Relaxation rewrites an out-of-range `b<cc> ops, FAR` into
    #     b<!cc> ops, .Lrlx_N   # invert: skip the trampoline when NOT taken
    #     j FAR                 # JAL, ±1 MB range — always in range here
    #   .Lrlx_N:
    # We use the *exact* hardware range as the trigger, so any branch the
    # hardware can already encode is left untouched — corpus binaries stay
    # byte-identical.  Only genuinely unencodable branches are rewritten.
    _BRANCH_INV = {
        'BEQ': 'BNE', 'BNE': 'BEQ', 'BLT': 'BGE', 'BGE': 'BLT',
        'BLTU': 'BGEU', 'BGEU': 'BLTU', 'BEQZ': 'BNEZ', 'BNEZ': 'BEQZ',
        'BGT': 'BLE', 'BLE': 'BGT', 'BGTU': 'BLEU', 'BLEU': 'BGTU',
    }
    _B_LO, _B_HI = -4096, 4094       # 13-bit signed, even offsets

    def _addr_walk(self, lines: list[str]):
        """Yield (idx, addr, label_prefix, parts) for each instruction line,
        mirroring _pass1's address accounting exactly."""
        addr = 0
        section = '.text'
        for idx, raw in enumerate(lines):
            ...
            if line.startswith('.byte'):
                rest_b = line[len('.byte'):].strip()
                addr += len([v for v in rest_b.split(',') if v.strip()])
                continue
            if line.startswith('.p2align'):
                n = int(line.split()[1])
                align = 1 << n
            ...
            parts = line.replace(',', ' ').split()
            if parts:
                yield idx, addr, label_prefix, parts
                addr += self._instr_size(parts)

    def _relax_branches(self, lines: list[str]) -> list[str]:
        """Rewrite out-of-range conditional branches until every branch fits.
        Converges: relaxation only grows code (pushes targets farther), and a
        relaxed branch is near (to .Lrlx_N) + a JAL, so it never relaxes again."""
        counter = 0
        for _ in range(64):                       # safety cap; converges in ~1-2
            self.labels = {}
            self._pass1(lines)
            rewrites: dict[int, list[str]] = {}
            for idx, addr, lblpre, parts in self._addr_walk(lines):
                op = parts[0].upper()
                if op not in self._BRANCH_INV:
                    continue
                target = parts[-1].rstrip(',')
                if target not in self.labels:      # numeric offset — leave as-is
                    continue
                off = self.labels[target] - addr
                if self._B_LO <= off <= self._B_HI:
                    continue
                counter += 1
                skip = f".Lrlx_{counter}"
                inv = self._BRANCH_INV[op]
                ops = ' '.join(parts[1:-1])
                first = f"{inv} {ops}, {skip}"
                if lblpre is not None:
                    first = f"{lblpre}: {first}"
                rewrites[idx] = [first, f"j {target}", f"{skip}:"]
            if not rewrites:
                break
            new_lines: list[str] = []
    ...
    def assemble(self, source: str) -> bytes:
        ...
        full = self._preamble() + "\n" + source
        lines = full.split('\n')
        lines = self._relax_branches(lines)   # fix >±4 KB conditional branches
        self._pass1(lines)                    # final authoritative label map
