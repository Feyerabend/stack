"""
O5′ fixpoint — the self-hosted OPTIMIZING compiler reproduces itself
(SELFHOST M7.5 / OPTIMIZE §9.4, claim 2 — the capstone).
...
→ emitTacC — so optimization lives INSIDE the self-hosted compiler and on the F2
...
tree — expect the §8a/§8b arena wall).
...
prints once (M5.5 wart).

Usage:
    python3 self/tests/bootstrap_opt.py                       # full self-compile ladder
    python3 self/tests/bootstrap_opt.py --input ../07/tests/01_hello.lark --stages 2
    python3 self/tests/bootstrap_opt.py --level 0             # unoptimized self-compile
    python3 self/tests/bootstrap_opt.py --arena-mb 15000 --keep DIR
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="O5' optimizing-compiler fixpoint")
    ...
    if args.stages == 1:
        ...
        print(f"\nO5' FIXPOINT REACHED — {label} byte-identical ({hashes[0][:12]})")
    else:
