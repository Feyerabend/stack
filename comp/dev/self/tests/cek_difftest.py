"""
Differential test for the self-hosted CEK evaluator (SELFHOST M4, slice 1).

For every self-contained .lark file in the corpus it compares two program outputs:

  • the oracle — 07/src/cek.py (the frozen Python interpreter) run on the file,
    its stdout captured;
  • the port    — self/cek.lark, driven through the Python CEK, evaluating the
...
Pass 1.5 — SELFHOST §7); instead the driver is built by CONCATENATION:
...
Usage:
    python3 self/tests/cek_difftest.py [-v]      # -v: show a diff on failure
"""
