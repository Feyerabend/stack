"""
Differential test for the *typechecking* self-hosted compiler (SELFHOST M5.5 / F2⁺).

F2 closed the bootstrap with `lex → parse → emit` (the emitter drives off the
...
and prints `io` exactly once.  Same M0 IO idiom, now dictating compiler shape.
...
Usage:
    python3 self/tests/typecheck_difftest.py [-v]
"""
