"""
Differential test for the self-hosted parser (SELFHOST M2).

For every .lark file in the corpus it compares two pretty-printed ASTs:

  • the oracle — 07/src/parser.py (the frozen Python parser), serialized here
    into a canonical S-expression;
  • the port    — self/parse.lark (which imports self/lex.lark's tokenizer),
    run through the CEK interpreter (07/src/cek.py), printing the same form.
...
lex.lark type-checks standalone.  (Logged as a SELFHOST §7 finding.)  Instead
...
Usage:
    python3 self/tests/parse_difftest.py [-v]      # -v: show a diff on failure
"""
