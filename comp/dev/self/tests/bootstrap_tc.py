"""
F2⁺ native fixpoint — the *typechecking* self-hosted compiler reproduces itself.
...
that path.  This harness closes the stronger F2⁺ fixpoint: the compiler
...
source from stdin (like `bootstrap.py`) instead of embedding one corpus file.
    ...
    stage0  = the Python oracle (07/src/emit_c_ast.py) compiles tc_compiler.lark
...
This was OPTIMIZE.md §8's motivated first target, and it hit TWO no-GC-arena walls:
(§8a) the emitter's O(n^2) line join — fixed with a balanced join (`self/emit_c.lark`
`ecJoinNL`/`ecJoinPairs`); and (§8b) the infer pass's assoc-list `Subst` applied
O(n) times and never freed, which overflowed >12 GB.  The §8b fix — `types.lark`
...
Usage:
    python3 self/tests/bootstrap_tc.py                  # full ladder: C1==C2==C3
    python3 self/tests/bootstrap_tc.py --stages 2       # minimal: C1==C2
    python3 self/tests/bootstrap_tc.py --arena-mb 14336 # arena size (MiB)
    python3 self/tests/bootstrap_tc.py --keep DIR       # keep artifacts
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="F2+ typechecking native fixpoint")
