...
static Term *parse_atom(Parser *p, NameCtx *ctx) {
    ...
    if (isalpha(c) || c == '_') {
        ...
        if (strcmp(name, "W") == 0) {
            /* 'W' alone is the W-type binder, so it can never be a variable.
               Without this check the failure is a bare "expected '('" at the
               point of USE, which misdirects (bit once: V3-TFN, 2026-07-16). */
            if (peek(p) != '(') {
                fprintf(stderr, "parse: 'W' is reserved (the W-type binder: W(x : A). B) "
                                "and cannot be used as a variable name at ...%s\n",
                                p->src + p->pos);
                return NULL;
            }
            if (!expect(p, '(')) return NULL;
