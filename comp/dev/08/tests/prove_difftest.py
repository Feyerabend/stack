"""
The verification suite — PROVE.md V1.4.

Every file in lark/prove/ is a claim, and this harness is where the claim is
...
program is an ordinary Lark program.
    ...
    python3 08/tests/prove_difftest.py       # or: make -C 08 prove

⛔ Reads 07/ for nothing at all. This suite is entirely the fork's. (PROVE.md §0.1)
"""
...
EXPECT: dict[str, tuple] = {
    ...
    # The borrow idiom, and the row that has moved once per rung — which is what a
    # thermometer is for.  V1: 0 of 3, no refined tuple components.  V2.0: 1 of 3 —
    # the CONSUMER (`take(b2, 0)`), because the facts now survive the tuple pattern,
    # and no further, because the PRODUCER's body needs `size(Buf(n)) == n`, which
    # products cannot create.  V2.2: 3 of 3.  `size` stopped being an `fn` and became
    ...
    # V2.0 on its own, with no measure needed: a pair whose components carry facts
    ...
    # V2.1 declared the measure; V2.2 shows its equations to the solver.  The one
    ...
    # V2.2 — the equations in the logic, and the pair that actually exercises the two
    ...
    # Nil — and all three are caught.  Under V1 the last of those was not a failed
    ...
    #   because nobody has to call it for it to fire.  V1's one fact about a UF symbol
    ...
    # contract the callers trust and nobody checks is the false proof V2.2′ was about.
    #
    ...
    # human wrote the mutant for.  `08/tests/adversary.py` generates programs, checks
    ...
    #
    # 18 is one rule at the wrong polarity.  V2.2′ taught the checker to DROP a
    ...
    # 21 is V2.3: the measure's declared result, PROVED by structural induction over
    ...
    # `NONNEG_UF` — the set literal V1 conceded it "strictly should not have to" know —
    ...
    # 23 is V2.4: a BOOL-VALUED measure.  An Int measure hangs on `==` and congruence
    ...
    # V2.4b — the POLYMORPHIC-field thermometer.  The same allpos/head/divide shape as
    ...
    # V2.4c step 1 — `min`/`max` as UNINTERPRETED symbols with their defining axioms,
    ...
    # V2.4c step 2 — `if` in PREDICATE position.  A guarded predicate translates to
    ...
    # V2.4c step 3, Part A — the min/max-speaking adversary's first catch, and a
    ...
    # V2.2′ established (no term is ever built at sort FLOAT) is realised through
    ...
    # documented as a boundary in PROVE.md (beside Z-vs-32-bit and partial correctness), not
    ...
    # unproved — the V1 distinction: a bug in the source is not a weak proof.
    #
    # 11_measure_nonstructural is the one with teeth, and V2.2 is what armed it.  Its
    ...
    # instantiated at.  V2.1's structural check is what stands between the axioms and
    # that, and it is load-bearing for V2.2 rather than good manners.
    ...
    # NESTED MATCH IN A MEASURE — 34, PROVE V2.4c step 3 Part B.  `rdepth` looks at a
    ...
    # THE BINARY SEARCH TREE — 35, PROVE V2.4c step 4, the invariant the whole axis was
    # pointed at.  No new checker mechanism: it is Part B's nested match (now two deep, on
    # both children), V2.4's Bool measure, and a measure body that MENTIONS another measure
    ...
    # THE DESTRUCTING BINARY SEARCH TREE — 36, PROVE V2.4c step 5, the boundary 35 named
