...
Optimization levels bundle passes:
  ...
  -O1 : Tier-1 scalar passes on TAC — const fold/prop, algebraic simplify,
        copy prop, DCE, CSE.   [populated in milestone O1]
  -O2+: Tier-2/3/4.            [later milestones]
...
Correctness contract (OPTIMIZE.md §2, §6): once passes exist, the guard is
...
# to a fixpoint (O3 increment 2), a per-call `site = 0` would re-mint `.i0_…` on
...
#     (-1 / dividend) where the CEK raises — divergent (OPTIMIZE.md §8).
...
#     what keeps float `==` (and NaN) off this path (float folding deferred, §8d).
...
# could be a float (where `x - x` is NaN, not 0) — see OPTIMIZE.md §8d.
...
# job (O1 increment 2), where the backend paths are checked. The map is reset per
...
#
# Tier-4 instruction selection (OPTIMIZE.md §4). The generator materialises every
...
#
# Tier-4 branch layout (OPTIMIZE.md §4). Every `ICondJump` lowers to a taken
...
PASSES: list[tuple[str, object]] = [
    ...
    ("const_fold", const_fold),            # O1 increment 2
    ("copy_prop",  copy_prop),             # O1 increment 1
    ("algebraic",  algebraic_simplify),    # O1 increment 2
    ("cse",        cse),                   # O1 increment 2 — see CSE_ELIGIBLE below
    ("dce",        dce),                   # O1 increment 1
...
# -- CSE eligibility invariant (OPTIMIZE.md §8d item [4]) ---------------------
#
...
def wants_graph_coloring(opts: OptOptions) -> bool:
    """Whether asm.gen should use the graph-coloring allocator (O4 increment 2)
...
# Iterate the whole enabled sweep to a fixpoint (O3 increment 2). One sweep of
