...
def compose(s1: Subst, s2: Subst) -> Subst:
    ...
    _bind refuses the degenerate binding k |-> k, but _bind is not the only way a
    binding enters a substitution.  Composing {16 |-> a5} with {5 |-> a16} — two
    mappings each perfectly acyclic — applies the first to the range of the second
    and yields 5 |-> a5, a self-mapping no _bind ever saw.  apply() resolves a
    variable by chasing the chain, so that binding is not a wrong answer but a
    non-terminating one.

    k |-> k *is* the identity map, so dropping it is sound and total: it changes
    no answer and it ends the chain.  It is also all we guard: the real defect was
    check_fn_decl discarding an annotation it had already been given (see there),
    and with that repaired this case does not arise.  A belt is not a reason to
    keep wearing braces you cannot fasten in both languages.
    """
    result: Subst = {}
    for k, v in s2.items():
        nv = apply(s1, v)
        if isinstance(nv, TVar) and nv.id == k:
            continue                       # k |-> k is the identity; drop it
        result[k] = nv
    result.update(s1)
    return result
