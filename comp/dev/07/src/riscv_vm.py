...
class LarkVM:
    ...
    def __init__(self, mem_size: int = 65536) -> None:
        ...
        # -- Profiling counters (read after run(); output-neutral). --
        self.dyn_instrs  = 0     # real RV32I instructions retired
        self.stub_calls  = 0     # runtime-stub invocations intercepted
        self.heap_allocs = 0     # calls into _heap_alloc (every heap object)
        self.heap_bytes  = 0     # total bytes bump-allocated (4-byte aligned)
    ...
    def load(self, binary: bytes, labels: dict[str, int]) -> None:
        ...
        stub_map = {
            ...
            'string_index':    self._rt_string_index,
            'string_slice':    self._rt_string_slice,
            'char_to_string':  self._rt_char_to_string,
            'float_to_bits':   self._rt_float_to_bits,
            '__string_to_int_raw':   self._rt_string_to_int_raw,
            '__string_to_float_raw': self._rt_string_to_float_raw,
        }
    ...
    # -- string-decomposition prims (self-host M0) --
    # Byte-for-byte mirrors of cek.py's builtins; the self-host corpus is ASCII
    # so codepoint == byte and the CEK (codepoint) / C (byte) worlds agree.

    def _rt_string_index(self) -> None:
        """string_index(a0=str_ptr, a1=idx) → a0=codepoint, or -1 out of bounds"""
        s   = self._read_string(self.regs[10])
        idx = self.signed(self.regs[11])
        if 0 <= idx < len(s):
            self.regs[10] = ord(s[idx]) & 0xFFFFFFFF
        else:
            self.regs[10] = 0xFFFFFFFF          # -1
        self.pc = self.regs[1]

    def _rt_string_slice(self) -> None:
        """string_slice(a0=str_ptr, a1=lo, a2=hi) → a0=substring_ptr (clamped)"""
        s  = self._read_string(self.regs[10])
        lo = self.signed(self.regs[11])
        hi = self.signed(self.regs[12])
        n  = len(s)
        a  = max(0, min(lo, n))
        b  = max(0, min(hi, n))
        self.regs[10] = self._alloc_string(s[a:b] if a < b else "")
        self.pc = self.regs[1]

    def _rt_char_to_string(self) -> None:
        """char_to_string(a0=codepoint) → a0=one-char-string_ptr (byte-masked)"""
        self.regs[10] = self._alloc_string(chr(self.regs[10] & 0xFF))
        self.pc = self.regs[1]

    def _rt_float_to_bits(self) -> None:
        """float_to_bits(a0=float32_bits) → a0=uint32.  Identity: the register
        already holds the IEEE-754 f32 bit pattern (that is how floats are stored
        in this backend), which is exactly the unsigned-int reinterpretation.
        CONTRACT (OPTIMIZE §8d [5b]): this is the identity ONLY because of the f32
        bit-pattern float repr — a boxed float or a double (f64) repr would make
        this a real reinterpret/convert, and this stub (plus every float op) would
        need rework.  Note tac_vm._builtin_float_to_bits does a real struct
        unpack (native Python floats there), so the two backends agree by value."""
        self.regs[10] = self.regs[10] & 0xFFFFFFFF
        self.pc = self.regs[1]

    @staticmethod
    def _parse_int_strict(s: str):
        """Signed decimal, no whitespace/underscores/prefixes; wraps to i32.
        Returns None if malformed (mirrors cek._parse_int)."""
        if s == "":
            return None
        i, neg = 0, False
        if s[0] in "+-":
    ...
    @staticmethod
    def _parse_float_strict(s: str):
        """Lexer FLOAT shape: optional sign, digits, '.', digits (no exponent).
        Returns None if malformed (mirrors cek._parse_float)."""
        if s == "":
            return None
        i = 0
        if s[0] in "+-":
        ...
        while i < len(s) and "0" <= s[i] <= "9":
            i += 1; before += 1
        if before == 0 or i >= len(s) or s[i] != ".":
            return None
        i += 1
        after = 0
        while i < len(s) and "0" <= s[i] <= "9":
            i += 1; after += 1
        if after == 0 or i != len(s):
            return None
        return float(s)

    def _rt_string_to_int_raw(self) -> None:
        """__string_to_int_raw(a0=str_ptr) → a0=ptr to [word0, flag, payload].
        flag=1 → payload = parsed i32; flag=0 → payload = error-msg str_ptr.
        lower._lower_string_to_result wraps this into Ok/Err (tags via
        asm._collect_tags).  CONTRACT (OPTIMIZE §8d [5a]): word0 is the ADT tag
        slot that IGetField skips — lower reads flag via IGetField(idx=0)→word1 and
        payload via IGetField(idx=1)→word2, so the pack order below (word0, flag,
        payload) must match IGetField's `(idx+1)*4`. Not a real ADT; borrowed
        offsets. Change one side → change both; no test isolates the coupling."""
        n   = self._parse_int_strict(self._read_string(self.regs[10]))
        rec = self._heap_alloc(12)
        if n is None:
            # COVERAGE GAP (OPTIMIZE §8d [5c]): this Err string must match cek.py's
            # wording to keep the backends value-equal, but no corpus test compares
            # it — show_result/show_fresult discard the Err payload. If the wording
            # drifts between backends, nothing fails. Keep in sync with cek.py by
            # hand (or add a test that prints the Err payload).
            msg = self._alloc_string("string_to_int: not an integer")
            struct.pack_into('<III', self.memory, rec, 0, 0, msg)
        else:
            struct.pack_into('<III', self.memory, rec, 0, 1, n & 0xFFFFFFFF)
        self.regs[10] = rec
        self.pc = self.regs[1]

    def _rt_string_to_float_raw(self) -> None:
        """__string_to_float_raw(a0=str_ptr) → a0=ptr to [word0, flag, payload].
        flag=1 → payload = f32 bit pattern (backend float repr, §8d [5b]); flag=0 →
        payload = error-msg str_ptr.  Wrapped into Ok/Err by lower.py.  Same 3-word
        [word0=tag-slot, flag@word1, payload@word2] contract as _rt_string_to_int_raw
        (§8d [5a]); the float payload is stored as its f32 bits, not a boxed double."""
        f   = self._parse_float_strict(self._read_string(self.regs[10]))
        rec = self._heap_alloc(12)
        if f is None:
            msg = self._alloc_string("string_to_float: not a float")
            struct.pack_into('<III', self.memory, rec, 0, 0, msg)
        else:
            struct.pack_into('<III', self.memory, rec, 0, 1, self._f32_bits(f))
        self.regs[10] = rec
        self.pc = self.regs[1]
