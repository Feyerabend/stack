...
from exhaust import (
    ExhaustivenessError, BUILTIN_SIGS,
    check_texpr as _check_exhaustive,
)
from total import TotalityError, check_program as _check_totality
...
class AffineError(Exception):
    def __init__(self, name: str, msg: "str | None" = None) -> None:
        self.name = name
        super().__init__(msg or f"affine variable '{name}' used more than once")

class TraitBoundError(Exception):
    def __init__(self, method: str, typ: ty.Mono) -> None:
        super().__init__(f"no Show instance for {pretty(typ)}")


# The complete set of diagnostics the frontend may raise.  Every CLI runner
# catches exactly this tuple and exits 1 with a one-line message; anything
# else escaping the frontend is a bug (see tests/fuzz_frontend.py, P8).
DIAGNOSTICS = (
    LexError, _parser.ParseError,
    TypeError, AffineError, TraitBoundError,
    ExhaustivenessError, TotalityError,
)
...
def infer_pat(
    pat: Pat, fresh: Fresh, env: "Env | None" = None,
) -> tuple[TPat, Mono, Env, Subst]:
    """Infer the type of a pattern and the bindings it introduces.

    Returns (typed_pattern, pattern_type, local_env, substitution).
    When env is provided, constructor patterns look up the constructor's type
    and unify sub-pattern types with the declared field types — this ensures
    pattern-bound variables carry concrete types (e.g. `n` in `Ok(n)` gets
    type Int when Ok : Int → Result(Int, String)).
    """
    match pat:
        case PWild():
            t = fresh.var()
        ...
        case PCon(name=n, args=args):
            ...
            if env is not None and n in env:
                # Instantiate the constructor type and extract field types.
                con_t = instantiate(fresh, env[n])
                field_types: list[Mono] = []
                con_result = con_t
                while isinstance(con_result, TFn):
                    field_types.append(con_result.param)
                    con_result = con_result.result

                if len(field_types) == len(args):
                    for ft, a in zip(field_types, args):
                        tp, pt, env2, s2 = infer_pat(a, fresh, env)
                        s3 = _unify_wrap(apply(compose(s2, s), pt),
                                         apply(compose(s2, s), ft), pat)
                        s = compose(s3, compose(s2, s))
                        typed_args.append(_apply_tpat(s, tp))
                        for k, sc in env2.items():
                            if k in local:
                                raise TypeError(f"repeated pattern variable: {k!r}")
                            local[k] = apply_scheme(s, sc)
                    result_t = apply(s, con_result)
                    local = {k: apply_scheme(s, sc) for k, sc in local.items()}
                    return TPCon(n, tuple(typed_args), result_t), result_t, local, s

            # Fallback: no env, or constructor not in env.
            result_t = fresh.var()
            for a in args:
                tp, _t, env2, s2 = infer_pat(a, fresh, env)
                s = compose(s2, s)
        ...
        case PTuple(elems=elems):
            ...
            s = {}
            for e in elems:
                tp, t, env2, s2 = infer_pat(e, fresh, env)
                s = compose(s2, s)
            ...
            return TPTuple(tuple(typed_elems), t), t, local, s


# -- Free variables of a surface expression --
# Needed by the affine-capture check in the Lambda rule: closures are Copy
# (a function value may be called any number of times), so a lambda that
# *captures* an affine variable could hand out the same resource on every
# call — `let g = fn(x) => io in (g(1), g(2))` duplicates io without ever
# using the name twice in the outer scope.  Found by Phase 9 hardening
# review: the lambda rule copies `tracked`, so captures never counted.

def _pat_bound_names(pat: Pat) -> frozenset[str]:
    match pat:
        case PVar(name=n):
            return frozenset({n})
        case PCon(args=args):
            ...
            for a in args:
                out |= _pat_bound_names(a)
            return out
        case PTuple(elems=elems):
            out = frozenset()
            for e in elems:
                out |= _pat_bound_names(e)
            return out
        case _:   # PWild, PLit
            return frozenset()


def _free_var_names(expr: Expr, bound: frozenset[str]) -> set[str]:
    match expr:
        case Var(name=n):
        ...
        case TupleExpr(elems=elems):
            out: set[str] = set()
            for e in elems:
                out |= _free_var_names(e, bound)
            return out
        case Apply(fn=f, args=args):
            out = _free_var_names(f, bound)
            for a in args:
                out |= _free_var_names(a, bound)
            return out
        case BinOp(left=l, right=r):
            return _free_var_names(l, bound) | _free_var_names(r, bound)
        case UnaryOp(operand=o):
            return _free_var_names(o, bound)
        case LetExpr(name=n, value=v, body=b):
            return (_free_var_names(v, bound)
                    | _free_var_names(b, bound | {n}))
        case IfExpr(cond=c, then_=t, else_=e):
            return (_free_var_names(c, bound)
                    | _free_var_names(t, bound)
                    | _free_var_names(e, bound))
        case MatchExpr(scrutinee=s, arms=arms):
            out = _free_var_names(s, bound)
            for pat, arm_body in arms:
                out |= _free_var_names(arm_body, bound | _pat_bound_names(pat))
            return out
        case Lambda(params=params, body=b):
            return _free_var_names(b, bound | {p.name for p in params})
        case _:
            return set()
...
def infer(
    ...
    match expr:
        ...
        case MatchExpr(scrutinee=scrut, arms=arms):
            ...
            for (pat, arm_body) in arms:
                tp, pt, pat_env, s_pat = infer_pat(pat, fresh, env)
                s = compose(s_pat, s)
        ...
        case Lambda(params=params, body=body):
            # Affine-capture check: a closure may be called many times, so
            # capturing a tracked (affine) variable would duplicate the
            # resource on the second call.  Pass it as a parameter instead.
            lam_bound = frozenset(p.name for p in params)
            for cap in sorted(_free_var_names(body, lam_bound)):
                if cap in tracked:
                    raise AffineError(
                        cap,
                        f"affine variable {cap!r} captured by a lambda — a "
                        f"closure may be called many times, which would "
                        f"duplicate the resource; pass it as a parameter "
                        f"instead")
...
def _apply_tpat(s: Subst, pat: TPat) -> TPat:
    match pat:
        case TPWild(typ=t):
            return TPWild(apply(s, t))
        case TPVar(name=n, typ=t):
            return TPVar(n, apply(s, t))
        case TPLit(value=v, typ=t):
            return TPLit(v, apply(s, t))
        case TPCon(name=n, args=args, typ=t):
            return TPCon(n, tuple(_apply_tpat(s, a) for a in args), apply(s, t))
        case TPTuple(elems=elems, typ=t):
            return TPTuple(tuple(_apply_tpat(s, e) for e in elems), apply(s, t))
        case _:
            return pat


def _apply_texpr(s: Subst, expr: TExpr) -> TExpr:
    """Apply substitution s to all type annotations in a typed expression tree."""
    match expr:
        case TLit(value=v, typ=t):
            return TLit(v, apply(s, t))
        case XVar(name=n, typ=t):
            return XVar(n, apply(s, t))
        case XCon(name=n, typ=t):
            return XCon(n, apply(s, t))
        case TTupleExpr(elems=elems, typ=t):
            return TTupleExpr(tuple(_apply_texpr(s, e) for e in elems), apply(s, t))
        case XApply(fn=fn, args=args, typ=t):
            return XApply(_apply_texpr(s, fn), tuple(_apply_texpr(s, a) for a in args), apply(s, t))
        case TBinOp(op=op, left=l, right=r, typ=t):
            return TBinOp(op, _apply_texpr(s, l), _apply_texpr(s, r), apply(s, t))
        case TUnaryOp(op=op, operand=o, typ=t):
            return TUnaryOp(op, _apply_texpr(s, o), apply(s, t))
        case TLetExpr(name=n, value=v, body=b, typ=t):
            return TLetExpr(n, _apply_texpr(s, v), _apply_texpr(s, b), apply(s, t))
        case TIfExpr(cond=c, then_=th, else_=el, typ=t):
            return TIfExpr(_apply_texpr(s, c), _apply_texpr(s, th), _apply_texpr(s, el), apply(s, t))
        case TMatchExpr(scrutinee=sc, arms=arms, typ=t):
            new_arms = tuple((_apply_tpat(s, p), _apply_texpr(s, b)) for p, b in arms)
            return TMatchExpr(_apply_texpr(s, sc), new_arms, apply(s, t))
        case TLambda(params=params, body=b, typ=t):
            new_params = tuple((n, apply(s, pt)) for n, pt in params)
            return TLambda(new_params, _apply_texpr(s, b), apply(s, t))
        case _:
            return expr
...
# Phase 9; rejected here so no backend ever sees such a program.
...
def _show_satisfies(typ: ty.Mono, show_types: frozenset[str]) -> bool:
    """True if typ concretely implements Show."""
    match typ:
        case TVar():
            return False
        case TCon(name=n):
            return n in show_types
        case TApp(head=h):
            return h in show_types
        case _:
            return False


def _check_show_bounds(expr: TExpr, show_types: frozenset[str]) -> None:
    """Walk a substitution-applied typed expression and raise TraitBoundError
    for any direct call `show(arg)` where arg's type lacks a Show instance."""
    match expr:
        case XApply(fn=XVar(name="show"), args=args):
            if args and not _show_satisfies(args[0].typ, show_types):
                raise TraitBoundError("show", args[0].typ)
            for a in args:
                _check_show_bounds(a, show_types)
        case XApply(fn=fn_expr, args=args):
            _check_show_bounds(fn_expr, show_types)
            for a in args:
                _check_show_bounds(a, show_types)
        case TTupleExpr(elems=elems):
            for e in elems:
                _check_show_bounds(e, show_types)
        case TBinOp(left=l, right=r):
            _check_show_bounds(l, show_types)
            _check_show_bounds(r, show_types)
        case TUnaryOp(operand=o):
            _check_show_bounds(o, show_types)
        case TLetExpr(value=v, body=b):
            _check_show_bounds(v, show_types)
            _check_show_bounds(b, show_types)
        case TIfExpr(cond=c, then_=t, else_=e):
            _check_show_bounds(c, show_types)
            _check_show_bounds(t, show_types)
            _check_show_bounds(e, show_types)
        case TMatchExpr(scrutinee=s, arms=arms):
            _check_show_bounds(s, show_types)
            for _, body in arms:
                _check_show_bounds(body, show_types)
        case TLambda(body=b):
            _check_show_bounds(b, show_types)
        case _:
            pass  # TLit, TVar, TCon — no sub-expressions
...
def check_fn_decl(
    ...
    fresh: Fresh,
    copy_types: frozenset[str],
    show_types: frozenset[str] = frozenset(),
...
def _initial_env() -> tuple[Env, frozenset[str]]:
    ...
    env: Env = {
        ...
        "int_abs":         Scheme((), TFn(T_INT,    T_INT)),
        "float_abs":       Scheme((), TFn(T_FLOAT,  T_FLOAT)),
        "float_sqrt":      Scheme((), TFn(T_FLOAT,  T_FLOAT)),
        "float_floor":     Scheme((), TFn(T_FLOAT,  T_FLOAT)),
        "float_ceil":      Scheme((), TFn(T_FLOAT,  T_FLOAT)),
        "string_length":   Scheme((), TFn(T_STRING, T_INT)),
...
def _fn_type_from_annotations(decl: "FnDecl", fresh: Fresh) -> "Mono | None":
    """Extract the full function type from a FnDecl's annotations.
    Returns None if any param or the return type is unannotated.
    Used by the mutual-recursion pre-pass.
    """
    if decl.return_type is None:
        return None
    tvar_env: dict[str, TVar] = {}
    param_monos: list[Mono] = []
    for p in decl.params:
        if p.ann is None:
            return None
        param_monos.append(syntype_to_mono(p.ann, tvar_env, fresh))
    ret_t = syntype_to_mono(decl.return_type, tvar_env, fresh)
    fn_t: Mono = ret_t
    for pt in reversed(param_monos):
        fn_t = TFn(pt, fn_t)
    return fn_t
...
def typecheck(
    ...
    """
    _parser._ensure_stack()   # deep ASTs need headroom (see parser.py)
    env, copy_types = _initial_env()
    fresh = Fresh()
    typed_decls: list[TDecl] = []
    # Primitives always implement Show; user types are added in Pass 1.
    show_types: frozenset[str] = frozenset({"Int", "Float", "Bool", "String"})
    # Constructor signatures per ADT head, for exhaustiveness checking.
    ctor_sigs: dict[str, tuple[str, ...]] = dict(BUILTIN_SIGS)
    if _visited is None:
        _visited = set()

    # Pass 0: resolve imports — load and type-check each imported module,
    # then merge exported names into the current env and prepend their typed
    # declarations so the lowerer/backends can see the function bodies.
    if source_file:
        src_dir = os.path.dirname(os.path.abspath(source_file))
        for imp in program.imports:
            copy_types, imp_decls = _load_import(
                imp, src_dir, env, copy_types, fresh, _visited
            )
            typed_decls.extend(imp_decls)
            # Imported ADTs are matchable here; record their signatures.
            for d in imp_decls:
                if isinstance(d, TTypeDecl) and d.variants:
                    ctor_sigs[d.name] = tuple(v.name for v in d.variants)

    # Pass 1: register type declarations, trait methods, Copy impls, and Show impls.
    # Copy and Show impls must be processed here so pass 2 sees them.
    for decl in program.decls:
        if isinstance(decl, TypeDecl):
            copy_types = _register_type_decl(decl, env, fresh, copy_types)
            if isinstance(decl.body, tuple):
                ctor_sigs[decl.name] = tuple(v.name for v in decl.body)
        ...
        elif isinstance(decl, ImplDecl) and decl.trait_name == "Show":
            show_types = show_types | frozenset({_for_type_name(decl)})

    # Pass 1.2: totality — purely syntactic, needs only binding structure.
    _check_totality(program)

    # Pass 1.5: pre-register fully-annotated function types so that mutually
    # recursive functions can find each other during body type-checking.
    # Functions without complete annotations are skipped; they fall back to
    # the sequential (self-recursion-only) path.
    for decl in program.decls:
        if isinstance(decl, FnDecl) and decl.name not in env:
            ann_t = _fn_type_from_annotations(decl, fresh)
            if ann_t is not None:
                env[decl.name] = Scheme((), ann_t)

    # Pass 2: check value declarations and impl blocks.
    for decl in program.decls:
        match decl:
            case FnDecl():
                tdecl, sc = check_fn_decl(decl, env, fresh, copy_types, show_types)
                _check_exhaustive(tdecl.body, env, ctor_sigs, fresh)
                env[decl.name] = sc
                typed_decls.append(tdecl)

            case LetDecl():
                tdecl, sc = check_let_decl(decl, env, fresh, copy_types, show_types)
                _check_exhaustive(tdecl.value, env, ctor_sigs, fresh)
                env[decl.name] = sc
                typed_decls.append(tdecl)
            ...
            case ImplDecl():
                ...
                for m in decl.methods:
                    ...
                    tdecl, _ = check_fn_decl(fn, env, fresh, copy_types, show_types)
                    _check_exhaustive(tdecl.body, env, ctor_sigs, fresh)
...
def _load_import(
    ...
    Returns (updated copy_types, list of imported TDecls to inline).
    """
    ...
    # Return all typed declarations from the module so backends can lower them.
    return copy_types, list(imported_tprog.decls)
...
if __name__ == "__main__":
    ...
    try:
        ...
        pprint.pprint(tprog)
    except (TypeError, AffineError, ExhaustivenessError, TotalityError) as e:
