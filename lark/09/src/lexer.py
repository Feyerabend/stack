    ...
    def _read_number(self, line: int, col: int) -> Token:
        ...
        if self._peek() == "." and self._peek(1).isdigit():
            ...
            try:
                ...
                raise self._error(f"invalid float literal {text!r}", line, col)
        text = self.source[start:self.pos]
        # int() can fail even though every character passed isdigit():
        # str.isdigit() accepts superscripts like '²', and CPython refuses
        # int-str conversion beyond ~4300 digits.  Both get a diagnostic.
        try:
            return self._tok(TK.INT, text, int(text), line, col)
        except ValueError:
            raise self._error(f"invalid integer literal {text!r}", line, col)
