...
def _lit_val(v: object) -> Value:
    ...
    if isinstance(v, int):   return VInt(_i32(v))
...
def binop(op: str, l: Value, r: Value) -> Value:
    ...
    if isinstance(l, VInt) and isinstance(r, VInt):
        match op:
            case "+":  return VInt(_i32(l.n + r.n))
            case "-":  return VInt(_i32(l.n - r.n))
            case "*":  return VInt(_i32(l.n * r.n))
            case "/":  # RISC-V DIV semantics: truncate toward zero; x/0 = -1
                if r.n == 0: return VInt(-1)
                return VInt(_i32(int(l.n / r.n)))
            case "%":  # RISC-V REM semantics: x%0 = x
                if r.n == 0: return VInt(l.n)
                return VInt(_i32(l.n - int(l.n / r.n) * r.n))
...
def unaryop(op: str, v: Value) -> Value:
    if op == "-":
        if isinstance(v, VInt):   return VInt(_i32(-v.n))
...
def builtin(name: str, arg: Value, kont: list[Frame], m: Machine) -> State:
    match name:
        ...
        case "float_to_int":
            ...
            return Return(VInt(_i32(int(_f32(arg.f)))), kont)
        case "int_to_string":
            return Return(VStr(show(arg)), kont)
        ...
        case "int_abs":
            assert isinstance(arg, VInt)
            return Return(VInt(_i32(abs(arg.n))), kont)
        case "float_abs":
            assert isinstance(arg, VFloat)
