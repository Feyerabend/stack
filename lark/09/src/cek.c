...
static char   _arena_buf[LARK_ARENA_SIZE];
static LkArena _ar;
...
static LkVal binop(const char* op, LkVal l, LkVal r) {
    ...
    if (l.kind == V_INT && r.kind == V_INT) {
        /* Lark Int is wrapping two's-complement i32 (RISC-V semantics).
         * Arithmetic is done in uint32_t: unsigned overflow is defined in C,
         * signed overflow is not. */
        int32_t a = l.i, b = r.i;
        uint32_t ua = (uint32_t)a, ub = (uint32_t)b;
        if (strcmp(op, "+")  == 0) return lk_int((int32_t)(ua + ub));
        if (strcmp(op, "-")  == 0) return lk_int((int32_t)(ua - ub));
        if (strcmp(op, "*")  == 0) return lk_int((int32_t)(ua * ub));
        if (strcmp(op, "/")  == 0) {
            /* RISC-V DIV: x/0 = -1; INT_MIN / -1 = INT_MIN (C would trap). */
            if (b == 0) return lk_int(-1);
            if (a == INT32_MIN && b == -1) return lk_int(INT32_MIN);
            return lk_int(a / b);
        }
        if (strcmp(op, "%")  == 0) {
            /* RISC-V REM: x%0 = x; INT_MIN % -1 = 0. */
            if (b == 0) return lk_int(a);
            if (a == INT32_MIN && b == -1) return lk_int(0);
            return lk_int(a % b);   /* C '%' truncates toward zero, as RV32 */
        }
...
static LkVal unaryop(const char* op, LkVal v) {
    if (strcmp(op, "-") == 0) {
        if (v.kind == V_INT)   return lk_int((int32_t)(0u - (uint32_t)v.i));
...
static LkVal apply_builtin(const char* name, LkVal arg) {
    ...
    if (strcmp(name, "int_abs") == 0) {
        BUILTIN_EXPECT("Int", V_INT);
        /* Wrapping i32: int_abs(INT32_MIN) = INT32_MIN, matching the
         * Python backends and RV32 (negation wraps). */
        int32_t v = arg.i;
        return lk_int(v < 0 ? (int32_t)(0u - (uint32_t)v) : v);
    }
...
void cek_run(const LkProg* prog) {
    _ar   = (LkArena){ .base = _arena_buf, .cap = LARK_ARENA_SIZE, .used = 0 };
    _ksp  = 0;
    _top_n = 0;
    _disp_n = 0;
    _con_n  = 0;
