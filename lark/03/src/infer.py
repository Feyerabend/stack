...
from typed_tree import (
    TProgram, TDecl, TFnDecl, TLetDecl, TTypeDecl, TVariant,
...
def is_copy(t: Mono, copy_types: frozenset[str]) -> bool:
    match t:
        case TVar():
            return False   # conservative: unknown type is not Copy
        case TCon(name=n):
...
def typecheck(program: Program) -> TProgram:
    ...
    # Pass 1: register all type declarations so constructors are in scope.
    for decl in program.decls:
        if isinstance(decl, TypeDecl):
            copy_types = _register_type_decl(decl, env, fresh, copy_types)

    # Pass 2: check value declarations in order.
    for decl in program.decls:
        match decl:
            case FnDecl():
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    prog = parser_mod.parse_file(path)

    try:
        tprog = typecheck(prog)
        pprint.pprint(tprog)
