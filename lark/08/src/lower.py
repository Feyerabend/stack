...
class Lowerer:
    ...
    def lower(self, tprog: TProgram) -> TAC:
        ...
        for decl in tprog.decls:
            if isinstance(decl, TFnDecl):
                ...
                for m in decl.methods:
                    self._global_fns.add(m.name + "$" + decl.for_type)
    ...
    def _expr(self, expr: TExpr, fn: Function, env: dict[str, Val]) -> Val:
        ...
        match expr:
            ...
            case TVar(name=n):
                if n in env:
                    return env[n]
                # Global function used as a value — wrap in a closure stub.
                if n in self._global_fns:
                    dst = fn.fresh("fn")
                    fn.emit(IAllocClosure(dst, n, ()))
                    return dst
                return Tmp(n)   # shouldn't happen after type checking
    ...
    def _lower_match(
        ...
        # Phase 8: statically unreachable — the exhaustiveness checker
        # guarantees some arm matches.  Kept as defence in depth so the IR
        # stays well-formed without trusting the frontend.
        fn.emit(ICall(None, "__lark_match_fail", ()))
        fn.emit(ILabel(l_end))
        return dst
