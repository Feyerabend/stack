...
/* ── weaken primitive type cache */

static int wkc_init      = 0;
static int wkc_expr_fam  = -1;
static int wkc_ctx_fam   = -1;
static int wkc_ty_fam    = -1;
static int wkc_ext_ctor  = -1;

static void init_wkc(void) {
    if (wkc_init) return;
    wkc_init     = 1;
    wkc_expr_fam = ind_lookup("Expr");
    wkc_ctx_fam  = ind_lookup("Ctx");
    wkc_ty_fam   = ind_lookup("Ty");
    if (wkc_ctx_fam >= 0)
        wkc_ext_ctor = ind_ctor_lookup(wkc_ctx_fam, "ext");
}
...
static int conv_spine(Arena *a, int depth, Spine *sp1, Spine *sp2) {
    ...
    } else if (sp1->kind == SP_WEAKEN) {
        if (!conv(a, depth, sp1->weaken_s.a_ty,  sp2->weaken_s.a_ty))  return 0;
        if (!conv(a, depth, sp1->weaken_s.ctx_g, sp2->weaken_s.ctx_g)) return 0;
        if (!conv(a, depth, sp1->weaken_s.ty_t,  sp2->weaken_s.ty_t))  return 0;
    }
...
Val *infer(Arena *a, int depth, TCtx *tctx, Env *env, Term *t) {
    ...
    case TM_WEAKEN: {
         ...
         * Trusted primitive: check e : Expr g t, return Expr (ext a g) t. */
        init_wkc();
        if (wkc_expr_fam < 0 || wkc_ctx_fam < 0 || wkc_ty_fam < 0) {
            fprintf(stderr, "type error: 'weaken' used before Expr/Ctx/Ty are defined\n");
            return NULL;
        }
        /* Evaluate a, g, t to build expected types */
        Val *a_val = nbe_eval(a, env, t->weaken.ty_a);
        Val *g_val = nbe_eval(a, env, t->weaken.ctx_g);
        Val *t_val = nbe_eval(a, env, t->weaken.ty_t);
        /* Check body : Expr g t */
        Val **ty_args = (Val **)arena_alloc(a, 2 * sizeof(Val *));
        ty_args[0] = g_val; ty_args[1] = t_val;
        Val *ty_val = vl_indtype(a, wkc_expr_fam, 2, ty_args);
        if (!check(a, depth, tctx, env, t->weaken.body, ty_val)) return NULL;
        /* Return Expr (ext a g) t */
        Val **ext_args = (Val **)arena_alloc(a, 2 * sizeof(Val *));
        ext_args[0] = a_val; ext_args[1] = g_val;
        Val *ext_a_g = vl_indcon(a, wkc_ctx_fam, wkc_ext_ctor, 2, ext_args);
        Val **ret_args = (Val **)arena_alloc(a, 2 * sizeof(Val *));
        ret_args[0] = ext_a_g; ret_args[1] = t_val;
        return vl_indtype(a, wkc_expr_fam, 2, ret_args);
    }
