...
def binop(op: str, l: Value, r: Value) -> Value:
    ...
    if isinstance(l, VFloat) and isinstance(r, VFloat):
        match op:
            case "+":  return VFloat(l.f + r.f)
            case "-":  return VFloat(l.f - r.f)
            case "*":  return VFloat(l.f * r.f)
            case "/":  return VFloat(l.f / r.f)
            case "==": return VBool(l.f == r.f)
            case "!=": return VBool(l.f != r.f)
            case "<":  return VBool(l.f <  r.f)
            case "<=": return VBool(l.f <= r.f)
            case ">":  return VBool(l.f >  r.f)
...
def unaryop(op: str, v: Value) -> Value:
    if op == "-":
        ...
        if isinstance(v, VFloat): return VFloat(-v.f)
...
def builtin(name: str, arg: Value, kont: list[Frame], m: Machine) -> State:
    match name:
        ...
        case "float_to_int":
            ...
            return Return(VInt(int(arg.f)), kont)
        case "int_to_string":
            return Return(VStr(show(arg)), kont)
        ...
        case "float_abs":
            assert isinstance(arg, VFloat)
            return Return(VFloat(abs(arg.f)), kont)
        case "float_sqrt":
            import math as _math
            assert isinstance(arg, VFloat)
            v = _math.sqrt(arg.f) if arg.f >= 0.0 else float('nan')
            return Return(VFloat(v), kont)
        case "float_floor":
            import math as _math
            assert isinstance(arg, VFloat)
            return Return(VFloat(float(_math.floor(arg.f))), kont)
        case "float_ceil":
            import math as _math
            assert isinstance(arg, VFloat)
            return Return(VFloat(float(_math.ceil(arg.f))), kont)
        case "string_length":
