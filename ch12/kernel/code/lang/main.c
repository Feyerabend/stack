...
static int process_line(const char *raw, const char *origin, int quiet) {
    ...
    if (is_let) {
        ...
        /* With an explicit type annotation, CHECK the body against it (so a
         * definition that does not have its declared type is rejected). Without
         * an annotation there is nothing to check against, and a bare lambda
         * would not infer, so fall back to the no-check define. */
