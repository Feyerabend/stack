 ...
 * against it (bidirectionally) before registering as name.  This is the checked
 * counterpart of def_define_nocheck: a definition whose body does not have the
 * declared type is rejected (returns -1).  Bare lambdas in the body are fine as
 * long as the annotation drives checking; eliminator arguments still need their
 * usual explicit motive annotations.  Returns the def index on success, -1 on
 * failure.
 */
