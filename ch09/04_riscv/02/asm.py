    ...
    def parse_instruction(self, line: str, addr: int) -> Instruction:
        ...
        # Additional pseudo-instructions
            ...
            return Instruction('BLT',
                               rs1=self.parse_register(parts[2]),  # swapped
                               rs2=self.parse_register(parts[1]),  # swapped
                               imm=offset,
                               funct3=0b100)
            ...
            # bgtu rs1, rs2, label -> bltu rs2, rs1, label (swap operands)
            target = parts[3]
            if target in self.labels:
                offset = self.labels[target] - addr
            else:
            ...
            # beqz rs1, label -> beq rs1, zero, label
            target = parts[2]
            if target in self.labels:
                offset = self.labels[target] - addr
            else:
